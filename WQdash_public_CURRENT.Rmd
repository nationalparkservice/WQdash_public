...
pkgs <- c("flexdashboard", "shiny", "knitr",  "here", "tidyverse", "janitor", "mgcv", "plyr", "scales", "rtf", "rmarkdown", "shinyWidgets", "leaflet", "RgoogleMaps", "plotrix", "GGally", "plotly", "httr", "jsonlite", "tidyverse", "readr", 
          # "sp", 
          # "rgdal", 
          "sf", 
          # "rgeos", 
...
FuncBaseMap <- function(maptype) {
  ...
  if(maptype == "initial") { # These are specific to the initial map
    if(length(rv$sitesPointsSub[rv$sitesPointsSub$OrganizationFormalName %in% rv$userSelect$sel_Organization,]) > 0) { # Available water quality stations
  map %<>% addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$OrganizationFormalName %in% rv$userSelect$sel_Organization,], group = "Available monitoring sites", label = ~SITELAB, popup = ~SITEPOP, radius = 4, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = TRUE, weight = 1, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_availableStations"))} # WQ stations
    
      if(!is.null(rv$selectedStations) & !is.null(rv$userSelect$sel_Organization)) {
    if(nrow(rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier %in% rv$selectedStations$MonitoringLocationIdentifier[rv$selectedStations$OrganizationFormalName %in% rv$userSelect$sel_Organization],]) > 0) { # Selected stations are outlined in white (data to be imported)
    map %<>%
...
FuncParkHUCOverlay <- function(huc10_filenam, huc12_filenam , parks_filenam) {
  # Spatial intersect of park unit boundaries and HUC10 and HUC12 boundaries. Only run this function when the park or HUC boundary geodatabases are updated
  ...
  parks_sf <- sf::st_transform(parks_sf, crs = sf::st_crs(huc10_sf)) # Convert to WGS84
  if(!all(st_is_valid(parks_sf))) {parks_sf <- sf::st_make_valid(parks_sf)}
  all(st_is_valid(parks_sf))
  saveRDS(parks_sf, here::here("park_boundaries", "parks_sf.RDS"))
  rm(parks_gdb)
  
  # Read in the sf files for TESTING ONLY >>>>
  huc10_sf <- readRDS(here::here("watershed_boundaries", ("huc10_sf.RDS")))
  parks_sf <- readRDS(here::here("park_boundaries", "parks_sf.RDS"))
...
# <<<< for testing only
  ...
  # Read in the files for TESTING ONLY >>>>
...
# <<<< for testing only
  ...
  park_code = "LIRI"
  FuncSelHUC <- function(huc = "HUC10", huc_sf = huc10_overlap_sf, park_code) {
    huc_on_park <- park_sf_overlaps_list[[huc]] %>%
    dplyr::filter(unit_code == park_code) %>%
    dplyr::pull(tolower(huc))
    
    huc_sf_subset <- huc_sf %>% dplyr::filter(get(tolower(huc)) %in% huc_on_park)
    return(huc_sf_subset)
  }
  
 
ggplot() +
  geom_sf(data =  FuncSelHUC(huc = "HUC12", huc_sf = huc12_overlap_sf, park_code), fill = "blue", alpha = 0.3) + 
...
observeEvent(eventExpr = input$button_showStations, {
  ...
  shinyjs::hideElement("new_data_P4")
    ...
    # download.file(unitBoundaryURL, tempUnitOutput) # readOGR geoJSON driver needs dsn to be a local file, so download the file first, then read it
     imported_dat <- tryCatch(sf::st_read(unitBoundaryURL) %>% sf::st_make_valid(.), error=function(e) print("Error retrieving Park unit boundary data"))

    # imported_dat <- tryCatch(sf::st_read(dsn = tempUnitOutput, dropNULLGeometries = FALSE), error=function(e) print("Error retrieving Park unit boundary data")) # return error message if problems still arise with downloading from web services
    ...
    shiny::req("sf" %in% class(imported_dat))
    imported_dat <- sf::st_transform(imported_dat, crs = "+proj=longlat +datum=WGS84")
    ...
    rm(unitBoundaryURL, imported_dat)
...
# Limited to specified date range and minimum activities number
    ...
    sitesPoints <- sf::st_as_sf(x = sites, 
                        coords = c("LongitudeMeasure", "LatitudeMeasure"),
                        crs = "+init=epsg:4269")# NAD83 <<<<<<<<<<<<<<< NEED TO CHECK ON THIS, IT SEEMS INDIVIDUAL RECORDS MAY HAVE DIFFERENT DATUMS <<<<<<<
    
    sitesPoints <- sf::st_transform(sitesPoints, crs = st_crs(rv$unitPoly)) # convert to WGS84. Gives WARNING >>>>> +init=epsg:XXXX syntax is deprecated. It might return a CRS with a non-EPSG compliant axis order.
  ...
  buff_bbox <- sf::st_bbox(rv$sitesPointsSub) + c(-0.25, -0.25, 0.25, 0.25)
  ...
  gageURL <- paste0("https://waterservices.usgs.gov/nwis/site/?format=rdb&bBox=", paste(buff_bbox, collapse =","), "&startDT=", as.character(input$sel_ImportStartDate), "&endDT=", as.character(input$sel_ImportEndDate), "&seriesCatalogOutput=true&outputDataTypeCd=dv&parameterCd=00060&siteType=ES,ST&agencyCd=USGS&siteStatus=all")
  ...
  # tempMeteoStationsBbox<-tryCatch(ncdc_stations(extent = c(rv$sitesPointsSub@bbox[2:1,"min"]-.25, rv$sitesPointsSub@bbox[2:1,"max"]+.25), limit = 500, datacategoryid="PRCP", token = "GxPYgaqBEDAkerwNZiXWQyIpqkAQTYSv"), error=function(e) print("Error retrieving weather stations information")) # added a buffer
   ...
   tempMeteoStationsBbox<-tryCatch(ncdc_stations(extent = c(buff_bbox[2], buff_bbox[1], buff_bbox[4], buff_bbox[3]), limit = 500, datacategoryid="PRCP", token = "GxPYgaqBEDAkerwNZiXWQyIpqkAQTYSv"), error=function(e) print("Error retrieving weather stations information")) # added a buffer. X represents longitude
  cat("line2303")
...
# 
    rv$selectedStations <- 
      rbind.fill(rv$selectedOriginStation, if(!is.null(rv$selectedUpstreamStations)) rv$selectedUpstreamStations, if(!is.null(rv$selectedDownstreamStations)) rv$selectedDownstreamStations) %>% # combine into one data frame
      dplyr::rename(MonitoringLocationIdentifier = identifier) %>%
      left_join(unique(rv$sitesPointsSub[, c("MonitoringLocationIdentifier", "OrganizationFormalName")]), by = "MonitoringLocationIdentifier") %>%
    ...
    if(clicked_site %in% rv_param_temp$param_map_highlight) { # if the click is in a selected site
      cat("WAS ALREADY SELECTED\n")
        ...
        leafletProxy("param_map") %>%
          addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier == clicked_site,], group = "Available monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations")) # and change the site to unselected on the map
        cat("REMOVED CLICKED SITE FROM MAP\n")
          ...
          addCircleMarkers(data = FuncParamAvailSites("by_station"), group = "Available monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations"))
           ...
           leafletProxy("param_map") %>% # and highlight the site on the map
            addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier == clicked_site,], group = "Available monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_availableStations"))
...
conditionalPanel(
  ...
  renderUI({
    ...
    map_zoom <- min(RgoogleMaps::MaxZoom(lonrange = c(st_bbox(rv$unitPoly)$xmin, st_bbox(rv$unitPoly)$xmax), latrange = c(st_bbox(rv$unitPoly)$ymin, st_bbox(rv$unitPoly)$ymax)))
...
output$map <- renderLeaflet({
  ...
  if(any(is.null(rv$userSelect$sel_SelectMethod), is.null(rv$userSelect$sel_HUClevel), is.null(rv$userSelect$sel_Organization), is.null(rv$unitPoly), is.null(rv$HUC10), is.null(rv$sitesPointsSub), rv$parkID=="", !rv$userSelect$sel_Organization %in% rv$sitesPointsSub$OrganizationFormalName)) {
      map <- leaflet()
      } else {
        ...
        map %<>%
          ...
          setView(lng = switch((is.null(isolate(input$map_center$lng))+1), isolate(input$map_center$lng), mean(st_bbox(rv$unitPoly)$xmin, st_bbox(rv$unitPoly)$xmax)), lat = switch((is.null(isolate(input$map_center$lat))+1), isolate(input$map_center$lat), mean(st_bbox(rv$unitPoly)$ymin, st_bbox(rv$unitPoly)$ymax)), zoom = input$sel_MapZoom)
      }
})
...
if(!is.null(rv$selectedGages)) {
  ...
  if(input$sel_ParamMapShow == "show")
    ...
    # Need to do this on first go-around because don't yet have a param_map to update with proxy. 
      ...
      if(length(sel_sites) > 0) {
        param_map %<>%
      addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier %in% sel_sites,], group = "Available monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_availableStations"))
      }
  ...
  #   # if(input$sel_SummarizeBy == "by_char") { 
  #       selected_sites_data <- rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier %in% input$sel_ShowStations,]
  #     } else {
  #       
  #       if(input$sel_SummarizeBy == "by_station" & !is.null(input$sel_ShowOneStation)) {
  #     selected_sites_data <- rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier %in% input$sel_ShowOneStation,]
  #       } else {
...
cat("line2095")
  ...
  if(input$sel_SummarizeBy == "by_char" & !is.null(input$sel_SurvYr)){
    ...
    #     left_join(rv$sitesPointsSub[, c("MonitoringLocationIdentifier", "LatitudeMeasure", "LongitudeMeasure", "SITELAB")], by = "MonitoringLocationIdentifier") # >>>>>>>>>>>>>>>>> TRUNCATES LAT-LONG
    # 
    ...
    if(input$sel_ParamMapShow == "thresh") {
      ...
      thresh_mat %<>%
        ...
        left_join(subset(rv$sitesPointsSub, select = c("MonitoringLocationIdentifier", "Latitude", "Longitude", "SITELAB")), by = "MonitoringLocationIdentifier") %>%
        ungroup()
      cat("line5269")
      param_map %<>%
        addControl(tags$div(tag.map.title, HTML(input$sel_ShowChar, " in Year ", input$sel_SurvYr)), position = "topleft", className="map-title") %>%
        addMinicharts(
          lng = thresh_mat$Longitude,
          lat = thresh_mat$Latitude,
          time = thresh_mat$Yr,
          type = "pie",
          transitionTime = 0,
...
cat("line5292")
      perc_mat <- rv_param_temp$plotDat %>%
        ...
        left_join(subset(rv$sitesPointsSub, select = c("MonitoringLocationIdentifier", "Latitude", "Longitude", "SITELAB")), by = c("MonitoringLocationIdentifier")) %>%
        ungroup()
      param_map %<>% addControl(tags$div(tag.map.title, HTML(input$sel_ShowChar, " in Year ", input$sel_SurvYr)), position = "topleft", className="map-title") %>%
        addMinicharts(
          lng = perc_mat$Longitude,
          lat = perc_mat$Latitude,
          time = perc_mat$Yr,
          type = "pie",
          transitionTime = 0,
          height = input$sel_MapPointSize,
  ...
  param_map %<>%
    ...
    setView(lng = switch((is.null(input$param_map_center$lng))+1, input$param_map_center$lng, mean(st_bbox(rv$unitPoly)$xmin, st_bbox(rv$unitPoly)$xmax)), lat = switch((is.null(input$param_map_center$lat))+1, input$param_map_center$lat, mean(st_bbox(rv$unitPoly)$ymin, st_bbox(rv$unitPoly)$ymax)), zoom = input$sel_ParamMapZoom)
...
- [watershed boundaries--current version Sept 2023] https://nrcs.app.box.com/v/huc/folder/39290322977 
- [park unit boundaries] https://irma.nps.gov/DataStore/Reference/Profile/2224545?lnv=True [citation is: National Park Service (NPS), Chief Cartographer, Land Resources Division, WASO Fort Collins Land Resources Program Center. 2024. Administrative Boundaries of National Park System Units - National Geospatial Data Asset (NGDA) NPS National Parks Dataset. NPS - Land Resources Division]
...
* What happens to this dashboard when/if the public data servers change?

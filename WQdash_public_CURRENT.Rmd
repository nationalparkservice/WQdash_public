...
# > Watershed boundaries are clipping to parks, and should not
...
# Home
# -	Sample size matrix doesn’t work [B]UG
# -	One character vector error messages
# 
# Time series
# -Map zoom slider should be much wider
# -	When season-of-year, it plots as two cols and is squished
...
# -	[BUG] MANY CHAR FOR ONE SITE IS PBORKEN
# -	Map goes half lost
...
cat("779")
  ...
  char_list <- lapply(sort(unique(dat$CharacteristicName)), function(i) {
    p %+% 
      dplyr::filter(dat, ~ CharacteristicName == i) + ggtitle(paste0(i, " (# of surveys per ", input$sel_TimeSubunit, ")"))
    })
  cat("785")
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  shiny::req(length(rv$selectedStations$MonitoringLocationIdentifier) > 0)
    ...
    withProgress(message = "Processing data request...", detail = "Importing data from Water Quality Portal, 10 sites at a time", value = 0, { # >>>>>>>>>>>>>> PICK UP FROM HERE--BREAK IT UP https://community.openai.com/t/parallelise-calls-to-the-api-is-it-possible-and-how/35498/10
...
observeEvent(input$param_map_marker_click, {
  isolate({
  # local({
  shiny::req(!is.null(input$sel_SummarizeBy), !is.null(input$sel_ParamMapShow), !is.null(input$param_map_marker_click$id))# the last one keeps the map from aborting when user clicks on a marker that is not a site marker
        ...
        cat("REMOVED CLICKED SITE FROM MAP\n")
          ...
          if(input$sel_SummarizeBy == "by_station") { # if summarizing by station
            ...
            cat("UPDATED VECTOR WITH ONLY CLICKED SITE\n")
             leafletProxy("param_map") %>%
          addCircleMarkers(data = FuncParamAvailSites("by_station"), group = "Imported monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_importedStations"))
           ...
           leafletProxy("param_map") %>% # and highlight the site on the map
            addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier == clicked_site,], group = "Imported monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_importedStations"))
...
if(all(any(is.null(input$sel_ParamMapShow), input$sel_ParamMapShow == "show"), isolate(is.null(rv_param_temp$update_from_sidebar))))
    ...
    # Need to do this on first go-around because don't yet have a param_map to update with proxy.
      ...
      if(length(sel_sites) > 0) {
        ...
        param_map %<>%
      addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier %in% sel_sites,], group = "Imported monitoring sites", layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_importedStations"))
      }
...
renderUI({
      ...
      radioButtons("sel_TimeSubunit",
               label = h6("Time unit for 'Sample Size Matrix'"),

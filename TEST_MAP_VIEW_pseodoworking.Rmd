...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, y_scale, categLevelsPlot = rv_param_temp$categLevelsPlot, sel_add_gage_precip = switch(is.null(input$sel_AddGagePrecip)+1, input$sel_AddGagePrecip, "none")) {
...
# Includes censored data
    ...
    thresh_list <- switch(rv_param_temp$threshType,
      ...
      # If adding streamflow data...
        if(any(sel_add_gage_precip!="streamflow", is.null(input$sel_GageStation), is.null(rv$gagesDat), is.null(rv$selectedGages))) {
          gageDat <- NULL
          gage_YScale <- NULL
        } else {
      ...
      # If adding precipitation data...
        if(any(sel_add_gage_precip!="precipitation", is.null(input$sel_PrecipStation), is.null(rv$meteoDat), is.null(rv$selectedMeteo))) {
          precipDat <- NULL
          precip_YScale <- NULL
        } else {
...
# renderUI({
#   isolate({
#     shiny::req(!is.null(rv_param_temp$update_from_sidebar), !is.null(output$param_map), !is.null(rv_param_temp$param_map_highlight))
#     shiny::req(rv_param_temp$update_from_sidebar == TRUE)})
#   
#   if(all(input$sel_SummarizeBy == "by_char", !identical(isolate(rv_param_temp$param_map_highlight), input$sel_ShowStations))) { # if selecting multiple stations for one characteristic and if map selection does not match user input. Need to use identical() so can deal with character(0)
#         rv_param_temp$param_map_highlight <- input$sel_ShowStations}
#     
#     if(all(input$sel_SummarizeBy == "by_station", !identical(isolate(rv_param_temp$param_map_highlight), input$sel_ShowOneStation))) { # if selecting one station and if map selection does not match user input....
...
# renderUI({
#   shiny::req(!is.null(input$sel_ParamMapShow), !is.null(input$sel_SummarizeBy), isolate(!is.null(rv_param_temp$param_map_highlight)), isolate(!is.null(rv_param_temp$update_from_sidebar)))
#   shiny::req(input$sel_ParamMapShow == "show", isolate(rv_param_temp$update_from_sidebar == TRUE)) # when param_map marker set to "show"
#     cat("TRIGGERED UPDATE FROM SIDEBAR\n")
#   #
...
#         rv_param_temp$param_map_highlight <- input$sel_ShowStations
#         avail_sites <- rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier[rv_param_temp$subDat$CharacteristicName %in% input$sel_ShowChar]),]
#         cat("CALCULATED AVAIL SITES BY CHAR\n")
...
# 
#         avail_sites <- rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier),]
#         cat("CALCULATED AVAIL SITES BY STATION\n")
#         update_selected_sites <- TRUE # need to update map
#       }
#     }
# 
...
#         cat("HIGHLIGHTED SELECTED SITES\n")
#     }
# 
# })
```
...
if(!is.null(rv$selectedGages)) {
  ...
  if(any(is.null(input$sel_ParamMapShow), input$sel_ParamMapShow == "show") & isolate(rv_param_temp$update_from_sidebar != FALSE)) # may be TRUE or NULL
    {
    # isolate({
...
tags$style("#out_ThreshPieparam_map{height:100vh;overflow-x:scroll;overflow-y:scroll}")

...
#   if(is.null(input$sel_ShowThresholds)) {
#     updateSelectInput(session,
#                     "sel_ShowThresholds",
#                     selected = "showthresh_none")
#     }
#   
#   if(is.null(input$sel_AddGagePrecip)) {
#     updateSelectInput(session,
#                     "sel_AddGagePrecip",
#                     selected = "none")
#   }
#   
#   if(is.null(input$sel_YScale)) {
#     updateSelectInput(session,
#                     "sel_YScale",
#                     selected = "identity")
#   }
...
#   }
#   
#   if(is.null(input$sel_FreeY)) {
#     updateCheckboxInput(session,
#                         "sel_FreeY",
#                       value = FALSE)
#   }
#   
#   if(is.null(input$sel_AddQuantile)) {
#     updateCheckboxInput(session,
#                       "sel_AddQuantile",
#                       value = TRUE)
#   }
#   
#   if(is.null(input$sel_PlotByStation)) {
#     updateCheckboxInput(session,
#                       "sel__PlotByStation",
#                       value = TRUE)
#   }
...
FuncQuantLines <- function(quant_df, combine_stations, dat_df, p) {
  if(!is.null(quant_df)) {
    if(nrow(quant_df)>0) {
      ...
      if(rv_param_temp$sel_summarize_by == "by_char" & combine_stations==TRUE & length(unique(dat_df$MonitoringLocationIdentifier)) > 1) {
...
FuncPlotScatter <- function(plot_dat, quant_df = NULL, categ_df = NULL, x_nam = "ActivityStartDate", combine_stations, fixed_free_y = "fixed", trans_y = "identity", trans_gage_y = "identity", trans_precip_y = "identity", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, gage_dat = NULL, precip_dat = NULL, month_summary = FALSE, sel_add_gage_precip = switch(is.null(rv_param_temp$sel_add_gage_precip)+1, rv_param_temp$sel_add_gage_precip, "none"), sel_categ) {
...
if(x_nam == "ActivityStartDate") {
  ...
  if(rv_param_temp$sel_summarize_by == "by_char" & combine_stations==TRUE) {
 ...
 cat("line1488")
    p_points <- p_points +
      ...
      if(rv_param_temp$sel_summarize_by == "by_char" & combine_stations==TRUE) { # if coloring by monitoring site, use viridis color scale
        p_points <- p_points +
...
FuncPlotBox <- function(plot_dat, quant_df = NULL, x_nam = "Yr", combine_stations = FALSE, fixed_free_y = "fixed", trans_y = "identity", trans_gage_y = "identity", trans_precip_y = "identity", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, gage_dat = NULL, precip_dat = NULL, sel_add_gage_precip = switch(is.null(rv_param_temp$sel_add_gage_precip)+1, rv_param_temp$sel_add_gage_precip, "none")) {
...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, y_scale, categLevelsPlot = rv_param_temp$categLevelsPlot, sel_add_gage_precip = switch(is.null(rv_param_temp$sel_add_gage_precip)+1, rv_param_temp$sel_add_gage_precip, "none"), sel_plot_by_station) {
...
# Includes censored data
  ...
  cat("line1921")
    if(plot_type %in% c("points", "points_smooth")) {
      ...
      ts_plot <- FuncPlotScatter(plot_dat = plot_dat, quant_df = quant_dat, categ_df = categLevelsPlot, x_nam = time_unit, combine_stations = ifelse(length(unique(plot_dat$CharacteristicName)) == 1, ifelse(sel_plot_by_station, FALSE, TRUE), FALSE), fixed_free_y = ifelse(free_y, "free_y", "fixed"), trans_y = y_scale, trans_gage_y = gage_YScale, trans_precip_y = precip_YScale, add_loess = ifelse(plot_type == "points_smooth", TRUE, FALSE), incl_cens = incl_cens, thresh_list = thresh_list, rotate_x = TRUE, gage_dat = gageDat, precip_dat = precipDat, month_summary = month_summary, sel_categ = sel_categ)}

    if(plot_type == "box") {
      ts_plot <- FuncPlotBox(plot_dat = plot_dat, quant_df = quant_dat, x_nam = time_unit, combine_stations = ifelse(length(unique(plot_dat$CharacteristicName)) == 1, ifelse(sel_plot_by_station, FALSE, TRUE), FALSE), fixed_free_y = ifelse(free_y, "free_y", "fixed"), trans_y = y_scale, trans_gage_y = gage_YScale, trans_precip_y = precip_YScale, incl_cens = incl_cens, thresh_list = thresh_list, rotate_x = TRUE, gage_dat = gageDat, precip_dat = precipDat)}
...
cat("line1930")
    ...
    param_plotly <- FuncPlotlyFormat(
      ...
      is_grouped_boxplots = (plot_type == "box" & length(unique(plot_dat$CharacteristicName)) == 1 & length(unique(plot_dat$MonitoringLocationIdentifier)) > 1 & sel_plot_by_station == FALSE) # boxplots for multiple stations, all combined in a single plot
    ) 
...
tags$style("#param_map {height: calc(100vh - 350px) !important;overflow-y:scroll}") # expand map to fill screen
```
...
cat("line5039")
  ...
  shiny::validate(
    ...
    if(all(!is.null(input$sel_PlotByStation), !is.null(rv_param_temp$showStations))) {
...
cat("line2220")
  ...
  if(all(!is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$quantDat))) {
    rv_param_temp$finalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = "ActivityStartDate", free_y = switch(is.null(input$sel_FreeY)+1, input$sel_FreeY, FALSE), y_scale = switch(is.null(input$sel_YScale)+1, input$sel_YScale, "identity"), sel_plot_by_station = switch(is.null(input$sel_PlotByStation)+1, input$sel_PlotByStation, TRUE))
    cat("line2224")
  }
...
output$out_season_plots <- renderPlotly({
  ...
  shiny::req(!is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$quantDat))
    ...
    rv_param_temp$seasonalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = switch(is.null(input$sel_SeasonalTimeUnit)+1, input$sel_SeasonalTimeUnit, "Wk"), free_y = switch(is.null(input$sel_FreeY)+1, input$sel_FreeY, FALSE), y_scale = switch(is.null(input$sel_YScale)+1, input$sel_YScale, "identity"), sel_plot_by_station = switch(is.null(input$sel_PlotByStation)+1, input$sel_PlotByStation, TRUE))
...
# # >>>>>>>>>>> PICK UP FROM HERE. NEED TO DISABLE THIS WHEN NOT RELEVANT FOR USER INPUT.
...
#     status = "button_menu",
#     icon = icon("sliders"),
#     tooltip = "Options for user to modify map",
#            renderUI({
#   shiny::req(!is.null(rv$unitPoly), !is.null(rv_param_temp$sel_summarize_by), !is.null(rv_param_temp$plotDat), !is.null(input$sel_ParamMapShow))
#   sliderTextInput("sel_SurvYr", force_edges = TRUE, label = h6("Choose a survey year"), choices = sort(unique(rv_param_temp$plotDat$Yr)), selected = max(rv_param_temp$plotDat$Yr, na.rm = TRUE), dragRange = FALSE, animate = TRUE, width = "85%")
#   }),
...
-------------------------------------
```{r TS_plots_message}
tags$div(renderText({FuncPlotsMessage("start")}), style = "font-size:14px; color:red;")
```
```{r plot_input}
div(style="display:flex; flex-direction:row; max-width:100%; justify-content:space-evenly; flex-wrap: wrap; align-items: center;",
    div(style ="flex: 0 0 55; padding:2px;",
...
div(style ="flex: 0 0 35; padding:2px;",
  ...
  checkboxInput("sel_PlotByStation",
    ...
    renderUI({
      shiny::req(all(!is.null(input$sel_SummarizeBy), input$sel_SummarizeBy == "by_char"), !is.null(input$sel_PlotByStation), input$sel_PlotByStation == TRUE)
  ...
  renderUI({
    shiny::req(any(is.null(input$sel_PlotByStation), input$sel_PlotByStation == TRUE))

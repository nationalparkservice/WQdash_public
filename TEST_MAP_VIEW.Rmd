...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, y_scale, categLevelsPlot = rv_param_temp$categLevelsPlot) {
  ...
  # > PICK UP FROM HERE. For One characteristic, cannot select from map unless at least one station already selected and also the available stations should be the only ones showing. Cannot update map to have NO sites selected, plots don't show. When switch btwn station and char, the residual selected station does not change. SEasonly plots time unit doesn't real-time update. Show one monitoring site per plot doesn't work. Lower threshold and lower % should be moved out of real-time options?? Also allow to select order.
...
shiny::req(input$sel_ParamMapShow == "show")  #<<<<<<<<<< NEED TO CHANGE SO MAP SHOWS ALL BY DEFAULT
    ...
    if(clicked_site %in% prior_sites) { # check if the click is in a selected site, then remove the site from selected sites
        current_sites <- prior_sites[prior_sites != clicked_site] # then remove it from the list of selected sites
      } else {
        current_sites <- c(prior_sites, clicked_site) # append the ID of clicked site
      }

    # Now update the map to show the current selected sites
    # Things to do if there is no selected site
    if(length(current_sites) < 1) {

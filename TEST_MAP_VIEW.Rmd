...
load_rv_param_temp <- readRDS("C:/Users/echeng/Desktop/WQdash_public/MAP_rv_param_temp.RDS")
load_rv <- readRDS("C:/Users/echeng/Desktop/WQdash_public/MAP_rv.RDS")
...
rv_param_temp$update_from_sidebar <- FALSE # during this process, put a halt to this
 clicked_site <- input$param_map_marker_click$id
  ...
  # > PICK UP FROM HERE. MAP works great, and it correctly updates user sidebar, but when I refresh plot data then all the selected sites on map disappear. For ONE STATION, all works unless I remove all sites from map. Then it stops working. Sometimes the plots include sites that are not selected?
    # Need to clear out plots when first load. SEasonly plots time unit doesn't real-time update. Show one monitoring site per plot doesn't work. Lower threshold and lower % should be moved out of real-time options?? Also allow to select order. Progress bar terminates early.
    
    if(clicked_site %in% isolate(rv_param_temp$param_map_highlight)) { # if the click is in a selected site
      cat("line1544")
        rv_param_temp$param_map_highlight <- isolate(rv_param_temp$param_map_highlight)[isolate(rv_param_temp$param_map_highlight) != clicked_site] # then remove it from the list of selected sites
        ...
        cat("REMOVED-->", clicked_site)
        } else { # if the click is in an unselected site
          if(input$sel_SummarizeBy == "by_station") { # if summarizing by station
            rv_param_temp$param_map_highlight <- clicked_site # only the clicked site is saved
          #   leafletProxy("param_map") %>% # unselect all available sites
          # addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier)[unique(rv_param_temp$subDat$MonitoringLocationIdentifier) != clicked_site],], group = "Available monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations"))
            
          } else { # if summarizing by characteristic
            rv_param_temp$param_map_highlight <- c(isolate(rv_param_temp$param_map_highlight), clicked_site) # append the ID of clicked site
          }
          
          # Unselect all available sites
          leafletProxy("param_map") %>%
          addCircleMarkers(data = FuncParamAvailSites(input$sel_SummarizeBy), group = "Available monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations"))
          
          # leafletProxy("param_map") %>% # highlight the clicked site on the map
    ...
    if(input$sel_SummarizeBy == "by_char") {
      updateCheckboxGroupInput(session,
                               "sel_ShowStations",
                               selected = isolate(rv_param_temp$param_map_highlight))
    }
...
```{r update_map_userinput}
# Update map selection and selection vector when user input changes from side bar
renderUI({
  shiny::req(!is.null(input$sel_ParamMapShow), !is.null(input$sel_SummarizeBy), !is.null(isolate(rv_param_temp$update_from_sidebar)))
  shiny::req(input$sel_ParamMapShow == "show", isolate(rv_param_temp$update_from_sidebar == TRUE)) # when param_map marker set to "show"
    cat("UPDATE RV PARAM MAP")  
  #   
  ...
  # saveRDS(isolate(reactiveValuesToList(input)), here::here("TEMP_input.RDS"))
    ...
    update_selected_sites <- FALSE
    if(all(input$sel_SummarizeBy == "by_char", !is.null(input$sel_ShowChar))) { # if selecting multiple stations for one characteristic
cat("line1611")
      if(!identical(isolate(rv_param_temp$param_map_highlight), input$sel_ShowStations)) { # if map selection does not match user input. Need to use identical() so can deal with character(0)
        rv_param_temp$param_map_highlight <- input$sel_ShowStations
        # avail_sites <- rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier[rv_param_temp$subDat$CharacteristicName %in% input$sel_ShowChar]),]
        update_selected_sites <- TRUE # need to update map
      }
    }
cat("line1618")
    if(all(input$sel_SummarizeBy == "by_station", !is.null(input$sel_ShowOneStation))) { # if selecting one station for many characteristics
      if(!identical(isolate(rv_param_temp$param_map_highlight), input$sel_ShowOneStation)) { # if map selection does not match user input
        rv_param_temp$param_map_highlight <- input$sel_ShowOneStation
        cat("line1622")
        # avail_sites <- rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier),]
        update_selected_sites <- TRUE # need to update map
      }
    ...
    if (update_selected_sites == TRUE) {
        cat("line1622")
        leafletProxy("param_map") %>% # unselect all available sites
          addCircleMarkers(data = FuncParamAvailSites(input$sel_SummarizeBy), group = "Available monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations"))

      leafletProxy("param_map") %>% # highlight only the user-selected input sites
    addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% isolate(rv_param_temp$param_map_highlight),], group = "Available monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_availableStations"))
    }
  cat("line1631")
})
...
if(!is.null(rv$selectedGages)) {
  ...
  #   rv_param_temp$update_from_sidebar = TRUE
  # } else {
  #     rv_param_temp$update_from_sidebar = FALSE
  #   }
  # 

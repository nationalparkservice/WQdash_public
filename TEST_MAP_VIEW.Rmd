...
# Leaflet should also update whenever user changes input selection

# renderUI({
#   shiny::req(!is.null(rv$sitesPointsSub), !is.null(input$sel_SummarizeBy))
#   
#   if(input$sel_SummarizeBy == "by_char") { # map selection updates to whatever user inputs. So when user switches between "by_char" and "by_station" the selected sites will update accordingly
...
shiny::req(input$sel_ParamMapShow == "show")  
  ...
  # > PICK UP FROM HERE. For ONE CHARACTERISTIC, if I click to disappear all stations it is correct on input but it will show the site selected from ONE STATION. For One characteristic, cannot select from map unless at least one station already selected and also the available stations should be the only ones showing. For ONE SITE, when I remove all sites from map it correctly updates to blank in sidebar but not in map. Cannot update map to have NO sites selected, plots don't show. Need to clear out plots when first load. Wigs out on one site, multiple char. When switch btwn station and char, the residual selected station does not change. SEasonly plots time unit doesn't real-time update. Show one monitoring site per plot doesn't work. Lower threshold and lower % should be moved out of real-time options?? Also allow to select order. Progress bar terminates early.

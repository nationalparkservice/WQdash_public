...
observeEvent(eventExpr = input$button_refreshPlotData, { 
  ...
  # rv_param_temp$showStations <- rv_param_temp$selectedMapSites # <<<<<<<<<<<<<<<
...
```{r click_map_plots}          
# Had to make sure layerId only on the site shapefiles, not also the HUC shapefiles

# User selects stations by clicking (to show data plots)
observeEvent(input$param_map_marker_click, {
  shiny::req(!is.null(input$sel_ParamMapShow))
shiny::req(input$sel_ParamMapShow == "show")  #<<<<<<<<<< NEED TO CHANGE SO MAP SHOWS ALL BY DEFAULT
  ...
  # <<<<<<<<<<<< PICK UP FROM HERE. When I click a second site, it goes bonkers. I also can't unclick for one site, many stations

    clicked_site <- input$param_map_marker_click$id
    
    if(is.null(rv_param_temp$selectedMapSites)) {
      rv_param_temp$selectedMapSites <- c(clicked_site)
    } else {
...
# renderUI({
#   shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(input$sel_SummarizeBy), !is.null(rv_param_temp$seasonalTimeUnit), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$quantDat), !is.null(rv_param_temp$threshType))
# 
...
# 
# 
#   rv_param_temp$map_plot_dat <- rv_param_temp$plotDat %>% dplyr::filter(MonitoringLocationIdentifier == rv_param_temp$selectedMapSites)
#   rv_param_temp$map_quant_dat <- rv_param_temp$quantDat %>% dplyr::filter(MonitoringLocationIdentifier == rv_param_temp$selectedMapSites)
#   
...
renderUI({
  rv_param_temp$selectedMapSites <- input$sel_ShowOneStation
}), # update this for map click # <<<<<<<<<<<<<<<<<<<<<<<<
...
cat("line1985")
  ...
  param_map %<>%
    addCircleMarkers(data = avail_sites_data, group = "Available monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, popup = ~SITEPOP, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = FALSE, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations")) # WQ stations for which data (for selected parameter) are available  
cat("line1988")
...
if(!is.null(rv$selectedGages)) {
  ...
  if(input$sel_ParamMapShow == "show") { # White outline on sites selected from left panel
    ...
    if(!is.null(input$sel_ShowOneStation)) {

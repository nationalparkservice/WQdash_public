...
```{r click_map_plots}          
# Update site-specific plots based on map click 
...
renderUI({
  ...
  shiny::req(!is.null(input$param_map_marker_click$id)) # <<<<<<<<<<<<<<< MAP CLICK
  
  ### THINGS TO DO IF MAPVIEW
  
  rv_param_temp$map_plot_dat <- rv_param_temp$plotDat %>% dplyr::filter(MonitoringLocationIdentifier == input$param_map_marker_click$id) # <<<<< CAN CHANGE THIS SO THIS IS ONLY IF MAPVIEW, OTHERWISE IT SHOULD BE THE FULL DATA
...
renderUI({ # <<<<<<<<<<<<< CHANGE ALL MAP INPUTS' SHINY::REQ
  ...
  selectInput("sel_ParamMapShow",
              ...
              choices = switch((input$sel_SummarizeBy == "by_char")+1, c("Selected monitoring site" = "show", "Overall median" = "overall_median", "Overall threshold levels" = "overall_thresh"), c("Selected monitoring sites" = "show", "Overall median value" = "overall_median", "Overall threshold levels" = "overall_thresh", "Annual median value" = "median", "Annual threshold levels" = "thresh", "Annual percentile levels" = "perc")),
...
cat("line3026")
  ...
  show_layers <- data.frame(layer = c(paste0(rv$userSelect$sel_HUClevel, " boundaries"), "Selected monitoring sites", "Imported stream lines", "Discharge facilities", "Available gage stations", "Available weather stations"), show = c(TRUE, TRUE, FALSE, FALSE, FALSE, FALSE)) # dataframe to track what layers to show
  
  show_char <- switch((rv_param_temp$plotSummarizeBy == "by_char")+1, input$sel_ShowStationChars, input$sel_ShowChar) # if summarize by station, this can be multiple characteristics; otherwise, it's just one characteristic <<<<<<<<<<<<<<<<<<<<<<<<<<<< CHECK IF THIS IS RIGHT
...
if(!is.null(rv$selectedGages)) { 
  ...
  if(input$sel_ParamMapShow == "show") {
    ...
    param_map %<>%
      addCircleMarkers(data = switch((rv_param_temp$plotSummarizeBy == "by_station") + 1, rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% input$sel_ShowStations,], rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% input$sel_ShowOneStation,]), group = "Selected monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_selectedStations")) # selected WQ stations are shown as larger, white-outlined points
  }
  cat("line3098")
  ...
  if(rv_param_temp$plotSummarizeBy == "by_char") {
    ...
    if(input$sel_ParamMapShow == "median") {
      ...
      param_map %<>%
        ...
        addCircleMarkers(data = temp_median_plot, lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~palNumeric(median_val), fillOpacity = 1, color = ~palNumeric(median_val), stroke = FALSE, opacity = 1, options = pathOptions(pane = "pane_param_selectedStations")) %>%
    ...
    if(input$sel_ParamMapShow == "thresh") {
      ...
      param_map %<>%
        ...
        addMinicharts(
          ...
          layerId = thresh_mat$MonitoringLocationIdentifier,
    ...
    if(input$sel_ParamMapShow == "perc") {
      ...
      param_map %<>% addControl(tags$div(tag.map.title, HTML(input$sel_ShowChar, " in Year ", input$sel_SurvYr)), position = "topleft", className="map-title") %>%
        addMinicharts(
          ...
          layerId = perc_mat$MonitoringLocationIdentifier,

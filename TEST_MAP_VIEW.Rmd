...
load_rv_param_temp <- readRDS("~/NPS PROJECTS_BIGPIC/WQdash_public/MAP_rv_param_temp.RDS")
load_rv <- readRDS("~/NPS PROJECTS_BIGPIC/WQdash_public/MAP_rv.RDS")
...
# <<<<<<<<<<<<<<<<<< SPLOT OUT REFRESH DATA FROM THE PLOT GENERATION
...
```{r click_map_plots}          
# Update site site selection based on map clicks. DO NOT update plots yet, that occurs only after user presses button to refresh plots.
# These were originally in observeEvent refreshPlots

# User selects stations by clicking (to show data plots)
# observeEvent(input$param_map_marker_click, {
#   shiny::req(!is.null(rv), input$sel_ParamMapShow == "show")  #<<<<<<<<<< NEED TO CHANGE SO MAP SHOWS ALL BY DEFAULT
# 
#     clicked_site <- input$param_map_marker_click$id
# 
#       if(clicked_site %in% rv_param_temp$selectedMapSites) { # check if the click is in a selected site, then remove the site from selected sites
#         rv_param_temp$selectedMapSites <- rv_param_temp$selectedMapSites[rv_param_temp$selectedMapSites != clicked_site] # then remove it from the list of selected sites
#       } else {
#         rv_param_temp$selectedMapSites <- c(rv_param_temp$selectedMapSites, clicked_site) # append the ID of clicked site
#         }
# 
#     # Now update the map to show the current selected sites
# 
#     leafletProxy("param_map") %>%
#       addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% rv_param_temp$selectedMapSites,], group = "Selected monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_selectedStations"))
# 
...
#     #   updateCheckboxGroupInput(session,
#     #                            "sel_ShowStations",
#     #                            selected = rv_param_temp$selectedMapSites)
#     # }
#     # 
...
# <<<<<< THESE SHOULD ONLY BE RUN AFTER REFRESHPLOT FIRST PRESSED, OTHERWISE WILL HAVE SAME PROBLEM WITH CATEG COLORS. NEED TO CODE THAT IN.

renderUI({
  shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$seasonalTimeUnit), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$quantDat), !is.null(rv_param_temp$threshType))

  shiny::req(input$sel_ParamMapShow == "show", !is.null(input$param_map_marker_click$id)) # Map click highlights sites only if marker type is on "show"


  rv_param_temp$map_plot_dat <- rv_param_temp$plotDat %>% dplyr::filter(MonitoringLocationIdentifier == input$param_map_marker_click$id)
  rv_param_temp$map_quant_dat <- rv_param_temp$quantDat %>% dplyr::filter(MonitoringLocationIdentifier == input$param_map_marker_click$id)
})
...
renderUI({
  ...
  rv_param_temp$selectedMapSites <- input$sel_ShowStations # update this for map
  }),
...
renderUI({
  ...
  cat("line1667")
    ...
    cat("line1672")
    rv_param_temp$selectedMapSites <- input$sel_ShowOneStation # update this for map click
    cat("line1674")
}),
...
renderUI({ # <<<<<<<<<<<<< CHANGE ALL MAP INPUTS' SHINY::REQ
  shiny::req(!is.null(input$sel_SummarizeBy))
  selectInput("sel_ParamMapShow",
              label = h6("Show on map: "),
              choices = switch((input$sel_SummarizeBy == "by_char")+1, c("SELECTABLE monitoring sites" = "show", "Overall median" = "overall_median", "Overall threshold levels" = "overall_thresh"), c("SELECTABLE monitoring sites" = "show", "Overall median value" = "overall_median", "Overall threshold levels" = "overall_thresh", "Annual median value" = "median", "Annual threshold levels" = "thresh", "Annual percentile levels" = "perc")),
...
output$param_map <- renderLeaflet({
  ...
  shiny::req(!is.null(rv_param_temp$subDat), !is.null(rv$sitesPointsSub), !is.null(rv$unitPoly), !is.null(rv$userSelect), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$palOrg), !is.null(input$sel_ParamMapShow))
...
iconMeteo = makeAwesomeIcon(icon= "fa-cloud", library = "fa", squareMarker = TRUE, markerColor = "white")
  ...
  param_map <- leaflet() %>%
    addMapPane("pane_param_selectedStations", zIndex = 260) %>% # always keep selected monitoring station points on "top" of map layers (highest zIndex)
    addMapPane("pane_param_filteredStations", zIndex = 250) %>%
...
cat("line3026")
  ...
  show_layers <- data.frame(layer = c(paste0(rv$userSelect$sel_HUClevel, " boundaries"), "Filtered monitoring sites", "Selected monitoring sites", "Imported stream lines", "Discharge facilities", "Available gage stations", "Available weather stations"), show = c(TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE)) # dataframe to track what layers to show

  show_char <- switch((rv_param_temp$plotSummarizeBy == "by_char")+1, input$sel_ShowStationChars, input$sel_ShowChar) # if summarize by station, this can be multiple characteristics; otherwise, it's just one characteristic <<<<<<<<<<<<<<<<<<<<<<<<<<<< I NED TO PUT THIS SOMEWHERE
...
if(!is.null(rv$selectedGages)) {
  ...
  if(input$sel_ParamMapShow == "show") {
    ...
    param_map %<>%
      addCircleMarkers(rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier),], group = "Filtered monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, layerId = ~MonitoringLocationIdentifier, label = ~SITELAB, popup = ~SITEPOP, radius = 12, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = FALSE, opacity = 1, options = pathOptions(pane = "pane_param_filteredStations"))
  }
  cat("line3098")
  ...
  if(rv_param_temp$plotSummarizeBy == "by_char") {
    ...
    if(input$sel_ParamMapShow == "median") {
      ...
      param_map %<>%
        ...
        addCircleMarkers(data = temp_median_plot, lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, label = ~SITELAB, popup = ~SITEPOP, radius = 12, fillColor = ~palNumeric(median_val), fillOpacity = 1, color = ~palNumeric(median_val), stroke = FALSE, opacity = 1, options = pathOptions(pane = "pane_param_filteredStations")) %>%
...
cat("line5039")
  ...
  shiny::req(!is.null(rv_param_temp$map_plot_dat), !is.null(rv_param_temp$map_quant_dat))
  
    rv_param_temp$finalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$map_plot_dat, quant_dat = rv_param_temp$map_quant_dat, time_unit = "ActivityStartDate", free_y = input$sel_FreeY, y_scale = input$sel_YScale)
...
renderPlotly({
  ...
  shiny::req(!is.null(rv_param_temp$map_plot_dat), !is.null(rv_param_temp$map_quant_dat))
    ...
    rv_param_temp$seasonalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$map_plot_dat, quant_dat = rv_param_temp$map_quant_dat, time_unit = rv_param_temp$seasonalTimeUnit, free_y = input$sel_FreeY, y_scale = input$sel_YScale)

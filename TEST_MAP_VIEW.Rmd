...
if(!is.null(rv$selectedGages)) { 
  ...
  shiny::req(!is.null(input$sel_ShowStations), !is.null(input$sel_ShowOneStation))
  ...
  if(rv_param_temp$plotSummarizeBy == "by_char") {
    ...
    if(input$sel_ParamMapShow == "median") {
      ...
      param_map %<>%
        ...
        addCircleMarkers(data = temp_median_plot, lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~palNumeric(median_val), fillOpacity = 1, color = ~palNumeric(median_val), stroke = FALSE, opacity = 1, options = pathOptions(pane = "pane_param_selectedStations")) %>%

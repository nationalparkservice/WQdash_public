...
### HIGH PRIORITY
# > Filter and Explore should auto-show default
# > Pop-up table should work without threhodsl list
# > Offer an EPA threshold list??
# > Rename data summary tabs on import page
...
FuncParamTable <- function(df, threshold_df, MonitoringLocationIdentifier) {
  ...
  rows <- lapply(1:nrow(df), function(i) {
    ...
    # Threshold lookup
    thresh_sub <- if (is.null(threshold_df)||nrow(threshold_df)==0) {
      data.frame()
    } else {
      threshold_df[tolower(threshold_df$Characteristic) == tolower(param), ]
    }
...
showModal(modalDialog(
 title = "Done",
 HTML("In the <strong>'2. Filter sites to download'</strong> panel, select your filtering method and options, then click <strong>'Summarize Selected Sites'</strong> to preview the data before downloading.")
))
...
showModal(modalDialog(
  title = "Done",
  HTML(paste0("<strong>You have selected ", sum(rv$importDataSummary$ResultCount), " data records from ", length(unique(rv$importDataSummary$MonitoringLocationIdentifier)), " sites.</strong> Review your selection in the 'Selected Sites' and 'Selected Characteristic Groups' tabs. <br><br>Selected monitoring sites are shown as larger, white-outlined circles on the map.", 
              ifelse(input$sel_SelectMethod == "methodHuc", " The ORIGIN SITE is outlined in RED. The selected streamlines upstream of the origin are shown in white; downstream, in brown.", ""),
...
#          HTML(paste0("You will be downloading ", sum(rv$importDataSummary$ResultCount), "data records from ", length(unique(rv$importDataSummary$MonitoringLocationIdentifier)), " sites for download. See the 'Selected Sites' and 'Selected Characteristic Groups' tabs for summaries of the data. <br><br>Selected monitoring sites are shown as larger, white-outlined circles on the map. The ORIGIN SITE is outlined in RED. The selected streamlines upstream of the origin are shown in white; downstream, in brown. Select gage and weather stations to download (if available), then press the green <span style='color: green;'>'DOWNLOAD SELECTED DATA'</span> button to import the water quality data")),
#          HTML(paste0("You will be downloading ", sum(rv$importDataSummary$ResultCount), "data records from ", length(unique(rv$importDataSummary$MonitoringLocationIdentifier)), " sites for download. See the 'Selected Sites' and 'Selected Characteristic Groups' tabs for summaries of the data. <br><br>Selected monitoring sites are shown as larger, white-outlined circles on the map. Select gage and weather stations to download (if available), then press the green <span style='color: green;'>'DOWNLOAD SELECTED DATA'</span> button to import the water quality data"
#   ))
#   )
# ))
...
observeEvent(input$param_map_right_click, {
  ...
  site_threshold_df <- if(is.null(isolate(rv_param_temp$threshDF))) {
    NULL
  } else {
    isolate(rv_param_temp$threshDF) %>% dplyr::filter(Site == site)
  }
...
output$table_import_summary <- DT::renderDT({
  ...
  DT::formatStyle(columns = c(1:9), width = "auto")
  })
...
options = list(
  ...
  DT::formatStyle(columns = c(1:4), width = "auto")
  })

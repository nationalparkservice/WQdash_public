...
FuncClearAll <- function(rv, rv_param_temp) {
  ...
  lapply(c("new_data_P1", "new_data_P2", "new_data_P3", "create_categfile_P1",
...
observeEvent(eventExpr = input$button_showStations, {
  ...
  shiny::req(input$sel_UnitCode!="", !is.null(input$sel_MinActivities)) # make sure user has entered Park unit code

  FuncClearAll(rv, rv_param_temp)
  
  rv$parkName <- input$sel_UnitCode
  rv$parkID <- unique_parks_df$unit_code[unique_parks_df$unit_name_combined == rv$parkName]
  rv_param_temp$sel_min_activities <- input$sel_MinActivities
...
showModal(modalDialog(
  title = "Done",
  HTML("In the <strong>'3 (of 4). Filter sites to download'</strong> panel, choose the monitoring sites you want to download data from. You can:<ul>
...
unlink(tmp_summary)
  ...
  # Enforce data recency filter
  cutoff_year <- lubridate::year(Sys.Date()) - as.integer(input$sel_ActiveWithinYrs)

recent_sites <- rv$selectedDataSummary %>%
  dplyr::group_by(MonitoringLocationIdentifier) %>%
  dplyr::summarise(max_yr = max(YearSummarized, na.rm = TRUE)) %>%
  dplyr::filter(max_yr >= cutoff_year) %>%
  dplyr::pull(MonitoringLocationIdentifier)

rv$selectedDataSummary <- dplyr::filter(rv$selectedDataSummary, MonitoringLocationIdentifier %in% recent_sites)
rv$selectedStations <- dplyr::filter(rv$selectedStations, MonitoringLocationIdentifier %in% recent_sites)
rv$wqp_url_list <- rv$wqp_url_list[sapply(rv$wqp_url_list, function(url) any(recent_sites %in% unlist(strsplit(url, "&siteid="))))]
...
showModal(modalDialog(
  title = "Done",
  HTML(paste0("<strong>You have selected ", sum(rv$selectedDataSummary$ResultCount), " data records from ", length(unique(rv$selectedDataSummary$MonitoringLocationIdentifier)), " sites.</strong> Selected monitoring sites are shown as larger, white-outlined circles on the map.", 
              ...
              "<br><br>Review your selection in the 'Summary of Selected Characteristics' and 'Summary of Selected Site-Characteristics' tabs. <br><br>Select gage stations (if available), then press the <strong>DOWNLOAD SELECTED DATA</strong> button to import the water quality data."))
...
all_data[[i]] <- read.csv(tmp_wqp) %>%
  dplyr::filter(CharacteristicName %in% unique(rv$selectedDataSummary$CharacteristicName))
unlink(tmp_wqp)
Sys.sleep(0.5) # pause between data pulls
}
...
wellPanel(
  id = "new_data_P0",
  p(strong("1. Choose data source, then click 'BEGIN DATA SEARCH' button")),
...
shinyjs::hidden( # initially hide this well panel, show only if user wants to import data from WQP
  wellPanel(
    id = "new_data_P1",
    p(strong("2 (of 4). Choose a park, then click 'SHOW MONITORING SITES' button")),
    ...
    selectInput("sel_UnitCode", label = h6("Enter a park name or 4-digit park code (e.g., BITH): "), choices = c("Choose a park" = "", unique_parks_df$unit_name_combined), selectize = TRUE),
      ...
      numericInput("sel_MinActivities",
                 label = h6("Limit to sites with at least [ENTER AN INTEGER] sample records in WQP: "),
                 value = 10,
                 min = 1),
    
    actionButton("button_showStations", "SHOW MONITORING SITES", width = "100%", style="color:black; background-color: lightgreen; border: 1px solid black; display:inline-block; margin:5px")
...
shinyjs::hidden( # initially hide this well panel
  wellPanel(
    id = "new_data_P2",
    p(strong("3 (of 4). Filter sites to download, then click 'SUMMARIZE SELECTED SITES' button")),
...
conditionalPanel(
  ...
  div(style = "margin-top: -8px; margin-bottom: 5px;",
    ...
    numericInput("sel_ActiveWithinYrs",
                 label = h6("When summarizing selected sites, exclude sites that have NOT sampled within the past [ENTER AN INTEGER] years: "),
                 value = 4,
                 min = 1),
...
# UI panel to select other stations ----
shinyjs::hidden( # initially hide this well panel
  wellPanel(
    id = "new_data_P3",
    p(strong("4 (of 4). Select stream gage data to import, then click 'DOWNLOAD SELECTED DATA' button")),
    ...
    actionButton("button_downloadData", "DOWNLOAD SELECTED DATA", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border: 1px solid black; margin:5px"),
...
output$table_char_summary <- DT::renderDT({
  shiny::validate(
    need(!is.null(rv$selectedDataSummary), message = "No data selected. Follow prompts on the left to select data for import, then click <strong>'SUMMARIZE SELECTED SITES'</strong> button."))

  shiny::req(!is.null(rv$selectedDataSummary))

  DT::datatable(
    rv$selectedDataSummary %>% dplyr::group_by(CharacteristicType, CharacteristicName) %>% dplyr::summarize(`# of Data Records` = sum(ResultCount, na.rm = TRUE), .groups = "drop") %>% dplyr::rename(`Characteristic Type` = CharacteristicType, Characteristic = CharacteristicName),
    rownames = FALSE,
    filter = "top",
    selection = "none",
    class="compact stripe",

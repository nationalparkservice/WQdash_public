...
grabit <- function(url) {
  ...
  download.file(url, tmpfile, mode = "w")
...
observeEvent(eventExpr = input$button_summarizeStations, {
 ...
 # Select stations by distance from origin--tried dataRetrieval findNLDI, but seems to only pull USGS sites
  if(input$sel_SelectMethod == "methodDist") {
    ...
    for(n in names(nldiURLS)) {
      ...
      if(!is.na(nldiURLS[[n]])) { # if URL is specified
        ...
        if(httr::http_status(httr::GET(tempURL))$category != "Success") { # make sure the service call is valid
          showModal(
            urlModal(tempURL, title = "Data Retrieval Error", subtitle = htmltools::HTML(paste0("Error retrieving data from the Hydro Network-Linked Data Index for the `", n, "` query. The message from the web service is: `", httr::http_status(httr::GET(tempURL))$message, "`.<br><br><strong>To confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads.</strong> If this is an issue with the dashboard, help us improve this dashboard by reporting the problem via the project GitHub page: https://github.com/nationalparkservice/WQdash_public<br><br>")))
            )
          }
   
        shiny::validate(need(httr::http_status(httr::GET(tempURL))$category == "Success", message = FALSE)) # if not a valid service call, abort
        
        checkhydro <- tryCatch(sf::st_read(tempURL, quiet = TRUE), error = function(e) NULL)
  ...
  for(i in 1:length(subset_stations_list)) {
    ...
    summaryResponse <- httr::GET(summaryURL)
if(httr::http_status(summaryResponse)$category != "Success") {
  showModal(urlModal(summaryURL, title = "Data Retrieval Error", subtitle = htmltools::HTML(paste0("Error retrieving data selection information from the Water Quality Portal. The message from the web service is: `", httr::http_status(summaryResponse)$message, "`.<br><br><strong>To confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads.</strong> If this is an issue with the dashboard, help us improve this dashboard by reporting the problem via the project GitHub page: https://github.com/nationalparkservice/WQdash_public<br><br>"))))
}
    shiny::req(httr::http_status(summaryResponse)$category == "Success")
    
    all_summaries[[i]] <- readr::read_csv(httr::content(summaryResponse, "text", encoding = "UTF-8"),
...
for(i in 1:length(rv$wqp_url_list)) {
  ...
  wqpResponse <- httr::GET(wqp_url)
  if(httr::http_status(wqpResponse)$category != "Success") {
    showModal(
      urlModal(wqp_url, title = "Data Retrieval Error", subtitle = htmltools::HTML(paste0("Error retrieving data from the Water Quality Portal for the requested query. The message from the web service is: `", httr::http_status(wqpResponse)$message, "`.<br><br><strong>To confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads.</strong> If an error results, email WQP support (wqx@epa.gov) for assistance. If this is an issue with the dashboard, help us improve this dashboard by reporting the problem via the project GitHub page: https://github.com/nationalparkservice/WQdash_public<br><br>")))
    )
  }
  shiny::validate(need(httr::http_status(wqpResponse)$category == "Success", message = FALSE))
  
  # tmpDat <- tempfile(fileext = ".csv")
  # writeBin(httr::content(wqpResponse, "raw"), tmpDat)
  # all_data[[i]] <-  read.csv("tempDat.csv") %>%
  #   dplyr::filter(CharacteristicName %in% unique(rv$selectedDataSummary$CharacteristicName))
  # unlink(tmpDat)
  # Sys.sleep(0.5) # pause between data pulls
    download.file(wqp_url, "tempDat.csv", mode = "w") # in Windows, mode = "w" when opening text files and "wb" for other types, such as .jpg
  all_data[[i]] <- read.csv("tempDat.csv") %>%
    dplyr::filter(CharacteristicName %in% unique(rv$selectedDataSummary$CharacteristicName))
  Sys.sleep(0.5) # pause between data pulls
}
...
# tempDat <- dplyr::bind_rows(all_data)
# saveRDS(tempDat, here::here("tempDat.RDS"))
# rm(all_data)
...
cat("Line3348")
orgResponse <- httr::GET(orgURL)
if(httr::http_status(orgResponse)$category != "Success") {
  showModal(
    urlModal(orgURL, title = "Data Retrieval Error", subtitle = htmltools::HTML(paste0("Error retrieving organization information from the Water Quality Portal. The message from the web service is: `", httr::http_status(orgResponse)$message, "`.<br><br><strong>To confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads.</strong> If this is an issue with the dashboard, help us improve this dashboard by reporting the problem via the project GitHub page: https://github.com/nationalparkservice/WQdash_public<br><br>")))
  )
}
shiny::req(httr::http_status(orgResponse)$category == "Success")

rv$orgs <- readr::read_csv(httr::content(orgResponse, "text", encoding = "UTF-8"),

...
# tempMeteoDat <- NULL
#   tempMeteoDat <- meteo_pull_monitors(monitors = sort(unique(rv$selectedMeteo$MeteoShortID)), keep_flags = TRUE, date_min = range(rv$dat$ActivityStartDate)[1], date_max = range(rv$dat$ActivityStartDate)[2], var = "PRCP") %>% 
#     dplyr::filter(!is.na(prcp)) %>% # pull precipitation data (tenths of mm) for the stations and date range corresponding with downloaded WQP data

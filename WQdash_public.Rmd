...
### NEXT CHANGES:
# > Import
# -- Switch to dataRetrieval--WAS NOT WORKING
# -- Prior to import, show number of sites and data records. DONE--FIX THE TABLES A BIT, THEN ADD TEXT IN POP-UP GIVING TOTALS AND DIRECTING THEM TO TABS. THEN TEST REST, INCLUDING FINAL SITES SUMMARY.
# -- Allow user to upload a shapefile or select a state (instead of park)
# -- *****Allow selection of stations within the unit shape only (not overlapping watersheds)
# -- NOAA precip data
...
# -- *****Add right-click station data summary (change --> most recent 5 data; 10% and 90% q lines)
#If more than 3 plots showing on a page, need to show x-axis on all of them
# -- Give option to show data in 1-4 columns
# -- *****Need an UNDEFINED threshold category...which is basically if threshold levels have not been defined
...
print("line2635")
  ...
  showModal(modalDialog(
    title = "Done",
    "In the '2. Filter sites to download' panel of the left sidebar, select monitoring sites by HUC or by distance from an origin site, then click the 'Summarize Selected Sites' button."
  ))
...
# 
  ...
  searchURL <- paste0("search?siteid=", stationChoiceString, "&characteristicType=", charTypeChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no") # REMOVED SITETYPE & MINACTIVITIES & START & END DATES AS AN OPTION AT THIS STAGE)
    ...
    summaryURL <- paste0("https://www.waterqualitydata.us/data/summary/monitoringLocation/", searchURL, "&summaryYears=all&count=no&dataProfile=periodOfRecord") # this is an alternative to above
    
        if(http_status(GET(summaryURL))$category!="Success") { # make sure the service call is valid
      showModal(
        urlModal(summaryURL, title = "Data Retrieval Error", subtitle = paste0("Error retrieving data selection information from the Water Quality Portal. The message from the web service is: `", http_status(GET(summaryURL))$message, "`.\n\nTo confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads."))
      )
    }
    
    shiny::req(http_status(GET(summaryURL))$category=="Success") # if not a valid service call, abort

    download.file(summaryURL, "tempSummary.csv", mode = "w")
    rv$importDataSummary <- read_csv("tempSummary.csv", trim_ws = TRUE)
    
    rv$summary_url <- summaryURL
    
    
    
    
  cat("line2991")
  rv$wqp_url <- paste0("https://www.waterqualitydata.us/data/Result/", searchURL)
cat("line2993")
  # Create tab with the summary info after downloading it. Add totas to message. Check what generates the white highligh.
...
showModal(modalDialog(
  title = "Done",
  switch((input$sel_SelectMethod == "methodHuc")+1, 
         "Selected monitoring sites are shown as larger, white-outlined circles on the map. The ORIGIN SITE is outlined in RED. The selected streamlines upstream of the origin are shown in white; downstream, in brown. Select gage and weather stations to download (if available) and select characteristic groups to download, then press the green 'DOWNLOAD SELECTED DATA' button to import the water quality data",
  ...
  if(!is.null(rv$meteoStations)|!is.null(rv$gageStations)) {
    shiny::req(!is.null(rv$importDataSummary))
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  if(http_status(GET(rv$wqp_url))$category!="Success") { # make sure the service call is valid
      showModal(
        urlModal(rv$wqp_url, title = "Data Retrieval Error", subtitle = paste0("Error retrieving data from the Water Quality Portal for the requested query. The message from the web service is: `", http_status(GET(rv$wqp_url))$message, "`.\n\nTo confirm this is a problem with the web service (and not the dashboard), enter the above URL in a browser and see if data successfully downloads. If an error results, email WQP support (wqx@epa.gov) for assistance."))
        )
      }

  shiny::validate(need(http_status(GET(rv$wqp_url))$category=="Success", message = FALSE)) # if not a valid service call, abort
...
### Selected Characteristic Groups
...
output$table_char_summary <- DT::renderDT({
  ...
  DT::datatable(
    rv$importDataSummary %>% dplyr::group_by(CharacteristicType, CharacteristicName) %>% dplyr::summarize(total_activity_count = sum(ActivityCount, na.rm = TRUE), total_result_count = sum(ResultCount, na.rm = TRUE), .groups = "drop"),
    rownames = FALSE,
    filter = "top",
    selection = "none",
    class="compact stripe",
...
wellPanel(
    ...
    p(strong("Define thresholds and categorical data")),

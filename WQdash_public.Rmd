...
FuncPlotDat <- function() {
  ...
  plotDat$DetectionQuantitationLimitMeasure.MeasureValue <- as.numeric(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)
    ...
    if(all(!is.null(rv_param_temp$sel_show_thresholds), !is.null(input$sel_ThreshUnit),  !is.null(input$sel_ThreshLower), !is.null(input$sel_ThreshUpper), !is.null(plotDat))) {
    if(all(rv_param_temp$sel_show_thresholds == "showthresh_custom", rv_param_temp$sel_summarize_by == "by_chartype")) {
    ...
    temp_thresh_custom <- list(
      ...
      "cut_breaks" = temp_cut,
      "cut_categ" = temp_categ) # removed "cut_colors" = temp_colors
...
FuncAddThresh <- function(v_char, all_thresh, p, xlow_temp = NULL, xhigh_temp = NULL, ylow_temp, yhigh_temp) {
  # Function to add threshold background color
  #
  # Args:
  #   v_char:  Characteristic Name
  #   all_thresh: NULL or the list for threshold background colors
  #   p: Base plot
  #   xlow_temp, xhigh_temp, ylow_temp, yhigh_temp: lower/upper x/y limits for threshold box
  #
  # Returns:
  #   Plot with threshold background
  #
  if(v_char %in% as.character(sapply(all_thresh, FUN = function(x) x[1]))) {
    thresh <- all_thresh[[which(sapply(all_thresh, FUN = function(x) x[1])==v_char)]]
    for (i in 1:length(thresh$cut_categ)) {
      ylow = max(thresh$cut_breaks[i], ylow_temp)
      yhigh = min(thresh$cut_breaks[i+1], yhigh_temp)
      if(ylow <= yhigh) {
        fill_col = switch(categ, "good" = "#009E73", "poor" = "#E69F00", "caution" = "#F0E442")
        p <- p +
          geom_rect(data = data.frame(ylow, yhigh), aes(xmin = xlow_temp, xmax = xhigh_temp, ymin = ylow, ymax = yhigh), fill = fill_col, alpha = 0.15, show.legend = FALSE, inherit.aes = FALSE)
      }
    }
...
FuncPlotScatter <- function(plot_dat, quant_df = NULL, categ_df = NULL, x_nam = "ActivityStartDate", combine_sites, fixed_free_y = "fixed", add_loess = TRUE, incl_cens = FALSE, all_thresh = NULL, rotate_x = FALSE, gage_dat = NULL, precip_dat = NULL, month_summary = FALSE, sel_add_gage_precip = NULL, sel_categ, trans_log_y, highlight_yr) {
 ...
 cat("line1488")
    p_points <- p_points +
      ...
      if(rv_param_temp$sel_summarize_by == "by_chartype" & combine_sites==TRUE) { # if coloring by monitoring site, use viridis color scale
        p_points <- p_points +
          suppressWarnings(geom_point(aes(shape = pointShape, color = MonitoringLocationIdentifier, fill = MonitoringLocationIdentifier, text = paste0("<span style='font-size:12px; font-weight:bold;'>", MonitoringLocationIdentifier, "<br>", CharSampleType, "<br>Date: ", ActivityStartDate, "<br>Value: ", y, "</span><br>Characteristic Set: ", CharacteristicSet, "<br>Detection Level: ", ResultDetectionConditionText)), size = 2, show.legend = c(fill = FALSE, color = TRUE, shape = FALSE))) + # this will give a false unknown aesthetics warning for 'text'
...
# Includes censored data
  ...
  cat("line1921")
    if(plot_type %in% c("points", "points_smooth")) {
      ...
      ts_plot <- FuncPlotScatter(plot_dat = plot_dat, quant_df = quant_dat, categ_df = categLevelsPlot, x_nam = time_unit, combine_sites = ifelse(length(unique(plot_dat$CharacteristicName)) == 1, ifelse(sel_combine_sites, TRUE, FALSE), FALSE), fixed_free_y = ifelse(free_y, "free_y", "fixed"), add_loess = ifelse(plot_type == "points_smooth", TRUE, FALSE), incl_cens = incl_cens, all_thresh = switch(is.null(rv_param_temp$threshList[input$sel_Thresholds]) + 1, rv_param_temp$threshList[input$sel_Thresholds], NULL), rotate_x = TRUE, gage_dat = gageDat, precip_dat = precipDat, month_summary = month_summary, sel_add_gage_precip = sel_add_gage_precip, sel_categ = sel_categ, trans_log_y = trans_log_y, highlight_yr = input$sel_HighlightSurvYr %>% debounce(1000))}

    if(plot_type == "box") {
      ts_plot <- FuncPlotBox(plot_dat = plot_dat, quant_df = quant_dat, x_nam = time_unit, combine_sites = ifelse(length(unique(plot_dat$CharacteristicName)) == 1, ifelse(sel_combine_sites, TRUE, FALSE), FALSE), fixed_free_y = ifelse(free_y, "free_y", "fixed"), incl_cens = incl_cens, all_thresh = switch(is.null(rv_param_temp$threshList[input$sel_Thresholds]) + 1, rv_param_temp$threshList[input$sel_Thresholds], NULL), rotate_x = TRUE, gage_dat = gageDat, precip_dat = precipDat, sel_add_gage_precip = sel_add_gage_precip, trans_log_y = trans_log_y)}
...
tempMeteoDat <- NULL
  ...
  FuncUpdateImportSettings(inputs_to_update = import_input) 
    ...
    showModal(modalDialog(
      title = "Done",
      paste0("Data import SUCCESSFUL. You may now navigate to the FILTER DATA tab to choose a subset of data to work with. OPTIONAL: To download the formatted data file (.RDS) to your computer, press the `Save RDS to Computer` button in the left side panel. ") 
    ))
  })
}, ignoreInit = TRUE) # end of observeEvent
```
...
# shiny::validate(need(rv_param_temp$sel_summarize_by == "by_chartype", message = "Currently, this display is available only when summarizing by characteristic (not by station)"))
...
output$out_perc_plots <- renderPlotly({
  ...
  perc_plot_title <- paste0("Percentile levels by ", switch((rv_param_temp$sel_summarize_by == "by_chartype")+1, "WQ characteristic", "monitoring site"), " (rows) and year (cols) for ", switch((rv_param_temp$sel_summarize_by == "by_chartype")+1, unique(rv_param_temp$plotDat$MonitoringLocationIdentifier), unique(rv_param_temp$plotDat$CharacteristicName)), " calculated", switch((input$sel_QuantileGroupBy=="quant_all")+1, " separately by site", " for all sites combined"))
...
wellPanel(
  ...
  renderUI({
    ...
    thresh_choice_values <- c("showthresh_none",
                              switch(!is.null(rv_param_temp$threshList)+1, NA, "showthresh_file"),
  ...
  renderUI({ # Set custom threshold limits
    shiny::req(!is.null(rv_param_temp$subDat), !is.null(input$sel_ShowThresholds), !is.null(input$sel_SummarizeBy), !is.null(rv_param_temp$currentInputs$sel_ThreshLower), !is.null(rv_param_temp$currentInputs$sel_ThreshUpper))
    shiny::req(input$sel_ShowThresholds == "showthresh_custom")
    conditionalPanel(
      condition = "input.sel_ShowThresholds == 'showthresh_custom'",
      p(strong("Define up to TWO threshold cutoff values")),
      ...
      if(all(!is.null(rv_param_temp$currentInputs$sel_ThreshLower), !is.na(rv_param_temp$currentInputs$sel_ThreshLower))) { # If a lower threshold is defined, then ask for classification and give option for upper threshold

        selectInput("sel_ThreshCateg1",
                    label = paste0("Values <", rv_param_temp$currentInputs$sel_ThreshLower, " (inclusive) are classified as:"),
                    choices = c("good", "caution", "poor"),
                    selected = switch(is.null(input$sel_ThreshCateg1)+1, input$sel_ThreshCateg1, "good"))
        
        br()
        
        numericInput("sel_ThreshUpper",
                     label = "Second cutoff (must be higher than first cutoff):",
                     value = rv_param_temp$currentInputs$sel_ThreshUpper,
                     min = NA,
                     max = NA,
                     step = NA)
        
        # If lower AND upper threshold are defined, ask for classification of the values btwn the thresholds
      
      if(all(!is.null(rv_param_temp$currentInputs$sel_ThreshLower), !is.na(rv_param_temp$currentInputs$sel_ThreshLower), !is.null(rv_param_temp$currentInputs$sel_ThreshUpper), !is.na(rv_param_temp$currentInputs$sel_ThreshUpper))) { # various packages have functions that can check for NA or NULL simultaneously, but don't want to deal with possible function deprecation in the future
        selectInput("sel_ThreshCateg2",
                    label = paste0("Values >", input$sel_ThreshLower, " and <", input$sel_ThreshUpper, " (inclusive) are classified as:"),
                    choices = c("good", "caution", "poor"),
                    selected = switch(is.null(input$sel_ThreshCateg2)+1, input$sel_ThreshCateg2, "good")
                    )
        }
        
        selectInput("sel_ThreshCateg3",
                    label = paste0("Values >", switch(is.null(input$sel_ThreshUpper)+1, input$sel_ThreshLower, input$sel_ThreshUpper), " are classified as:"),
                    choices = c("good", "caution", "poor"),
                    selected = switch(is.null(input$sel_ThreshCateg3)+1, input$sel_ThreshCateg3, "good"))
      }
...
renderUI({
  shiny::req(!is.null(rv_param_temp$plotDat), !is.null(input$sel_PlotType))
      ...
      sliderTextInput("sel_HighlightSurvYr", force_edges = TRUE, label = h4(strong("Choose a survey year to highlight blue in points plots")), choices = avail_yrs, selected = switch(is.null(input$sel_HighlightSurvYr)+1, input$sel_HighlightSurvYr, "None"), dragRange = FALSE, animate = FALSE, width = "500px")
  }),
...
TEMP - Define Categorical Characters
====================================== 
...
renderUI({
  FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = FALSE)
  if(any(!rv$importSettings$sel_Organization %in% rv$sitesPointsSub@data$OrganizationFormalName))

...
### PICK UP FROM HERE
#> Site card--don't need to add unit in parameter name col
#> RIght click anywhere in circle
#> CHAT one Ecoli not showing in plot options
...
# 
  ...
  # saveRDS(subset_stations_list, here::here("subset_stations_list.RDS"))
  # 
  ...
  # rv$wqp_url <- paste0("https://www.waterqualitydata.us/data/Result/search?siteid=", stationChoiceString, "&characteristicType=", charTypeChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=resultPhysChem&providers=NWIS&providers=STORET")
    rv$wqp_url <- paste0("https://www.waterqualitydata.us/data/Result/search?siteid=", stationChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=resultPhysChem&providers=NWIS&providers=STORET") # Aug 2025 issue with WQP is I can get many sites IF I don't filter for Characteristic Type, or I can get very few sites and filter for Characteristic Type
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  shiny::req(length(rv$selectedStations$MonitoringLocationIdentifier) > 0)
    ...
    withProgress(message = "Processing data request...", detail = "Importing data from Water Quality Portal, a few stations at a time (please be patient!)", value = 0, {
  ...
  # set_import_count <- set_import_count + 1
  
  # setProgress(1/12, detail = paste0("Importing set #", set_import_count, " (up to 20 sites imported per set)"))

  # download.file(wqp_url, paste0("tempWQPDat", set_import_count, ".csv"), mode = "w") # in Windows, mode = "w" when opening text files and "wb" for other types, such as .jpg

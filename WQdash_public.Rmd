...
# Format WQP data ----
  ...
  # observeEvent(input$button_goSave, {
    ...
    # write_csv(rawDat, paste0("Data_out/", input$out_prefix, "_WQP_raw.csv")) # save the raw WQP data # <<<<<<<<<<<<<<<<<<<<<< PICK UP FROM HERE. DOWNLOAD TO USER'S COMPUTER.
  ...
  rv_param_temp$newImport <- TRUE
    ...
    showModal(modalDialog(
      title = "Done",
      paste0("Data import SUCCESSFUL. To download the formatted .RDS file to your computer, press the `Download Formatted RDS` button in the left side panel. You may now navigate to the FILTER DATA tab to choose a subset of data to work with.") 
    ))
  })
}, ignoreInit = TRUE) # end of observeEvent
```
...
renderUI({
  shiny::req(!is.null(rv$wqp_url))
  em(tags$a(href=rv$wqp_url, "WQP data URL"))# target="_blank", 
})

output$download_RDS <- downloadHandler(
  shiny::req(!is.null(rv)),
  filename = function() {
    paste0("WQdash_", rv$parkID, "_", paste0(gsub(pattern = "-", replacement = "", x = Sys.Date())))
    },
...
Test link to jump to an otherwise hidden page of the dashboard
tags$a("Create custom categorical", onclick="customHref('page-Define-Categorical-Characters'); ")

Test alternative Mann Kendall packages (that hopefully don't app_cache)

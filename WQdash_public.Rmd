...
# >>>>>>>>>>>>> PICK UP FROM HERE--CATEG RESULTS DOESN'T WORK WHEN IMPORTING, DOESN'T SHOW THAT CATEG OPTION FOR COLOR, AND PLOT BY ONE STATION DOESN'T WORK.
...
filterDat$ThreshCateg <- filterDat$CategName <- filterDat$CategResultMeasureValue <- NA # Create these columns so they can be filled if threshold or categorical color files are loaded. Also resets to NA when the filter data are changed.
  ...
  #   cat("ADDING THRESHOLD DATA TO FILTERED DATA FRAME")
...
FuncPlotlyFormat <- function(plotly_list, plotly_title, subplot_titles_vec = NULL, xlab, shared_ylab = NULL, shared_ylab_position = NULL, ylab_vec = NULL, total_height, top_margin = 120, bottom_margin = 20, left_margin = 80, right_margin = 60, is_grouped_boxplots = FALSE) { 
  ...
  for(p in 1:length(plotly_list)) {
    temp_plot <- ggplotly(
      ...
      layout(
        # hoverinfo = "text",
        hovermode = 'x unified', # for semi-transparent hover box
        hoverlabel=list(bgcolor='rgba(255,255,255,0.75)', font=list(color='black', size = 12)), # for semi-transparent hover box. 
  ...
  for (v in 1:nrow(unique_df)) { # for each df row
   ...
   if(!is.null(thresh_list)){
    ...
    if(sel_categ != "none" & !is.null(categ_df)) {
      levs <- unlist(base::strsplit(str_replace_all(categ_df$CategOrder[categ_df$CharSampleType == sel_categ], "[()]", ""), split="//")) # these are the categorical levels from the category import file--pull from there rather than from data to ensure no levels are omitted just because they were not in the data
      ...
      cols <- unlist(base::strsplit(str_replace_all(categ_df$CategColor[categ_df$CharSampleType == sel_categ], "[()]", ""), split="//"))
...
# if(all(!is.null(highlight_yr), highlight_yr != "None")) {
# shiny::req(as.integer(highlight_yr) %in% unique(plot_dat$Yr)){
...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, sel_add_gage_precip = switch(is.null(rv_param_temp$sel_add_gage_precip)+1, rv_param_temp$sel_add_gage_precip, "none"), sel_combine_sites, sel_plot_ht = switch(is.null(input$sel_PlotHt)+1, 280+20*input$sel_PlotHt, 280), trans_log_y) {
...
# Includes censored data
    ...
    # Categorical data columns
      ...
      if(!is.null(plot_type)) {
        month_summary <- all(sel_add_gage_precip != "none", plot_type %in% c("points", "points_smooth"))
      } else {
        month_summary <- FALSE
      }
  cat("line1921")
...
#   saveRDS(ts_plot, "TEMP_ts_plot.RDS")

    # PUT THE PLOTS TOGETHER
cat("line1930")
    shiny::req(!is.null(ts_plot[[1]])) # this is the list of plots
...
FuncSummaryStats <- function(dat, plot_summarize_by) { # <<<<<<<<<<<<<<<<<<<<< PICK UP FROM HERE--CALCULATE THESE ON FILTER PAGE. ALLOW USER TO GROUP BY STATION OR CHAR??
...
observeEvent(eventExpr = input$saved_thresh_file, {
  ...
  tempThresh_df <- out_thresh_file %>%
    ...
    rm(tempThresh_df, tempThresh_list, out_thresh_file)
...
renderUI({
  shiny::req(all(!is.null(rv_param_temp$subDat), !is.null(rv_param_temp$threshList)))
...
unlink("Rplots.pdf")
tags$style("#out_ts_plots{height: calc(95vh - 350px) !important;overflow-y:scroll}")
...
unlink("Rplots.pdf")
tags$style("#out_season_plots{height: calc(95vh - 350px) !important;overflow-y:scroll}")
...
actionButton("button_getCategFile", "Create Definitions File", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border:1px; margin:5px")
...
shinyjs::hidden(
  wellPanel(
    ...
    actionButton("button_showPalettes", "Show Color Palettes", width = "100%"),
    ...
    selectizeInput("sel_CategPalette", # using selectize so user can start by typing in text
                   ...
                   choices = c("YlGnBu", "YlGn", "RdPu", "Purples", "PuRd", "PuBuGn", "PuBu", "OrRd", "Oranges", "Greys", "Greens", "GnBu", "BuPu", "BuGn", "Blues", "Set2", "Set1", "Paired", "Dark2", "Accent", "Spectral", "RdYlGn", "RdYlBu", "RdGy", "RdBu", "PuOr", "PRGn", "PiYG", "BrBG"),
                   multiple = FALSE),
    actionButton("button_updateCategFile", "Add to Color Coding File", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border:1px; margin:5px")
...
div(style="display:inline-block; width:100px; height:30px; margin-right:10px;", actionButton("button_categAddRow", "Add a Row"))
div(style="display:inline-block; width:180px; height:30px;", actionButton("button_categDeleteRow", "Delete Selected Row(s)"))
...
wellPanel(
  ...
  renderUI({ # Set custom threshold limits
    ...
    renderUI({
      shiny::req(all(!is.null(rv_param_temp$subDat), !is.null(input$sel_ShowThresholds), input$sel_SummarizeBy == "by_chartype"))
      shiny::req(input$sel_ShowThresholds == "showthresh_custom", input$sel_NumThresh > 0)
...
tags$div(renderText({
  ...
  paste0("WARNING!! Percentile lines SHOULD NOT be used to determine if values are high or low. Percentiles ONLY indicate the typical range of values AMONG THE DATA YOU SELECTED. If you are interested in identifying actual high/low values, use THRESHOLD LIMITS.")
...
div(style="display:flex; flex-direction:row; max-width:95%; justify-content:center; flex-wrap: wrap; align-items: center;",
    # Left side inputs

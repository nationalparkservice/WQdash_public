...
# Format WQP data ----
  ...
  rv_param_temp$newImport <- TRUE
    ...
    showModal(modalDialog(
      title = "Done",
      paste0("Data import SUCCESSFUL. You may now navigate to the FILTER DATA tab to choose a subset of data to work with. OPTIONAL: To download the formatted .RDS file to your computer, press the `Download Formatted RDS` button in the left side panel. ") 
    ))
  })
}, ignoreInit = TRUE) # end of observeEvent
```
...
# Format plot data for time series plot tab
  if(any(is.null(rv_param_temp$subDat),  is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets), is.null(rv_param_temp$plotSummarizeBy), is.null(input$sel_AddQuantile), is.null(input$sel_Categ), is.null(input$sel_AddGagePrecip), is.null(rv_param_temp$threshType))) { 
   ...
   shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$threshType), !is.null(rv_param_temp$quantDat))

...
# > highlight yr color--how does it react when changing plot data?
...
FuncBaseMap <- function(maptype) {
  ...
  map %<>% addCircleMarkers(data = init_sites, group = "Available monitoring sites", label = ~SITELAB, popup = ~SITEPOP, radius = 4, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = TRUE, weight = 1, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_availableStations"))
    ...
    if(all(!is.null(rv$selectedOriginStation), !is.null(rv$userSelect$sel_SelectMethod))){ # the origin station for up-down selection is outlined in red
  ...
  # These are for any map type
   if(all(rv$userSelect$sel_SelectMethod=="methodHuc",  !is.null(rv$selectedHucs))) { # layer the selected polygons on map, if selection method is by HUC 
     cat("mapping HUCs")
  ...
  if(all(!is.null(rv$selectedOriginStation), !is.null(rv$userSelect$sel_SelectMethod))){
    if(rv$userSelect$sel_SelectMethod=="methodDist") {
...
FuncFilterSelectParam <- function(sel_NPSparam, sel_YrRange, sel_SummaryOrg, sel_ActivityType, sel_ResultStatus, sel_MinYrs) {
  shiny::req(!is.null(rv$dat))
...
FuncInitFilter <- function() {
  ...
  rv_param_temp$default$sel_ResultStatus <- sort(intersect(unique(rv$dat$ResultStatusIdentifier), c("Accepted", "Final", "Historical")))
  ...
  rv_param_temp$selectParam <- FuncFilterSelectParam(
...
FuncRefreshFilter <- function(sel_MinYrs, sel_SummaryParam, sel_SummaryOrg, sel_YrRange, sel_ActivityType, sel_ResultStatus, sel_DefineSeasons) {
  # Update filter data
  shiny::req(!is.null(rv$userSelect$sel_Organization))
  if(all(unique(rv$dat$OrganizationFormalName) %in% rv$userSelect$sel_Organization)==FALSE) {
    ...
    cat("line731")
    filterDat %<>%
      dplyr::filter(Yr >= as.integer(sel_YrRange[1]) & Yr <= as.integer(sel_YrRange[2]) & OrganizationFormalName %in% sel_SummaryOrg & CharacteristicName %in% sel_SummaryParam & ActivityTypeCode %in% sel_ActivityType & ResultStatusIdentifier %in% sel_ResultStatus)
    ...
    rv$userSelect$sel_DefineSeasons <- sel_DefineSeasons
...
FuncPlotlyFormat <- function(plotly_list, plotly_title, subplot_titles_vec = NULL, xlab, shared_ylab = NULL, shared_ylab_position = NULL, ylab_vec = NULL, total_height, top_margin = 120, bottom_margin = 20, left_margin = 80, right_margin = 60, hover_tip = "text", is_grouped_boxplots = FALSE) { # <<<<< CHANGE BACK TO "text" FOR HOVER_TIP
...
cat("line1551")  
    ...
    # If highlighting years
if(all(!is.null(highlight_yr1), as.integer(highlight_yr1) %in% unique(plot_dat$Yr))) {
  highlight_dat1 <- subset(dat_df, Yr == as.integer(highlight_yr1), select = c(x, y))
      if(nrow(highlight_dat1) > 0) {
        p1_points <- p1_points +
          geom_point(data = highlight_dat1, aes(x = x, y = y), color = "orange", size = 2.5)
      }
}

if(all(!is.null(highlight_yr2), as.integer(highlight_yr2) %in% unique(plot_dat$Yr))){
  highlight_dat2 <- subset(dat_df, Yr == as.integer(highlight_yr2), select = c(x, y))
      if(nrow(highlight_dat2) > 0) {
        p1_points <- p1_points +
          geom_point(data = highlight_dat2, aes(x = x, y = y), color = "turquoise2", size = 2.5)
      }
}
...
div(style="display:flex; flex-direction:row; max-width:100%; justify-content:space-evenly; flex-wrap: wrap; align-items: center;",
  ...
  shinyWidgets::dropdownButton(
    ...
    renderUI({
      ...
      avail_yrs1 <- c("None", as.character(sort(unique(rv_param_temp$plotDat$Yr))))
      selectInput(
        "sel_HighlightSurvYr1",
        label = "Choose a survey year to highlight ORANGE",
        choices = avail_yrs1,
        selected = switch(is.null(input$sel_HighlightSurvYr1)+1, input$sel_HighlightSurvYr1, "None")
      )
  }),
      ...
      shiny::req(!is.null(rv_param_temp$plotDat), !is.null(input$sel_PlotType))
      shiny::req(input$sel_PlotType %in% c("points", "points_smooth"))
      avail_yrs2 <- c("None", as.character(sort(unique(rv_param_temp$plotDat$Yr))))
      selectInput(
        "sel_HighlightSurvYr2",
        label = "Choose a survey year to highlight BLUE",
        choices = avail_yrs2,
        selected = switch(is.null(input$sel_HighlightSurvYr2)+1, input$sel_HighlightSurvYr2, "None")
      )
  }),
 ...
 ## Color by categorical character      
  renderUI({
    ...
    # renderUI({
    #   shiny::req(!is.null(rv_param_temp$plotDat))
    #   default_title = paste0("Data for ",  switch((length(unique(rv_param_temp$plotDat$MonitoringLocationIdentifier)) > 1)+1, unique(rv_param_temp$plotDat$MonitoringLocationIdentifier), "multiple sites"))
    #   textInput(
    #     "sel_plot_title",
    ...
    # renderUI({
    #   shiny::req(!is.null(rv_param_temp$plotDat)) 
    #   default_subtitle = paste0(switch((length(unique(rv_param_temp$plotDat$CharacteristicName)) > 1) + 1, unique(rv_param_temp$plotDat$CharacteristicName), "multiple water characteristics"), " (", min(rv_param_temp$plotDat$Yr, na.rm = TRUE), " to ", max(rv_param_temp$plotDat$Yr, na.rm = TRUE), ")")
    #   textInput(
    #   "sel_plot_subtitle",

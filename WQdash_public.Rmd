...
### NEXT TO DO
# >>>> SOMETIMES all orgs are deselected upon init map generation. that is b/c FuncUpdateHome sets selection to none selected, then checkboxGroupInput("sel_Organization") doesn't re-render with the updated rv$sitesPointsSub
# >>>> Pie charts format problem--the pies are very small and left-aligned
# >>>> When I summarize selected data, the transparent de-selected sites disappear; When I update selected data, the red circle around origin site disappears and the up/down lines disappear, but it appears again when I filter to add gage stations (the red circle appears even if the site does not show)
# >>>> When I select a new park to import I get maps with the wrong legend and sometimes map center/view wrong
# >>>> Change the rt-click quick-peek
# >>>> formatting issues with seasonal plot user entries
# > If I change data avail filters before Summarize Selected Data, the data filters don't work--or if i set the filters too rigid so nothing qualifies, it may crash
# > Combine import and filter onto single page if possible with sites summary info
# > TADA data cleaning
# > Save/show import/filter settings for .RDS
# > Show threshold cutoffs being used 
# > All HUCs should only be those overlapping park unit (so for HUC12 it should not be all of them)
# > SNowtel example for pop-up right click: https://nwcc-apps.sc.egov.usda.gov/imap/

### FINAL CLEANUP
# > Increase speed of download, etc.
# > User documentation
# > Comment code, make layout/variable structure/etc. consistent
...
# # Install TADA from GitHub if not already done
...
# Deal with leaflet hidden map issue
tags$script(HTML("
...
# Call this to force an input value to reset to a specified value--I'm not using it at the moment but if I need to, the way to do it is: session$sendCustomMessage("resetValue", list(id = "inputID", value = someValue))
tags$script("
    Shiny.addCustomMessageHandler('resetValue', function(msg) {
...
# Remove sidebar resize handles text
tags$style(HTML("
...
FuncDataRetrievalError <- function(url, data_description, source_name, extra_msg = "") {
  showModal(
    urlModal(url, title = "Data Retrieval Error",
      subtitle = htmltools::HTML(paste0(
        "Error retrieving ", data_description, " from the ", source_name, ".",
        "<br><br><strong>To confirm this is a problem with the web service (and not the dashboard), ",
        "enter the above URL in a browser and see if data successfully downloads.</strong> ",
        if(nchar(extra_msg) > 0) paste0(extra_msg, " ") else "",
        "If this is an issue with the dashboard, help us improve this dashboard by reporting ",
        "the problem via the project GitHub page: https://github.com/nationalparkservice/WQdash_public<br><br>"
      )))
  )
}
...
FuncBaseMap <- function(maptype) {
  ...
  rv_param_temp$palOrg <- leaflet::colorFactor(palette = cbp1_org, domain = unique(rv$sitesPointsSub$OrganizationFormalName), levels = unique(rv$sitesPointsSub$OrganizationFormalName))
  ...
  map <- leaflet(options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.25)) %>%
    ...
    addPolygons(data = rv$unitPoly, group = "NPS unit", color = "black", fillColor = "yellow", fillOpacity = opacity_unselected, weight = 1, options = pathOptions(pane = "pane_unitPoly")) %>% # park unit outline
    addPolygons(data = rv[[rv$importSettings$sel_HUClevel]], group = rv$importSettings$sel_HUClevel, color = "blue", fillColor = "lightblue", fillOpacity = opacity_unselected, weight = 0.5, layerId = ~HUC, options = pathOptions(pane = "pane_HUC")) %>% # HUC polygons
    addScaleBar() %>%
  ...
  init_sites <- subset(rv$sitesPointsSub, OrganizationFormalName %in% isolate(rv$importSettings$sel_Organization) & MonitoringLocationTypeName %in% isolate(rv$importSettings$sel_SiteType))
  ...
  map %<>% addCircleMarkers(data = init_sites, group = "Available monitoring sites", label = ~SITELAB, popup = ~SITEPOP, radius = 4, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = TRUE, weight = 1, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_availableStations"))
      ...
      if(!is.null(rv$downloadStations)) {
        download_stations <- rv$sitesPointsSub %>% dplyr::filter(MonitoringLocationIdentifier %in% rv$downloadStations$MonitoringLocationIdentifier)

        if(nrow(download_stations) > 0) {
          print("Mapping selected sites")
    map %<>%
      addCircleMarkers(data = download_stations, group = "Selected monitoring sites", label = ~SITELAB, popup = ~SITEPOP, radius = 8, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_selectedStations"))
    ...
    if(rv$importSettings$sel_SelectMethod=="methodDist") {
      ...
      map %<>% 
        addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier == rv$selectedOriginStation$identifier,], group = "Selected monitoring sites", layerId = "origin_marker", label = ~SITELAB, popup = ~SITEPOP, radius = 8, fillOpacity = 0, stroke = TRUE, weight = 4, color = "red", opacity = 1, options = pathOptions(pane = "pane_selectedOrigin"))}
    }
    }
  ...
  # These are for any map type
   if(all(rv$importSettings$sel_SelectMethod=="methodHuc",  !is.null(rv$selectedHucs))) { # layer the selected polygons on map, if selection method is by HUC 
     print("mapping HUCs")
    map %<>%
      addPolygons(data = rv[[rv$importSettings$sel_HUClevel]][rv[[rv$importSettings$sel_HUClevel]]$HUC %in% rv$selectedHucs,], group = rv$importSettings$sel_HUClevel, color = "blue", fillColor = "lightblue", fillOpacity = opacity_selected, weight = 1.5, layerId = ~HUC, options = pathOptions(pane = "pane_HUC"))
   }
  ...
  if(all(!is.null(rv$selectedOriginStation), !is.null(rv$importSettings$sel_SelectMethod))){
    ...
    map %<>%
        addPolylines(data = rv$selectedUpstreamLines, color = "blue", weight = 2, opacity = 1, group = "Selected monitoring sites", options = pathOptions(pane = "pane_selectedStreamlines")) # upstream lines in blue
  }
    }
}
  ...
  if(!is.null(rv$gageStations)) { # Add USGS gage (streamflow) stations only if found
    ...
    map %<>%
      addAwesomeMarkers(icon = iconGages, data = rv$gageStations, group = "Gage stations (water drop icon)", label = ~GAGELAB, popup = ~GAGEPOP, options = pathOptions(pane = "pane_gageStations")) # selected stream gage stations
  }

  print("about to map meteoStations")
  if(!is.null(rv$meteoStations)) { # Add weather stations only if found
    print("map meteoStations")
    map %<>%
      addAwesomeMarkers(icon = iconMeteo, data = rv$meteoStations, group = "Weather stations (cloud icon)", label = ~METEOLAB, popup = ~METEOPOP, options = pathOptions(pane = "pane_meteoStations")) # weather stations
  }
  
  if(!is.null(rv$facilitiesPointsSub)) { # Add EPA-monitored discharge facilities only if found
    print("map EPA facilities")
    map %<>%
      addAwesomeMarkers(icon = iconCWA, data = rv$facilitiesPointsSub, group = "Discharge facilities (bldg icon)", label = ~FACLAB, popup = ~FACPOP)
  }
  ...
  if(all(rv$selectedGages$GageID %in% rv$gageStations$GageID)) { 
    ...
    map %<>%
      addAwesomeMarkers(icon = iconGagesSelected, data = rv$selectedGages, group = "Gage stations (water drop icon)", label = ~GAGELAB, popup = ~GAGEPOP, options = pathOptions(pane = "pane_gageStations_selected"))
  }}
  ...
  if(all(!is.null(rv$meteoStations), !is.null(rv$selectedMeteo))) {
    if(all(rv$selectedMeteo$MeteoID %in% rv$meteoStations$MeteoID)) {
      ...
      map %<>%
        addAwesomeMarkers(icon = iconMeteoSelected, data = rv$selectedMeteo, group = "Weather stations (cloud icon)", label = ~METEOLAB, popup = ~METEOPOP, options = pathOptions(pane = "pane_meteoStations_selected"))
    }}
...
FuncUpdateFilterInputs <- function() {
  ...
  updateNumericInput(session,
                     "sel_QuantileProbLower",
                     value = 0.05)
  
  updateNumericInput(session,
                     "sel_QuantileProbUpper",
                     value = 0.95)
...
FuncRefreshFilter <- function(sel_MinYrs, sel_SummaryParam, sel_SummaryOrg, sel_YrRange, sel_ActivityType, sel_ResultStatus, sel_DefineSeasons) {
  ...
  if(all(unique(rv$dat$OrganizationFormalName) %in% rv$importSettings$sel_Organization)==FALSE) {
    ...
    if(nrow(filterDat) > 0 && isTRUE(sel_DefineSeasons %in% c("WaterSeasons", "CustomSeasons")))
    {
...
FuncPlotDat <- function() {
  ...
  plotDat$DetectionQuantitationLimitMeasure.MeasureValue <- as.numeric(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)
    ...
    if(all(rv_param_temp$sel_show_thresholds == "showthresh_custom", rv_param_temp$sel_summarize_by == "by_chartype", setequal(unique(plotDat$CharSampleType), rv_param_temp$showCharTypes), input$sel_NumThresh > 0)) {# setequal instead of identical b/c latter requires order to also match
      ...
      chartype_key <- unique(plotDat$CharSampleType)
      shiny::req(length(chartype_key) == 1)
      rv_param_temp$threshCustom[[chartype_key]] <- list(
  ...
  if(any(is.null(input$sel_AddQuantile), input$sel_AddQuantile == FALSE, is.null(input$sel_QuantileProbLower), is.null(input$sel_QuantileProbUpper))) {
      ...
      if(isTRUE(rv_param_temp$sel_summarize_by == "by_chartype") && isTRUE(rv_param_temp$sel_quantile_group_by == "quant_all")) { # if 'by_chartype' and calculating one quantile across all stations. isTRUE returns FALSE for NULL, logical(0) and anything weird. && means the second condition is never evaluated if the first is FALSE.
...
FuncGageDat <- function(gage_station) {
  ...
  if(nrow(gageDat) > 0 && isTRUE(rv$importSettings$sel_DefineSeasons %in% c("WaterSeasons", "CustomSeasons")))
    {
...
FuncPrecipDat <- function(precip_station) {
  ...
  precipDat <- subset(rv$meteoDat, MeteoShortID == rv$selectedMeteo$MeteoShortID[rv$selectedMeteo$MeteoName == precip_station] & ActivityStartDate >= min(rv_param_temp$plotDat$ActivityStartDate, na.rm = TRUE) & ActivityStartDate <= max(rv_param_temp$plotDat$ActivityStartDate, na.rm = TRUE)) %>%
    ...
    # Assign custom seasons if applicable
  if(nrow(precipDat) > 0 && isTRUE(rv$importSettings$sel_DefineSeasons %in% c("WaterSeasons", "CustomSeasons")))
    {
...
FuncQuantLines <- function(quant_df, combine_sites, dat_df, p) {
  if(!is.null(quant_df)) {
    if(nrow(quant_df)>0) {
      ...
      if(isTRUE(rv_param_temp$sel_summarize_by == "by_chartype") && isTRUE(combine_sites) && isTRUE(length(unique(dat_df$MonitoringLocationIdentifier)) > 1)) {
        if(length(unique(quant_df$QuantLineLower)) > 1 | length(unique(quant_df$QuantLineUpper)) > 1) {
          showModal(modalDialog(
            title = "Percentile by Monitoring Site",
            "When combining monitoring sites in a plot, the percentile limits must be calculated across all sites rather than by individual sites. In the sidebar options, select 'Calculate percentile limits --> across sites', then refresh the plot data."
            ))
...
if(x_nam == "ActivityStartDate") {
  ...
  if(isTRUE(rv_param_temp$sel_summarize_by == "by_chartype") && isTRUE(combine_sites)) {
  ...
  for (v in 1:nrow(unique_df)) { # for each df row
   ...
   if(!is.null(thresh_list)){
    ...
    if(isTRUE(sel_categ != "none") && !is.null(categ_df)) {
      ...
      if(isTRUE(rv_param_temp$sel_summarize_by == "by_chartype") && isTRUE(combine_sites)) { # if coloring by monitoring site, use viridis color scale
        p_points <- p_points +
...
print("line1551")  
  ...
  if(isTRUE(sel_add_gage_precip == "streamflow") && !is.null(gage_dat)) {
  ...
  if(isTRUE(sel_add_gage_precip == "precipitation") && !is.null(precip_dat)) {
...
FuncPlotBox <- function(plot_dat, quant_df = NULL, x_nam = "Yr", combine_sites = FALSE, fixed_free_y = "fixed", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, gage_dat = NULL, precip_dat = NULL, sel_add_gage_precip = NULL, trans_log_y) {
  ...
  # If including censored data...
  if(incl_cens) {
    if(!is.null(y_lowerQL) && !is.na(y_lowerQL) && y_lowerQL >= yaxes_min) { # add LQL line
      p1_box <- p1_box +
        geom_hline(yintercept = y_lowerQL, linetype = "dotted", color = "gray")
      }
    if(!is.null(y_upperQL) && !is.na(y_upperQL) && y_upperQL <= yaxes_max) { # add UQL line
      p1_box <- p1_box +
...
# Includes censored data
    ...
    if(isTRUE(time_unit == "ActivityStartDate") && isTRUE(input$sel_PlotType == "box")) {time_unit <- "Yr"} # Box plots for time series are grouped by year
...
FuncPieMap <- function(pie_map, map_by_yr = input$sel_MapByYr, limit_type, limit_name, pie_size) {
        ...
        pie_map %<>% 
          ...
          addMinicharts(
            ...
            opacity = 1) %>%
          hideGroup("Filtered monitoring sites") %>%
...
FuncParamTable <- function(df, threshold_df, site) {
  ...
  # Bar chart builder
  build_css_bar <- function(low, q05, med, q95, high, recent_values_str, recent_dates_str, n) {
    ...
    for (i in seq_along(recent_values)) {
      ...
      if (val < q05) {
        ...
        if (q95 > q05) {
          relative_pos <- (val - q05) / (q95 - q05)
          x_pos <- 5 + relative_pos * 90
        } else {
          x_pos <- 50
        }
    ...
    median_pos <- if (q95 > q05) 5 + ((med - q05) / (q95 - q05)) * 90 else 50 
    
    # Count summary
    num_low <- sum(recent_values < q05)
    num_high <- sum(recent_values > q95)
    num_normal <- length(recent_values) - num_low - num_high
    
    # Generate summary text
    summary_parts <- c()
    if (num_low > 0) summary_parts <- c(summary_parts, sprintf("%d in lower 5%%", num_low))
    if (num_normal > 0) summary_parts <- c(summary_parts, sprintf("%d in middle 90%%", num_normal))
    if (num_high > 0) summary_parts <- c(summary_parts, sprintf("%d in upper 5%%", num_high))
    
    summary_text <- if (length(summary_parts) > 0) {
      paste(summary_parts, collapse = ", ")
    } else {
      "All values in middle 90% of historical data"
    }
    
    sprintf(
      '<div style="position: relative; height: 80px; background: linear-gradient(to right, #f0f4f8 0%%, #f0f4f8 5%%, #ffffff 5%%, #ffffff 95%%, #f0f4f8 95%%, #f0f4f8 100%%); border-radius: 8px; border: 1px solid #dee2e6; overflow: visible; margin-top: 12px;">
         <div style="position: absolute; top: -18px; left: -20px; font-size: 10px; color: #6c757d; font-weight: 600; text-align: left; white-space: nowrap;">LOWER 5%% (< %.1f)</div>
         <div style="position: absolute; top: -18px; left: 5%%; font-size: 10px; color: #495057; font-weight: 600; text-align: center; width: 90%%; white-space: nowrap;">MIDDLE 90%% OF DATA (%.1f - %.1f; Median: %.1f)</div>
         <div style="position: absolute; top: -18px; right: -20px; font-size: 10px; color: #6c757d; font-weight: 600; text-align: right; white-space: nowrap;">UPPER 5%% (> %.1f)</div>
         <div style="position: absolute; top: 0; bottom: 0; left: 5%%; width: 2px; background: #6c757d; opacity: 0.3; border-radius: 1px;"></div>
         <div style="position: absolute; top: 0; bottom: 0; left: 95%%; width: 2px; background: #6c757d; opacity: 0.3; border-radius: 1px;"></div>
         <div style="position: absolute; top: 0; bottom: 0; left: 5%%; width: 90%%; background: rgba(0, 123, 255, 0.08); z-index: 1;" title="5th to 95th percentile range: %.1f - %.1f"></div>
      ...
      q05, q05, q95, med, q95, q05, q95, median_pos, med, dots_html
    )
  }
  ...
  css <- "
    ...
    .bar-column { width: 320px; text-align: center; white-space: normal;}
  ...
  rows <- lapply(1:nrow(df), function(i) {
    ...
    q05 <- as.numeric(row$q05)
    med <- as.numeric(row$Median_Value_Numeric)
    q95 <- as.numeric(row$q95)
    ...
    param_info <- sprintf('<div class="param-info"><div class="param-name">%s</div><div class="param-details"><div>N = %d samples</div><div>Years: %s</div><div>Range: %.1f - %.1f %s</div><div>Median: %.1f %s</div><div>Most recent: %.1f (%s)</div></div></div>', param, n, row$Year_Range, low, high, unit, med, unit, val, row$Most_Recent_Sample_Date)

    # Bar chart
    bar_html <- tryCatch(build_css_bar(low, q05, med, q95, high, as.character(row$Recent_Values), as.character(row$Recent_Dates), n), error = function(e) sprintf('<div style="padding: 20px; text-align: center; color: #666;">Error: %s</div>', e$message))
    ...
    shiny::tags$table(
      ...
      shiny::tags$thead(
        shiny::tags$tr(
          ...
          shiny::tags$th("Recent Data Values (up to 5 shown, with most recent at top)", style = "width: 320px;"),
...
FuncClearAll <- function(rv, rv_param_temp) {
  for (nm in names(rv_defaults)) rv[[nm]] <- rv_defaults[[nm]]
  for (nm in names(rv_param_defaults)) rv_param_temp[[nm]] <- rv_param_defaults[[nm]]

  lapply(c("new_data_P1", "new_data_P2", "create_categfile_P1",
           "create_categfile_P2", "create_categfile_P3", "create_categfile_P4"),
         shinyjs::hideElement)
...
```{r startup}
  rv_defaults <- list(wqp_url_list = list(), parkName = NULL, parkID = NULL, unitPoly = NULL, HUC10 = NULL, HUC12 = NULL, sitesPointsSub = NULL, meteoStations = NULL, selectedMeteo = NULL, meteoDat = NULL, facilitiesPointsSub = NULL, selectedHucs = NULL, selectedStations = NULL, selectedOriginStation = NULL, selectedUpstreamStations = NULL, selectedDownstreamStations = NULL, selectedUpstreamLines = NULL, selectedDownstreamLines = NULL, selectedDataSummary = NULL, downloadDataSummary = NULL, downloadStations = NULL, orgs = NULL, gageStations = NULL, selectedGages = NULL, gagesDat = NULL, dat = NULL, importSettings = list())

rv_param_defaults <- list(defaultInputs = list(), currentInputs = list(), loadMap = FALSE, saved_map_groups = NULL, resetFilter = FALSE, newImport = NULL, palOrg = NULL, selectCharSampleType = NULL, N_plotly_page = NULL, subDat = NULL, subDatTypes = NULL, subDatYears = NULL, popupSiteSummary = NULL, filterNPlot = NULL, showStations = NULL, sel_summarize_by = NULL, sel_quantile_group_by = NULL, sel_add_gage_precip = NULL, sel_gage_station = NULL, sel_gage_yscale = NULL, sel_precip_station = NULL, sel_precip_yscale = NULL, saved_param_map_groups = NULL, plotDat = NULL, quantDat = NULL, ggplot_ts_list = NULL, ggplot_season_list = NULL, finalPlot = list(plotly_page = NULL, shared_legend = NULL), seasonalPlot = list(plotly_page = NULL, shared_legend = NULL), plotSummary = NULL, precipDat = NULL, categLevels = NULL, categLevelsPlot = NULL, categLevelsImport = NULL, threshList = NULL, threshDF = NULL, threshCustom = NULL, threshColors = c("#56B4E9", "#F0E442", "#E69F00"), sel_show_thresholds = NULL, percCateg = NULL, showDM = FALSE, seasonType = NULL, param_map_highlight = NULL, update_from_sidebar = NULL)
  
  rv <- do.call(reactiveValues, rv_defaults)
rv_param_temp <- do.call(reactiveValues, rv_param_defaults)
...
if(input$sel_GetData == "new_data") { 
  ...
  rv$parkName <- isolate(input$sel_UnitCode)
  ...
  shinyjs::hideElement("new_data_P1")
  ...
  parks_sf <- readRDS(here::here("dashboard_files", "parks_sf.RDS"))
   ...
   # Get HUC10 & HUC12 boundary data from web services ----
    ...
    for(hucLevel in 6:5) {
      ...
      # remove HTML-escaped ampersands # <<<<< NEEDED FOR OLDER R VERSIONS
hucURL <- gsub("&amp;", "&", hucURL, fixed = TRUE)
      
      tmp_huc <- tempfile(fileext = ".geojson")
tryCatch(
  # download.file(hucURL, tmp_huc, mode = "w", quiet = TRUE),
  curl::curl_download(hucURL, tmp_huc), # <<<<< NEEDED FOR OLDER R VERSIONS
  error = function(e) {
    FuncDataRetrievalError(hucURL, paste0(hucName, " boundary data"), "National Watershed Boundary Dataset")
    shiny::req(FALSE)
  }
)
hucPoly <- tryCatch(sf::st_read(tmp_huc, quiet = TRUE), error = function(e) NULL)
unlink(tmp_huc)
...
unlink(tmp_sites)
  ...
  temp_points <- sf::st_as_sf(x = sites, 
    ...
    temp_points <- sf::st_transform(temp_points, crs = sf::st_crs(rv$unitPoly)) # convert to WGS84
    ...
    sitesPoints <- sf::st_filter(temp_points, rv[["HUC10"]]) # These are the ones that fall within HUC10's, so keep them
    ...
    rv$importSettings$sel_Organization <- unique(rv$sitesPointsSub$OrganizationFormalName)
rv$importSettings$sel_SiteType <- unique(rv$sitesPointsSub$MonitoringLocationTypeName)[!grepl("Well", unique(rv$sitesPointsSub$MonitoringLocationTypeName))]

cat("rv$importSettings$sel_Organization: ", rv$importSettings$sel_Organization)
 
    rm(sites, temp_points, sitesPoints)
    # unlink("tempSites.csv")
  
  # Get USGS stream gauging station information, within HUC10's, from dashboard files ----
  ...
  rm(gages)
  if(exists("temp_gagepoints")) rm(temp_gagepoints)
  if(exists("gagePoints")) rm(gagePoints)

  # Get locations of weather stations, for the bounding box, from dashboard files ----
  ...
  rv$meteoStations <- temp_noaapoints
    ...
    showModal(modalDialog(
      ...
      "Could not find weather stations with at least 10 years of data, located within the park's HUC10 watershed boundaries box (active = most recent data are less than one year old). These data are not necessary for visualizing water quality data on the dashboard.",
      easyClose = FALSE,
      footer = tagList(
...
#     }
  ...
  facilitiesURL <- paste0(
  "https://ofmpub.epa.gov/echo/cwa_rest_services.get_facility_info?output=GEOJSON&p_maj=Y&p_wbd=",
  paste(rv$HUC10$HUC, collapse = ",")
)
print("line2614")
print(facilitiesURL)

# Try GET request
facilitiesResp <- tryCatch(httr::GET(facilitiesURL), error = function(e) NULL)

if (is.null(facilitiesResp) || httr::http_status(facilitiesResp)$category != "Success") {
  showModal(modalDialog(
    title = "No Discharge Facilities Data",
    "Error retrieving discharge facilities locations data from EPA ECHO. These data are not necessary for visualizing water quality data on the dashboard.",
    easyClose = FALSE,
  ...
  # Check if response message includes invalid HUCs
  resp_content <- tryCatch(httr::content(facilitiesResp), error = function(e) NULL)

  if (!is.null(resp_content$message) && length(stringr::str_detect(resp_content$message, "invalid:")) > 0) {
    lostHUCS <- unlist(stringr::str_extract_all(resp_content$message, "[[:digit:]]{10}"))
    subsetHUC <- setdiff(rv$HUC10$HUC, lostHUCS)

    facilitiesURL <- paste0(
      "https://ofmpub.epa.gov/echo/cwa_rest_services.get_facility_info?output=GEOJSON&p_maj=Y&p_wbd=",
      paste(subsetHUC, collapse = ",")
    )
    print(facilitiesURL)
  ...
  if (!is.null(facilitiesResp)) {
    tempFacilitiesOutput <- "tempFacilities.geojson"
    print("line2360")
    
    # Try downloading the file
    downloadSuccess <- tryCatch({
      download.file(facilitiesURL, tempFacilitiesOutput, quiet = TRUE)
      TRUE
    }, error = function(e) FALSE)

    if (downloadSuccess) {
      facilitiesPoints <- tryCatch(
        readOGR(dsn = tempFacilitiesOutput, dropNULLGeometries = FALSE),
        error = function(e) NULL
      )

      if (!inherits(facilitiesPoints, "SpatialPointsDataFrame")) {
        rv$facilitiesPointsSub <- NULL
        showModal(modalDialog(
          title = "No Discharge Facilities Data",
          "Error retrieving discharge facilities locations data from EPA ECHO. These data are not necessary for visualizing water quality data on the dashboard.",
          easyClose = FALSE,
          footer = tagList(actionButton("dismiss_discharge_modal", "Got it!"))
        ))
      } else {
        facilitiesPoints <- dplyr::mutate(
          facilitiesPoints,
          FACLAB = paste0("Discharge Facility: ", CWPName, " [", SourceID, "]"),
          FACPOP = paste0(
            'Discharge Facility: <a href="https://echo.epa.gov/detailed-facility-report?fid=',
            SourceID, '" target="_blank">', CWPName, '</a>',
            "<br>Source ID: ", SourceID,
            "<br>Type: ", CWPFacilityTypeIndicator,
            "<br>County: ", CWPCounty,
            "<br>State: ", CWPState
          )
        )
      ...
      showModal(modalDialog(
        title = "No Discharge Facilities Data",
        "Unable to download discharge facilities data from EPA ECHO. These data are not necessary for visualizing water quality data on the dashboard.",
        easyClose = FALSE,
        footer = tagList(actionButton("dismiss_discharge_modal", "Got it!"))
      ))
    }

    unlink("tempFacilities.geojson")  # cleanup
    rm(facilitiesURL, tempFacilitiesOutput)
  }
}
...
showModal(modalDialog(
  title = "Done",
  HTML("In the <strong>'2 (of 3). Filter sites to download'</strong> panel, choose the monitoring sites you want to download data from. You can:<ul>
    <li>Download data for the entire park, or </li>
    <li>Download from sites in specific (sub)watersheds, or
    <li>Download from sites upstream/downstream of a specified monitoring site.</li>
  </ul>
  When you're ready, click <strong>'Summarize Selected Data'</strong> to preview a summary of the data before downloading.")
))
...
# Action to summarize selected data ----
observeEvent(eventExpr = input$button_summarizeData, {
  ...
  if(is.null(input$sel_CharType)) {
    showModal(modalDialog(
      ...
      "Please select at least one characteristic group"))
    shiny::req(!is.null(input$sel_CharType))
  }
  
  rv$selectedDataSummary <- NULL
  rv$downloadDataSummary <- NULL
  rv$downloadStations <- NULL
 ...
 # Select stations by distance from origin--tried dataRetrieval findNLDI, but seems to only pull USGS sites
  if(input$sel_SelectMethod == "methodDist") {
    ...
    if(isTRUE(input$sel_DistKmUpstream == 0) && isTRUE(input$sel_DistKmDownstream == 0)) {
      showModal(modalDialog(
    ...
    withProgress(message = "Getting Information from Hydro Network-Linked Data Index...", value = 0, {
    ...
    nldiURLS <- list(
      selectedOriginStation = paste0("https://api.water.usgs.gov/nldi/linked-data/wqp/", input$sel_OriginStationID),
      selectedUpstreamLines = ifelse(input$sel_DistKmUpstream > 0, paste0("https://api.water.usgs.gov/nldi/linked-data/wqp/", input$sel_OriginStationID, "/navigation/", ifelse(input$sel_IncludeTribs, "UT", "UM"), "/flowlines?distance=", input$sel_DistKmUpstream), NA),
      selectedDownstreamLines = ifelse(input$sel_DistKmDownstream > 0, paste0("https://api.water.usgs.gov/nldi/linked-data/wqp/", input$sel_OriginStationID, "/navigation/", ifelse(input$sel_IncludeDiversions, "DD", "DM"), "/flowlines?distance=", input$sel_DistKmDownstream), NA),
      selectedUpstreamStations = ifelse(input$sel_DistKmUpstream > 0, paste0("https://api.water.usgs.gov/nldi/linked-data/wqp/", input$sel_OriginStationID, "/navigation/", ifelse(input$sel_IncludeTribs, "UT", "UM"), "/wqp?distance=", input$sel_DistKmUpstream), NA),
      selectedDownstreamStations = ifelse(input$sel_DistKmDownstream > 0, paste0("https://api.water.usgs.gov/nldi/linked-data/wqp/", input$sel_OriginStationID, "/navigation/", ifelse(input$sel_IncludeDiversions, "DD", "DM"), "/wqp?distance=", input$sel_DistKmDownstream), NA))
    ...
    for(n in names(nldiURLS)) {
      ...
      if(!is.na(nldiURLS[[n]])) { # if URL is specified
        ...
        tmp_nldi <- tempfile(fileext = ".geojson")
tryCatch(
  # download.file(tempURL, tmp_nldi, mode = "w", quiet = TRUE),
  curl::curl_download(tempURL, tmp_nldi), # <<<<< NEEDED FOR OLDER R VERSIONS
  error = function(e) {
    FuncDataRetrievalError(tempURL, paste0("data for the `", n, "` query"), "Hydro Network-Linked Data Index")
    shiny::req(FALSE)
  }
)
        
        checkhydro <- tryCatch(sf::st_read(tmp_nldi, quiet = TRUE), error = function(e) NULL)
unlink(tmp_nldi)
    
        if(is.null(checkhydro)) {
          showModal(modalDialog(
            title = "No Data",
            "Error retrieving information from the Hydro Network-Linked Data Index",
            footer = modalButton("OK")
            ))
        } else {
          checkhydro <- sf::st_transform(checkhydro, crs = 4326) # convert to WGS84
          rv[[n]] <- checkhydro 
        }
      }
    }
    ...
    rv$selectedStations <- 
      dplyr::bind_rows(sf::st_drop_geometry(rv$selectedOriginStation), if(!is.null(rv$selectedUpstreamStations)) sf::st_drop_geometry(rv$selectedUpstreamStations), if(!is.null(rv$selectedDownstreamStations)) sf::st_drop_geometry(rv$selectedDownstreamStations)) %>% # combine into one data frame
      dplyr::rename(MonitoringLocationIdentifier = identifier) %>%
      dplyr::left_join(unique(data.frame(rv$sitesPointsSub[, c("MonitoringLocationIdentifier", "OrganizationFormalName", "MonitoringLocationTypeName")])), by = "MonitoringLocationIdentifier") %>%
      dplyr::filter(OrganizationFormalName %in% input$sel_Organization & MonitoringLocationTypeName %in% input$sel_SiteType) %>% # limit to selected orgs and site types
      dplyr::distinct() %>%
      dplyr::arrange(MonitoringLocationIdentifier) %>%
      as.data.frame(.)
  ...
  withProgress(message = "Summarizing selected data...", value = 0, { 

  # BATCH PROCESSING: For downloading data, break up large data requests
  all_summaries <- list()
  ...
  subset_stations_list <- split(unique(rv$selectedStations$MonitoringLocationIdentifier), ceiling(seq_along(unique(rv$selectedStations$MonitoringLocationIdentifier))/20)) # import data for 20 stations (max) at a time

  for(i in 1:length(subset_stations_list)) {
    cat(i)
    s <- subset_stations_list[[i]]
    stationChoiceString <- paste0(utils::URLencode(s, reserved = TRUE), collapse = "&siteid=")
    ...
    # remove HTML-escaped ampersands # <<<<< NEEDED FOR OLDER R VERSIONS
  summaryURL <- gsub("&amp;", "&", summaryURL, fixed = TRUE)

    tmp_summary <- tempfile(fileext = ".csv")
tryCatch(
  # download.file(summaryURL, tmp_summary, mode = "w", quiet = TRUE),
  curl::curl_download(summaryURL, tmp_summary), # <<<<< NEEDED FOR OLDER R VERSIONS
  error = function(e) {
    FuncDataRetrievalError(summaryURL, "data selection information", "Water Quality Portal")
    shiny::req(FALSE)
  }
)
all_summaries[[i]] <- readr::read_csv(tmp_summary, trim_ws = TRUE, show_col_types = FALSE) %>%
  dplyr::select(-MonitoringLocationUrl, -CountyName, -StateName, -MonitoringLocationLatitude, -MonitoringLocationLongitude)
unlink(tmp_summary)

    rv$wqp_url_list[[i]] <- paste0("https://www.waterqualitydata.us/data/Result/search?siteid=", stationChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=resultPhysChem&providers=NWIS&providers=STORET") %>%
      gsub("&amp;", "&", ., fixed = TRUE) # <<<<< FIX AMPERSANDS BACK
    Sys.sleep(0.5) # pause between data pulls
  }
  
  rv$selectedDataSummary <- dplyr::bind_rows(all_summaries)
rm(all_summaries)

# Apply default data availability filters immediately (defaults: 30 yrs, 0 min results)
qualifying_site_chars_default <- rv$selectedDataSummary %>%
  dplyr::group_by(MonitoringLocationIdentifier, CharacteristicName) %>%
  dplyr::summarise(max_yr = max(YearSummarized, na.rm = TRUE),
                   total_results = sum(ResultCount, na.rm = TRUE),
                   .groups = "drop") %>%
  dplyr::filter(max_yr >= (lubridate::year(Sys.Date()) - 30),
                total_results >= 0)

rv$downloadDataSummary <- rv$selectedDataSummary %>%
  dplyr::inner_join(dplyr::select(qualifying_site_chars_default, MonitoringLocationIdentifier, CharacteristicName),
                    by = c("MonitoringLocationIdentifier", "CharacteristicName"))

rv$downloadStations <- rv$selectedStations %>%
  dplyr::filter(MonitoringLocationIdentifier %in% unique(qualifying_site_chars_default$MonitoringLocationIdentifier))
...
showModal(modalDialog(
  title = "Done",
  HTML(paste0("<strong>You have summarized ", sum(rv$selectedDataSummary$ResultCount), " data records from ", length(unique(rv$selectedDataSummary$MonitoringLocationIdentifier)), " sites.</strong> Selected monitoring sites are shown as larger, white-outlined circles on the map.", 
              switch((input$sel_SelectMethod == "methodDist")+1, "", "The ORIGIN SITE is outlined in RED. The selected streamlines upstream of the origin are shown in blue; downstream, in brown."),
              "<br><br>Review your selection in the 'Summary of Selected Characteristics' and 'Summary of Selected Site-Characteristics' tabs. <br><br>In panel 3 (of 3), filter the data to the subset you actually want to download (<strong>UPDATE SELECTED DATA</strong> to see how that changes the selected data), optionally choose gage stations to download data from, then press the <strong>DOWNLOAD DATA</strong> button to pull the data into the dashboard.")) 
)) # <<<<<< AFTER ADDING WEATHER SHOULD SAY 'GAGE AND WEATHER'
...
```{r action_button_updateSelectedData}
observeEvent(input$button_updateSelectedData, {
  req(input$map_bounds)
  shiny::req(!is.null(rv$selectedDataSummary), !is.null(rv$selectedStations))
  
  cutoff_year <- lubridate::year(Sys.Date()) - as.integer(input$sel_ActiveWithinYrs)
  
  qualifying_site_chars <- rv$selectedDataSummary %>%
    dplyr::group_by(MonitoringLocationIdentifier, CharacteristicName) %>%
    dplyr::summarise(max_yr = max(YearSummarized, na.rm = TRUE),
                     total_results = sum(ResultCount, na.rm = TRUE),
                     .groups = "drop") %>%
    dplyr::filter(max_yr >= cutoff_year, total_results >= input$sel_MinResults)
  
    if(nrow(qualifying_site_chars) == 0) {
    showModal(modalDialog(
      title = "No qualifying data",
      "No site-characteristic combinations meet the current data availability filters. Please relax your filter criteria."
    ))
    return()
  }
  
  rv$downloadDataSummary <- rv$selectedDataSummary %>%
    dplyr::inner_join(dplyr::select(qualifying_site_chars, MonitoringLocationIdentifier, CharacteristicName),
                      by = c("MonitoringLocationIdentifier", "CharacteristicName"))
  
  rv$downloadStations <- rv$selectedStations %>%
    dplyr::filter(MonitoringLocationIdentifier %in% unique(qualifying_site_chars$MonitoringLocationIdentifier))
  
  qualifying_sites_sf <- rv$sitesPointsSub %>%
    dplyr::filter(MonitoringLocationIdentifier %in% rv$downloadStations$MonitoringLocationIdentifier)
  
  leafletProxy("map") %>%
    clearGroup("Selected monitoring sites") 
  
  if(nrow(qualifying_sites_sf) > 0) {
    leafletProxy("map") %>% 
      addCircleMarkers(data = qualifying_sites_sf, group = "Selected monitoring sites",
                     label = ~SITELAB, popup = ~SITEPOP,
                     radius = 8, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName),
                     fillOpacity = 0.8, stroke = TRUE, weight = 4, color = "white", opacity = 1,
                     options = pathOptions(pane = "pane_selectedStations"))
  }
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  shiny::req(length(input$sel_CharType) > 0)
    ...
    if(is.null(rv$downloadStations) | length(rv$downloadStations$MonitoringLocationIdentifier) == 0 | is.null(rv$wqp_url_list)) {
    showModal(modalDialog(
      title = "No monitoring sites selected",
      "Please select monitoring sites from which you would like to import data (filter sites by HUC or by distance from origin, then click 'Summarize Selected Data' button)."
    ))
    }
  # shiny::req(!is.null(rv$downloadStations), length(rv$downloadStations$MonitoringLocationIdentifier) > 0)
  shiny::req(!is.null(rv$downloadStations), !is.null(rv$sitesPointsSub), !is.null(rv$wqp_url_list))
  shiny::req(length(rv$downloadStations$MonitoringLocationIdentifier) > 0, length(rv$wqp_url_list) > 0)

    withProgress(message = "Processing data request...", value = 0, {
      ...
      # Rebuild wqp_url_list from downloadStations in batches of 20
subset_stations_list <- split(rv$downloadStations$MonitoringLocationIdentifier,
  ceiling(seq_along(rv$downloadStations$MonitoringLocationIdentifier)/20))

tmp_url_list <- lapply(subset_stations_list, function(s) {
  stationChoiceString <- paste0(utils::URLencode(s, reserved = TRUE), collapse = "&siteid=")
  paste0("https://www.waterqualitydata.us/data/Result/search?siteid=", stationChoiceString,
    "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=resultPhysChem&providers=NWIS&providers=STORET")
})

rv$wqp_url_list <- tmp_url_list
      

# Begin WQP data download
      all_data <- list()
for(i in 1:length(rv$wqp_url_list)) {
  setProgress(1/12, detail = paste0("Importing set #", i, " of ", length(rv$wqp_url_list), " (up to 20 sites imported per set)"))
  
  wqp_url <- rv$wqp_url_list[[i]]
  
  tmp_wqp <- tempfile(fileext = ".csv")
tryCatch(
  download.file(wqp_url, tmp_wqp, mode = "w", quiet = TRUE),
  error = function(e) {
    FuncDataRetrievalError(wqp_url, "water quality data", "Water Quality Portal",
      "If an error results, email WQP support (wqx@epa.gov) for assistance.")
    shiny::req(FALSE)
  }
)
all_data[[i]] <- read.csv(tmp_wqp) %>%
  dplyr::filter(CharacteristicName %in% unique(rv$downloadDataSummary$CharacteristicName))
unlink(tmp_wqp)
Sys.sleep(0.5) # pause between data pulls
}
      
      tempDat <- do.call("rbind.data.frame", all_data) 
      rm(all_data)
     
      # Filter tempDat to qualifying site-char combinations only
      tempDat <- tempDat %>%
  dplyr::inner_join(dplyr::select(rv$downloadDataSummary, MonitoringLocationIdentifier, CharacteristicName) %>% dplyr::distinct(),
                    by = c("MonitoringLocationIdentifier", "CharacteristicName"))
...
# Format WQP data ----
 ...
 cat("line2918")
  ...
  tempDat2 <- FuncBreakDates(tempDat) %>%
    dplyr::mutate(ResultSampleFractionText_Resolved = switch(any(is.na(ResultSampleFractionText), ResultSampleFractionText %in% c("", " ", "None", "Total"))+1, ResultSampleFractionText, "Total")) %>% # <<<<<< TREATING NONE SAME AS TOTAL
    dplyr::mutate(CharacteristicSet = paste(OrganizationIdentifier, CharacteristicName, ResultSampleFractionText_Resolved, ResultMeasure.MeasureUnitCode, ResultAnalyticalMethod.MethodIdentifier, sep = "_")) %>% # this is the column for identifying comparable records (also optionally consider OrganizationIdentifier and LaboratoryName)
 ...
 cat("line3327")
  ...
  convert_factors <- c("OrganizationIdentifier", "OrganizationFormalName", "ActivityTypeCode", "ActivityStartTime.TimeZoneCode", "MonitoringLocationIdentifier", "HydrologicCondition", "HydrologicEvent", "ResultDetectionConditionText", "CharacteristicName", "ResultSampleFractionText", "ResultSampleFractionText_Resolved", "ResultStatusIdentifier", "ResultValueTypeName", "DetectionQuantitationLimitTypeName", "CharacteristicSet")
  ...
  rv$dat <- tempDat2 %>%
    dplyr::ungroup() %>%
    dplyr::mutate(CharSampleType = paste0(CharacteristicName, " (", ifelse(is.na(ResultSampleFractionText_Resolved), ResultMeasure.MeasureUnitCode, paste(ResultSampleFractionText_Resolved, ResultMeasure.MeasureUnitCode, sep = "; ")), ")")) %>% # add CharSampleType column--this includes characteristic name, sample fraction_resolved, and measurement unit
...
cat("Line3348")
# remove HTML-escaped ampersands # <<<<< NEEDED FOR OLDER R VERSIONS
orgURL <- gsub("&amp;", "&", orgURL, fixed = TRUE)

tmp_org <- tempfile(fileext = ".csv")
tryCatch(
  # download.file(orgURL, tmp_org, mode = "w", quiet = TRUE),
  curl::curl_download(orgURL, tmp_org), # <<<<< NEEDED FOR OLDER R VERSIONS
  error = function(e) {
    FuncDataRetrievalError(orgURL, "organization information", "Water Quality Portal")
    shiny::req(FALSE)
  }
)
rv$orgs <- readr::read_csv(tmp_org, trim_ws = TRUE, show_col_types = FALSE) %>%
  dplyr::select(OrganizationFormalName, Description = OrganizationDescriptionText,
                `Email/Web` = ElectronicAddress, Phone = Telephonic) %>%
  dplyr::mutate(Selected = TRUE) %>%
  dplyr::arrange(OrganizationFormalName) %>%
  as.data.frame()
unlink(tmp_org)
...
cat("line3368")
 ...
 cat("line3376")
  ...
  # remove HTML-escaped ampersands # <<<<< NEEDED FOR OLDER R VERSIONS
gagesDatURL <- gsub("&amp;", "&", gagesDatURL, fixed = TRUE)
  
tmp_gages <- tempfile(fileext = ".txt")
tryCatch(
  # download.file(gagesDatURL, tmp_gages, mode = "w", quiet = TRUE),
  curl::curl_download(gagesDatURL, tmp_gages), # <<<<< NEEDED FOR OLDER R VERSIONS
  error = function(e) {
    rv$gagesDat <- NULL
    showModal(modalDialog(
    ...
    shiny::req(FALSE)
      ...
      gagesDat <- tryCatch(read.delim(tmp_gages, comment.char = '#'), error=function(e) print("Error retrieving USGS streamflow data")) # These daily data are  'Discharge, cubic feet per second (Mean)'. QC: 'A' means approved for publication, processing and review completed; 'e' means value has been estimated
...
cat("line3506")
 ...
 cat("line3503")
    ...
    showModal(modalDialog(
      title = "Done",
      HTML(paste0("<strong>Data import SUCCESSFUL.</strong> OPTIONAL: To download the formatted data file (.RDS) to your computer, press the <strong>`Save RDS to Computer` </strong> button in the left side panel. You may now navigate to the other dashboard pages. ")) 
    ))
  # })
}, ignoreInit = TRUE) # end of observeEvent
```
...
cat("line3776")
updateRadioButtons(session, "sel_ShowThresholds", selected = "showthresh_file") # on initial load, automatically change ShowThresholds setting to use the thresholds file
cat("line3778")
    
    showModal(modalDialog(
      title = "Done",
      "Threshold limits file has been imported. Click the green 'REFRESH PLOT DATA' button to add thresholds to plots."
    ))
  }
}, ignoreInit = TRUE) # end of observeEvent
...
observe({
  ...
  if(!is.null(rv_param_temp$threshList)) {
    ...
    which_rows <- which(tolower(rv_param_temp$subDat$CharSampleType) == tolower(t)) # Don't require case to match
  ...
  rv_param_temp$popupSiteSummary <- rv_param_temp$subDat %>% 
    ...
    dplyr::summarise(
  Sample_Size = n(),
  Year_Range = ifelse(min(Yr, na.rm = TRUE) == max(Yr, na.rm = TRUE),
    ...
    q05 = quantile(ResultMeasureValue_replaced, 0.05, na.rm = TRUE),
  Median_Value_Numeric = median(ResultMeasureValue_replaced, na.rm = TRUE),
  q95 = quantile(ResultMeasureValue_replaced, 0.95, na.rm = TRUE),
  Highest_Value_Numeric = max(ResultMeasureValue_replaced, na.rm = TRUE),  
    Lowest_Value = {
      min_val <- min(ResultMeasureValue_replaced, na.rm = TRUE)
      conditions <- na.omit(unique(ResultDetectionConditionText[ResultMeasureValue_replaced == min_val]))
      conditions <- conditions[conditions != ""]
      ifelse(length(conditions) > 0,
             paste0(min_val, " (", paste(conditions, collapse = "/"), ")"),
             as.character(min_val))
      },
    Highest_Value = {
      max_val <- max(ResultMeasureValue_replaced, na.rm = TRUE)
      conditions <- na.omit(unique(ResultDetectionConditionText[ResultMeasureValue_replaced == max_val]))
      conditions <- conditions[conditions != ""]
      ifelse(length(conditions) > 0, 
             paste0(max_val, " (", paste(conditions, collapse = "/"), ")"),
             as.character(max_val))
      },
  Most_Recent_Value = dplyr::first(ResultMeasureValue_replaced, order_by = desc(ActivityStartDate)),
  Most_Recent_Sample_Date = dplyr::first(ActivityStartDate, order_by = desc(ActivityStartDate)),
  Recent_Values = paste(head(ResultMeasureValue_replaced[order(desc(ActivityStartDate))], 5), collapse = ","),
  Recent_Dates = paste(format(head(ActivityStartDate[order(desc(ActivityStartDate))], 5), "%Y-%m-%d"), collapse = ","),
  Recent_Detection_Conditions = paste(head(ResultDetectionConditionText[order(desc(ActivityStartDate))], 5), collapse = ","),
    .groups = 'drop'
  ) %>%
    dplyr::rename(
...
observeEvent(eventExpr = input$button_getCategFile, { 
  ...
  if(length(rv_param_temp$subDatTypes$CharSampleType[rv_param_temp$subDatTypes$DataType == "categorical"])==0) { # can't print validation message within observeEvent, so use modalDialogs
    rv_param_temp$categLevelsPlot <- NULL
    showModal(modalDialog(
      title = "No categorical characteristics",
      "Could not find any categorical characteristics among the filtered water quality data."
    ))
    }
...
observeEvent(input$param_map_right_click, {
  shiny::req(!is.null(rv_param_temp$popupSiteSummary), !is.null(rv_param_temp$subDat))
  
  site <- input$param_map_right_click$MonitoringLocationIdentifier
  summary_df <- isolate(rv_param_temp$popupSiteSummary) %>% dplyr::filter(Site == site) 
...
observeEvent(eventExpr = input$button_refreshPlotData, { 
  ...
  rv_param_temp$sel_quantile_group_by <- isolate(input$sel_QuantileGroupBy)
...
rv_param_temp$sel_show_thresholds <- isolate(input$sel_ShowThresholds) # save the input setting so plots don't change unless the plot data has been refreshed
  ...
  if(rv_param_temp$sel_summarize_by == "by_station") {
    ...
    sel_chars <- isolate(input$sel_ShowStationCharTypes)
    rv_param_temp$showCharTypes <- if(!is.null(sel_chars) && length(sel_chars) > 0) {
      sel_chars
    } else {
      sort(intersect(
        unique(rv_param_temp$subDat$CharSampleType[rv_param_temp$subDat$MonitoringLocationIdentifier == isolate(input$sel_ShowOneStation)]),
        rv_param_temp$subDatTypes$CharSampleType[rv_param_temp$subDatTypes$DataType == "numeric"]
      ))
    }
  }
...
# Format plot data for time series plot tab
  if(any(is.null(rv_param_temp$subDat),  is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharTypes), is.null(rv_param_temp$sel_summarize_by))) {
    ...
    rv_param_temp$plotDat <- NULL
      ...
      rv_param_temp$plotDat <- FuncPlotDat()
  }
})
```
...
map_point_size <- switch(is.null(input$sel_MapPointSize)+1, input$sel_MapPointSize, 0)
  ...
  iconCWA = makeAwesomeIcon(icon= "fa-building", library = "fa", markerColor = "lightgray")
    ...
    addTiles(options = tileOptions(noWrap = TRUE), group = "Street") %>%
    addProviderTiles("Esri.WorldImagery", options = providerTileOptions(noWrap = TRUE), group="Satellite") %>%
    addPolygons(data = rv$unitPoly, group = "NPS unit", color = "black", fillColor = "yellow", fillOpacity = opacity_unselected, weight = 1, options = pathOptions(pane = "pane_param_unitPoly")) %>% # park unit outline
    addPolygons(data = rv[[rv$importSettings$sel_HUClevel]], group = paste0(rv$importSettings$sel_HUClevel, " boundaries"), color = "blue", fillColor = "lightblue", fillOpacity = opacity_unselected, weight = 0.7, options = pathOptions(pane = "pane_param_HUC")) %>% # HUC polygons
    addScaleBar() %>%
...
print("DONE WITH STANDARD MAP")
  ...
  show_layers <- data.frame(layer = c(paste0(rv$importSettings$sel_HUClevel, " boundaries"), "Filtered monitoring sites", "Selected monitoring sites", "Imported stream lines", "Discharge facilities", "Selected gage stations", "Selected weather stations", "Site labels", "Map legend"), show = c(TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE, TRUE, TRUE)) # dataframe to track what layers to show
  print("line1796")
...
cat("line4438")
    ...
    pie_median %<>%
      ...
      if(input$sel_MapByYr == TRUE) {
        param_map %<>%
          ...
          param_map %<>%
            addControl(tags$div(tag.map.title, HTML(input$sel_ShowCharType, " across all surveys")), position = "topleft", className="map-title")
      }
    }
    ...
    ## THRESHOLDS
      ...
      shiny::validate(need(sum(!is.na(rv_param_temp$plotDat$ThreshCateg)) > 0, message = "Use sidebar options to define threshold limits to display, then click 'REFRESH PLOT DATA'"))
    ...
    if(sel_param_map_show == "perc") {
      shiny::validate(need(sum(!is.na(rv_param_temp$plotDat$PercCateg)) > 0, message = "Use sidebar options to define percentile limits to display, then click 'REFRESH PLOT DATA'"))
...
param_map %<>%
  ...
  addLegend(title = "Organization", 
          pal = rv_param_temp$palOrg, 
          values = sort(unique(avail_sites_data$OrganizationFormalName)), 
          group = "Map legend", 
          position = "bottomleft") %>%
  addLayersControl(
  ...
  zoom = ifelse(is.null(isolate(input$param_map_zoom)) || isTRUE(isolate(input$param_map_zoom) == 1) || isTRUE(isolate(input$param_map_zoom) == ""),
    min(RgoogleMaps::MaxZoom(
      lonrange = c(st_bbox(rv$sitesPointsSub)$xmin, st_bbox(rv$sitesPointsSub)$xmax),
      latrange = c(st_bbox(rv$sitesPointsSub)$ymin, st_bbox(rv$sitesPointsSub)$ymax))),
    isolate(input$param_map_zoom))) %>%
  ...
  this.on('contextmenu', function(e) {
    ...
    if (this._layers) {
      Object.values(this._layers).forEach(function(layer) {
        if (layer.options && 
            (layer.options.group === 'Filtered monitoring sites' || 
             layer.options.group === 'Selected monitoring sites') && 
...
# Layer persistence - runs after all hideGroup calls so it has final say
if(!is.null(isolate(rv_param_temp$saved_param_map_groups))) {
  hidden_groups <- setdiff(show_layers$layer[show_layers$show == TRUE],
                           isolate(rv_param_temp$saved_param_map_groups))
  for(g in hidden_groups) {
    param_map %<>% leaflet::hideGroup(g)
  }
  shown_groups <- intersect(c("Selected gage stations", "Selected weather stations", "Discharge facilities"),
                            isolate(rv_param_temp$saved_param_map_groups))
  for(g in shown_groups) {
    param_map %<>% leaflet::showGroup(g)
  }
}
...
tags$style("#param_map {height: calc(100vh - 280px) !important;}") # expand map to fill screen

observeEvent(rv_param_temp$loadMap, {
  rv_param_temp$saved_param_map_groups <- NULL
})

observeEvent(input$param_map_groups, {
  shiny::req(!is.null(input$param_map_groups), isTRUE(rv_param_temp$loadMap == TRUE))
  rv_param_temp$saved_param_map_groups <- input$param_map_groups
}, ignoreInit = TRUE)
```
...
shiny::validate(need(length(na.omit(rv_param_temp$plotDat$ThreshCateg)) > 0, message = "Use sidebar options to define threshold limits for the selected characteristic(s), then click 'REFRESH PLOT DATA'"))

  threshPieDat <- FuncPieDat(limit_type = "ThreshCateg")

  shiny::validate(
    need(!is.null(threshPieDat), message = "Use sidebar options to define threshold limits for the selected characteristic(s), then click 'REFRESH PLOT DATA'"))
...
```{r}
htmltools::img(src="Images/secchi_transparent.png", width = "7%", style="position:absolute; top:1px; right:25px;")
```
<br>

#### <font size="3">**Quick-Start Instructions**</font>

* <font size="2">**Proceed through the dashboard pages from left to right ('Get WQP Data' --> 'Explore Data').** On each page, selections in the left-side panel require clicking the green refresh button to execute (e.g., 'refresh plots' or 'update data'), while options located above tables/figures update automatically. </font>

* <font size="2">To begin, import water quality data from the Water Quality Portal (or load an existing .RDS file) by following the red-font instructions on the **'Get WQP Data' page.** *If you import data from the Water Quality Portal, you will have the option to save the formatted data as an .RDS file on your computer. Since importing from WQP can be slow, saving allows you to quickly reload your data in future sessions instead of re-importing (only re-import if you need more recent data that has been added to WQP since your last import).*</font>

* <font size="2">Then (optionally) proceed to the **'Filter WQP Data'** page to examine sample sizes and customize which subset of data to analyze on the 'Explore Data' page. You can skip this step and go directly to 'Explore Data,' which will use default filter criteria to display a subset of your imported data.</font>

* <font size="2">The **'Explore Data'** page presents maps, time series plots, threshold exceedence plots (if you upload a threshold limits file), and other summaries of the filtered WQP data.</font>

* <font size="2">The **'Raw Data'** page displays the filtered Water Quality Portal data. This page populates as soon as you import or load water quality data. You can search for specific records using the search/filter fields.</font>

<br>

#### <font size="3">**About This Dashboard**</font>

<font size="2">This dashboard is an interactive tool for you to explore water quality trends in and around national park units. Use it to:</font>

* <font size="2">See what water monitoring sites, weather stations, and EPA-regulated facilities occur in and near our parks</font>

* <font size="2">Examine scatterplots, boxplots, and line graphs of water quality and precipitation data</font>

* <font size="2">Identify unusually high or low water quality measurements and examine the underlying data records</font>

* <font size="2">Use a map time slider to see how water quality patterns change over space and time</font>

<br>

#### <font size="3">**About The Data**</font>

<font size="2">Click on the links below to learn more about each data source and the data used in this dashboard:</font>

* <font size="2">[Administrative Boundaries of National Park System Units](https://irma.nps.gov/DataStore/Reference/Profile/2224545?lnv=True) (NPS) </font>

* <font size="2">[National Watershed Boundary Dataset](https://www.usgs.gov/national-hydrography/watershed-boundary-dataset) (USGS), for watershed (HUC10) and subwatershed (HUC12) boundaries</font>

* <font size="2">[Water Quality Portal](https://www.waterqualitydata.us/) (a cooperative service sponsored by the USGS, EPA, and NWQMC), for water quality data</font> 

* <font size="2">[National Climatic Data Center](https://www.ncei.noaa.gov/cdo-web/) (NOAA), for precipitation data</font>

* <font size="2">[Enforcement and Compliance History Online](https://echo.epa.gov/) (EPA), for compliance information for facilities regulated under the Clean Water Act</font>
...
wellPanel(
  id = "new_data_P0",
  p(strong("1 (of 3). Choose data source, then click 'BEGIN DATA SEARCH' button")),
  ...
  conditionalPanel(
    condition = "input.sel_GetData == 'new_data'",
    selectInput("sel_UnitCode", label = h6("Enter a park name or 4-digit park code (e.g., BITH): "), 
                choices = c("Choose a park" = "", unique_parks_df$unit_name_combined), 
                selected = c("Choose a park" = ""),
                # selected = switch(is.null(input$sel_UnitCode)+1, input$sel_UnitCode, ""),
                selectize = TRUE)
    ),
  
  actionButton("button_beginSearch", "BEGIN DATA SEARCH", width = "100%", style="color:black; background-color: lightgreen; border: 1px solid black; display:inline-block; margin:5px")
...
shinyjs::hidden( # initially hide this well panel
  wellPanel(
    id = "new_data_P1",
    p(strong("2 (of 3). Select sites and characteristics, then click 'SUMMARIZE SELECTED DATA' button")),     
    p("Click on a map point for more information about the station and data. Blue hyperlinks will open web pages with additional information."),
    
    tags$div(renderText({
  shiny::req(all(!is.null(input$sel_AddQuantile), input$sel_AddQuantile == TRUE))
  
  paste0("WARNING!! Large data imports may trigger time-out errors from web services. Use the filter criteria below to limit import requests to only the necessary data.")
    ...
    radioButtons("sel_SelectMethod", label = h6("Select WQP monitoring sites based on: "),
                 choiceNames = c("Park (include only data collected within the park unit)", "HUCs (click on map to select one or more HUCs)", "Distance from origin site (enter query filters below)"),
                 choiceValues = c("methodPark", "methodHuc", "methodDist"),
                 selected = "methodPark"),

conditionalPanel(
  condition = "input.sel_SelectMethod == 'methodHuc'",
  div(style = "margin-top: -8px; margin-bottom: 5px;",
      actionButton("button_selectAllHucs", "Select All HUCs", 
                   style = "color:black; display:inline-block; border:1px; padding:2px 6px; width:48%; margin-right:2%; font-size:12px;"),
      actionButton("button_deselectAllHucs", "Deselect All HUCs", 
                   style = "color:black; display:inline-block; border:1px; padding:2px 6px; width:48%; font-size:12px;")
  )
),
    ...
    renderUI({
      ...
      checkboxGroupInput("sel_SiteType",
             ...
             selected = unique(rv$sitesPointsSub$MonitoringLocationTypeName)[!grepl("Well", unique(rv$sitesPointsSub$MonitoringLocationTypeName))])
    }),
    ...
    actionButton("button_summarizeData", "SUMMARIZE SELECTED DATA", width = "100%", style="color:black; background-color: lightgreen; border: 1px solid black; display:inline-block; margin:5px")
    ) # end subset-stations panel
  ) # end hide for subset-stations panel

# UI panel for data availability filters and download ----
shinyjs::hidden(
  wellPanel(
    id = "new_data_P2",
    p(strong("3 (of 3). Filter data to download, select gage stations, then click 'DOWNLOAD DATA' button")),

    numericInput("sel_ActiveWithinYrs",
             label = h6("Exclude site-characteristic combinations with no data in the past [ENTER AN INTEGER] years:"),
             value = 30,
             min = 1),
    
    numericInput("sel_MinResults",
             label = h6("Exclude site-characteristic combinations with fewer than [ENTER AN INTEGER] total data records:"),
             value = 0,
             min = 0),
    
    actionButton("button_updateSelectedData", "UPDATE SELECTED DATA", width = "100%",
             style="color:black; background-color: lightyellow; border: 1px solid black; display:inline-block; margin:5px"),
    ...
    renderUI({
      ...
      checkboxGroupInput("sel_Gages",
                         label = h6("(OPTIONAL) Import data from these USGS gage stations: "),
                         choices = sort(unique(rv$gageStations$GageName)))
      }),

    actionButton("button_downloadData", "DOWNLOAD DATA", width = "100%", style="color:black; background-color: lightgreen; display:inline-block; border: 1px solid black; margin:5px"),

    br(),

    renderUI({
    shiny::req(rv_param_temp$newImport == TRUE, !is.null(rv))
...
-------------------------------------
<center>*NOTE:  The Water Quality Portal and other web services accessed by this dashboard are undergoing changes. To help us keep up with changes in web services, please report data import issues to the dashboard GitHub page: https://github.com/nationalparkservice/WQdash_public*</center>
...
conditionalPanel(
  ...
  htmltools::tags$div(
    style = "display: flex; align-items: center; justify-content: center; gap: 15px; margin: 5px 0;",
    htmltools::tags$button(
      "🔄 Refresh Map (if it goes wonky)",
      onclick = "refreshMap()",
      style = "padding: 4px 10px; background: #5a9fd4; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px;"
    ),
    htmltools::tags$span(
      "Use mouse scroll wheel or trackpad to zoom in/out",
...
output$map <- renderLeaflet({
 ...
 if(rv_param_temp$loadMap == FALSE) {map <- leaflet()} # force visible map to blank--resets input$map
  shiny::validate(
  ...
  if(input$sel_GetData == "new_data"){
      ...
      rv$importSettings$sel_SelectMethod = input$sel_SelectMethod
      rv$importSettings$sel_HUClevel = input$sel_HUClevel
      # isolate(rv$importSettings$sel_Organization <- switch(is.null(rv$importSettings$sel_Organization)+1, rv$importSettings$sel_Organization, input$sel_Organization))
      # isolate(rv$importSettings$sel_SiteType <- switch(is.null(rv$importSettings$sel_SiteType)+1, rv$importSettings$sel_SiteType, input$sel_SiteType))
      } # make sure the map will show even if WQP data have not yet been imported. Isolate ones that I don't want it to cause re-render.
    # rv$importSettings <- list(sel_SelectMethod = input$sel_SelectMethod, sel_HUClevel = input$sel_HUClevel, sel_Organization = input$sel_Organization, sel_SiteType = input$sel_SiteType)
    # }
 
  # if(any(is.null(rv$importSettings$sel_SelectMethod), is.null(rv$importSettings$sel_HUClevel), is.null(isolate(rv$importSettings$sel_Organization)), is.null(isolate(rv$importSettings$sel_SiteType)), is.null(rv$unitPoly), is.null(rv$HUC10), is.null(rv$sitesPointsSub), rv$parkID=="")) {
  if(any(is.null(rv$importSettings$sel_SelectMethod), is.null(rv$importSettings$sel_HUClevel), is.null(rv$importSettings$sel_Organization), isolate(is.null(rv$importSettings$sel_SiteType)), is.null(rv$unitPoly), is.null(rv$HUC10), is.null(rv$sitesPointsSub), is.null(rv$parkID), rv$parkID=="", isolate(!rv$importSettings$sel_Organization %in% rv$sitesPointsSub$OrganizationFormalName))) {
      ...
      map <- leaflet()
        ...
        map %<>%
          addLegend(title = "CIRCLES are monitoring stations (colored by organization)", pal = rv_param_temp$palOrg, values = sort(isolate(rv$importSettings$sel_Organization)), group = "Map legend", position = "bottomleft") %>%
          addLayersControl(
            baseGroups = c("Street", "Satellite"),
            overlayGroups = na.omit(c("NPS unit", rv$importSettings$sel_HUClevel, "Available monitoring sites", ifelse(!is.null(rv$downloadStations), "Selected monitoring sites", NA), ifelse(!is.null(rv$facilitiesPointsSub), "Discharge facilities (bldg icon)", NA), ifelse(!is.null(rv$gageStations), "Gage stations (water drop icon)", NA), ifelse(!is.null(rv$meteoStations), "Weather stations (cloud icon)", NA), "Map legend")),
            options = layersControlOptions(collapsed = TRUE)) %>%
          setView(lng = switch(any(is.null(isolate(input$map_center$lng)), isolate(input$map_center$lng) == 0, isolate(input$map_center$lng) > st_bbox(rv[["HUC10"]])$xmax, isolate(input$map_center$lng) < st_bbox(rv[["HUC10"]])$xmin)+1, isolate(input$map_center$lng), mean(c(st_bbox(rv[["HUC10"]])$xmin, st_bbox(rv[["HUC10"]])$xmax))), lat = switch(any(is.null(isolate(input$map_center$lat)), isolate(input$map_center$lat) == 0, isolate(input$map_center$lat) > st_bbox(rv[["HUC10"]])$ymax, isolate(input$map_center$lat) < st_bbox(rv[["HUC10"]])$ymin)+1, isolate(input$map_center$lat), mean(c(st_bbox(rv[["HUC10"]])$ymin, st_bbox(rv[["HUC10"]])$ymax))),
                  zoom = ifelse(is.null(isolate(input$map_zoom)) || isTRUE(isolate(input$map_zoom) == 1) || isTRUE(isolate(input$map_zoom) == ""),
  min(RgoogleMaps::MaxZoom(
    lonrange = c(st_bbox(rv[["HUC10"]])$xmin, st_bbox(rv[["HUC10"]])$xmax),
    latrange = c(st_bbox(rv[["HUC10"]])$ymin, st_bbox(rv[["HUC10"]])$ymax))),
  isolate(input$map_zoom))
          )
        
        if(!is.null(isolate(rv_param_temp$saved_map_groups))) {
  all_overlay_groups <- na.omit(c("NPS unit", rv$importSettings$sel_HUClevel,
    "Available monitoring sites", "Selected monitoring sites",
    "Discharge facilities (bldg icon)", "Gage stations (water drop icon)",
    "Weather stations (cloud icon)", "Map legend"))
  hidden_groups <- setdiff(all_overlay_groups, isolate(rv_param_temp$saved_map_groups))
  if(any(c("HUC10", "HUC12") %in% isolate(rv_param_temp$saved_map_groups))) {
    hidden_groups <- setdiff(hidden_groups, rv$importSettings$sel_HUClevel)
  }
  # Don't hide Selected monitoring sites if it wasn't available when state was saved
  if(!is.null(rv$downloadStations) && !"Selected monitoring sites" %in% isolate(rv_param_temp$saved_map_groups)) {
    hidden_groups <- setdiff(hidden_groups, "Selected monitoring sites")
  }
  for(g in hidden_groups) {
    map %<>% leaflet::hideGroup(g)
  }
}
...
## Map updates by proxy--must ensure map exists before any proxy updates, or will get "gray box"
...
observeEvent(input$map_shape_click, {
  ...
  shiny::req(input$sel_GetData=="new_data")
    click <- input$map_shape_click
      ...
      leafletProxy("map") %>% addPolygons(data = rv[[rv$importSettings$sel_HUClevel]][rv[[rv$importSettings$sel_HUClevel]]$HUC==click$id,], group = rv$importSettings$sel_HUClevel, color = "blue", fillColor = "lightblue", fillOpacity = opacity_unselected, weight = 0.5, layerId = ~HUC, options = pathOptions(pane = "pane_HUC")) # and change the polygon to unselected color
      } else { # if the click is in an unselected HUC
        rv$selectedHucs <- c(rv$selectedHucs, click$id) # append the ID of clicked HUC polygon
        leafletProxy("map") %>% addPolygons(data = rv[[rv$importSettings$sel_HUClevel]][rv[[rv$importSettings$sel_HUClevel]]$HUC==click$id,], group = rv$importSettings$sel_HUClevel, color = "blue", fillColor = "lightblue", fillOpacity = opacity_selected, opacity = 1, weight = 1.5, layerId = ~HUC, options = pathOptions(pane = "pane_HUC"))
      } # and change the polygon to selected color
  }, ignoreInit = TRUE) # end of observeEvent

# Select all HUCs
observeEvent(input$button_selectAllHucs, {
  req(input$map_bounds)
  shiny::req(!is.null(rv[[input$sel_HUClevel]]), input$sel_SelectMethod == "methodHuc")
  rv$selectedHucs <- rv[[input$sel_HUClevel]]$HUC
  leafletProxy("map") %>%
    addPolygons(data = rv[[input$sel_HUClevel]], group = input$sel_HUClevel,
                color = "blue", fillColor = "lightblue", fillOpacity = opacity_selected,
                opacity = 1, weight = 1.5, layerId = ~HUC,
                options = pathOptions(pane = "pane_HUC"))
}, ignoreInit = TRUE)

# Deselect all HUCs
observeEvent(input$button_deselectAllHucs, {
  req(input$map_bounds)
  shiny::req(!is.null(rv[[input$sel_HUClevel]]), input$sel_SelectMethod == "methodHuc")
  rv$selectedHucs <- NULL
  leafletProxy("map") %>%
    addPolygons(data = rv[[input$sel_HUClevel]], group = input$sel_HUClevel,
                color = "blue", fillColor = "lightblue", fillOpacity = opacity_unselected,
                weight = 0.5, layerId = ~HUC,
                options = pathOptions(pane = "pane_HUC"))
}, ignoreInit = TRUE)

# User selects sites in park
observeEvent(input$sel_SelectMethod, {
  req(input$map_bounds)
  shiny::req(isTRUE(rv_param_temp$loadMap == TRUE), !is.null(input$sel_SelectMethod))
  
  if(input$sel_SelectMethod != "methodDist") {
    leafletProxy("map") %>% clearGroup("Originsite")
  }
  
  if(input$sel_SelectMethod == "methodPark") {
    leafletProxy("map") %>%
      addPolygons(data = rv$unitPoly, group = "NPS unit", 
                  color = "black", fillColor = "yellow", 
                  fillOpacity = opacity_selected, weight = 3,
                  options = pathOptions(pane = "pane_unitPoly"))
  } else {
    leafletProxy("map") %>%
      addPolygons(data = rv$unitPoly, group = "NPS unit",
                  color = "black", fillColor = "yellow",
                  fillOpacity = opacity_unselected, weight = 1,
                  options = pathOptions(pane = "pane_unitPoly")) 
  }
}, ignoreInit = TRUE)

# User changes filter for organization or site type
observeEvent(list(input$sel_Organization, input$sel_SiteType), {
  req(input$map_bounds)
  shiny::req(!is.null(rv$sitesPointsSub), !is.null(rv_param_temp$palOrg))
  
  
  org_filter <- if(is.null(input$sel_Organization)) character(0) else input$sel_Organization
  
  passing_sites <- rv$sitesPointsSub %>%
    dplyr::filter(OrganizationFormalName %in% org_filter & 
                  MonitoringLocationTypeName %in% input$sel_SiteType)
  
  failing_sites <- rv$sitesPointsSub %>%
    dplyr::filter(!MonitoringLocationIdentifier %in% passing_sites$MonitoringLocationIdentifier)
  
  leafletProxy("map") %>% clearGroup("Available monitoring sites")
  
  if(nrow(passing_sites) > 0) {
    leafletProxy("map") %>%
      addCircleMarkers(data = passing_sites, group = "Available monitoring sites",
                       label = ~SITELAB, popup = ~SITEPOP, radius = 4,
                       fillColor = rv_param_temp$palOrg(passing_sites$OrganizationFormalName),
                       fillOpacity = 0.8, stroke = TRUE, weight = 1,
                       color = rv_param_temp$palOrg(passing_sites$OrganizationFormalName), opacity = 0.8,
                       options = pathOptions(pane = "pane_availableStations"))
  }
  
  if(nrow(failing_sites) > 0) {
    leafletProxy("map") %>%
      addCircleMarkers(data = failing_sites, group = "Available monitoring sites",
                       label = ~SITELAB, popup = ~SITEPOP, radius = 4,
                       fillColor = rv_param_temp$palOrg(failing_sites$OrganizationFormalName),
                       fillOpacity = 0.1, stroke = TRUE, weight = 1,
                       color = rv_param_temp$palOrg(failing_sites$OrganizationFormalName), opacity = 0.1,
                       options = pathOptions(pane = "pane_availableStations"))
  }
  
}, ignoreInit = TRUE)

# User enters an origin site
observeEvent(input$sel_OriginStationID, {
  req(input$map_bounds)
  shiny::req(isTRUE(rv_param_temp$loadMap == TRUE),
             !is.null(input$sel_OriginStationID),
             isTRUE(input$sel_SelectMethod == "methodDist"),
             input$sel_OriginStationID != "",
             input$sel_OriginStationID %in% rv$sitesPointsSub$MonitoringLocationIdentifier)
  
  origin_site <- rv$sitesPointsSub[rv$sitesPointsSub$MonitoringLocationIdentifier == input$sel_OriginStationID, ]
  
  leafletProxy("map") %>%
    clearGroup("Origin site") %>%
    addCircleMarkers(data = origin_site, group = "Origin site",
                     label = ~SITELAB, popup = ~SITEPOP,
                     radius = 8, fillOpacity = 0, stroke = TRUE,
                     weight = 4, color = "red", opacity = 1,
                     options = pathOptions(pane = "pane_selectedOrigin"))
}, ignoreInit = TRUE)

## Preserve the user-selected layers to show
observeEvent(rv_param_temp$loadMap, {
  rv_param_temp$saved_map_groups <- NULL
})

observeEvent(input$map_groups, {
  req(input$map_bounds)
  shiny::req(!is.null(input$map_groups), isTRUE(rv_param_temp$loadMap == TRUE))
  rv_param_temp$saved_map_groups <- input$map_groups
}, ignoreInit = TRUE)
```

### Summary of Selected Characteristics

<center>*Summary of selected water quality data, grouped by water quality characteristic. Click column headings to sort. DOWNLOADING LARGE DATASETS CAN BE SLOW--for better performance, filter data to only what you need.*</center>
...
output$table_char_summary <- DT::renderDT({
  shiny::validate(
    need(!is.null(rv$downloadDataSummary), message = "No data selected. Follow prompts on the left to select data for import, then click <strong>'SUMMARIZE SELECTED DATA'</strong> button."))

  shiny::req(!is.null(rv$downloadDataSummary))

  DT::datatable(
    rv$downloadDataSummary %>% dplyr::group_by(CharacteristicType, CharacteristicName) %>% dplyr::summarize(`# of Data Records` = sum(ResultCount, na.rm = TRUE), .groups = "drop") %>% dplyr::rename(`Characteristic Type` = CharacteristicType, Characteristic = CharacteristicName),
    rownames = FALSE,
    filter = "top",
    selection = "none",
    class="compact stripe",
...
### Summary of Selected Site-Characteristics

<center>*Summary of selected water quality data, grouped by monitoring site and water quality characteristic. Click column headings to sort. DOWNLOADING LARGE DATASETS CAN BE SLOW--for better performance, filter data to only what you need.*</center>
...
output$table_import_summary <- DT::renderDT({
  ...
  DT::datatable(
    rv$downloadDataSummary %>%
      ...
      dplyr::summarize(`First Yr` = min(YearSummarized, na.rm = TRUE), `Most Recent Yr` = max(YearSummarized, na.rm = TRUE), `# of Data Records` = sum(ResultCount, na.rm = TRUE), .groups = "drop") %>%
...
### Summary of Downloaded Data, by Site

<center>*Summary of data downloaded from Water Quality Portal, for summary and visualization in this dashboard.*</center>
...
output$table_selected_orgs <- renderReactable({
  ...
  df_site <- rv$sitesPointsSub %>%
    dplyr::filter(MonitoringLocationIdentifier %in% rv$downloadStations$MonitoringLocationIdentifier & OrganizationFormalName %in% rv$importSettings$sel_Organization) %>%
...
wellPanel(
  ...
  wellPanel(
    ...
    p(strong("Filter data to summarize, then click 'REFRESH FILTER' button")),
  
  actionButton("button_refreshFilter", "REFRESH FILTER", style="border-color:black; background-color: lightgreen; width:100%; border: 1px solid black; margin:0px"), # filters data
...
#### <font size="1.25">**Sites and survey years by characteristic, for filtered data**</font>

* <font size="1.25"> Click on the triangle to the left of each characteristic to see summaries by site.</font>

* <font size="1.25"> The full list of characteristics imported are shown under 'Water quality characteristics to summarize', in the left sidebar. Only 'checked' characteristics are shown in this table.</font> 

* <font size="1.25"> The filtered data shown here can be visualized on the Explore Data page.</font>

* <font size="1.25"> Years and seasons are defined by the user as meteorological seasons (calendar year), USGS water year seasons (starts Oct. 1), or custom-defined seasons (see 'Define seasons as:' in left sidebar). These year and season definitions will be used on all subsequent pages of the dashboard.</font>
...
<center>*For each site, these heat maps show the number of samples collected per season (or month) each year for the selected water quality characteristic.*</center>
  ...
  fluidRow(
    ...
    column(3, style = "height: 100%;",
      ...
      actionButton("button_getCategFile", "Create Definitions File",
                   width = "100%",
                   style="color:black; background-color: lightgreen; display:inline-block; border: 1px solid black; margin:5px"),
      br(), br(),
      shinyjs::hidden(
        wellPanel(
...
br(),
          selectizeInput("sel_CategPalette",
                         label = h6("Select a color palette to apply, then click 'Add to Color Coding File'"),
          ...
          actionButton("button_updateCategFile", "Add to Color Coding File",
                       width = "100%",
                       style="font-size: 10pt; color:black; background-color: lightgreen; border: 1px solid black; display:inline-block; margin:5px"),
  ...
  mtext(input$sel_CategParam, at = 0.25, cex=1.25)
    ...
    div(style = "text-align: center;",
    em("The Double-click in any cell to edit contents. To delete a row, click it once to select and then choose 'Delete Selected Row(s).'")),
  br(),
      renderUI({
  FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = FALSE)
  if(any(!rv$importSettings$sel_Organization %in% rv$sitesPointsSub$OrganizationFormalName))
...
actionButton("button_refreshPlotData", "REFRESH PLOT DATA", style="border-color:black; background-color: lightgreen; width:100%; border: 1px solid black; margin:0px") # updates plots
...
shinyBS::bsCollapse(
  ...
  multiple = TRUE,
  open = list("Select data to plot", "Thresholds & Percentiles"),
  bsCollapsePanel(
    title = "Select data to plot", 
    renderUI({
      radioButtons("sel_SummarizeBy",
                   label = h6(" Summarize by:"),
                   choiceNames = list("many characteristics for ONE SITE", "ONE CHARACTERISTIC for many sites"),
                   choiceValues = list("by_station", "by_chartype"),
                   selected = "by_station")
      }),
...
style = "default"
  ...
  renderUI({
    ...
    radioButtons("sel_ShowThresholds",
                 label = h6("Show threshold limits on plot (shown as colored background grid)"),
  ...
  checkboxInput("sel_AddQuantile",
              label = h6("Add percentile lines to plot (shown as red horizontal lines)"),
...
explore_map_card <-
  bslib::card(
    ...
    full_screen = FALSE,
    card_header(
      HTML('<span style="color: red;">Select sites via the map or sidebar, then select characteristic(s) from the sidebar and click REFRESH PLOT DATA. From the map, RIGHT CLICK ON ANY SITE for quick-peek parameter summaries.</span><br><span style="color: gray; font-size: 90%;">Use mouse scroll wheel or trackpad to zoom in/out on map.</span>')
      ),
    card_body(
      class = "p-0",
...
explore_plots_card <-
  bslib::card(
    gap = 0,
    card_header(
      HTML('<span style="color: red;">Percentile limits (defaults are 5th and 95th percentiles) are shown as red horizontal lines. Threshold limits are shown in colored background grid (blue = good, yellow = caution, orange = poor). Use sidebar options to set or remove limits.</span>')
      ),
    card_body(
      class = "p-0",
      # div(style="display:flex; flex-direction:row; max-width:98%; justify-content:center; flex-wrap: wrap; align-items: center;",
...
div(
  ...
  if(isTRUE(rv_param_temp$sel_add_gage_precip == "streamflow") && !is.null(rv_param_temp$sel_gage_station)) { # if there is a selected gage station
    param_choices <- c(param_choices, "streamflow")
  }
  if(isTRUE(rv_param_temp$sel_add_gage_precip == "precipitation") && !is.null(rv_param_temp$sel_precip_station)) { # if there is a selected precip station
...
nav_panel(
  ...
  bslib::card(
    ...
    full_screen = TRUE,
    card_header(
      HTML('<span style="color: red;">WARNING!! Percentiles should NOT be used to determine if values are high or low. Percentiles only indicate a typical range of values AMONG THE DATA SELECTED.</span>')
      ),
...
- Default is at least 2 sampling activities within the past 6 years and a subset of site types and WQ characteristics. Try custom import instead.
...
* How to reorder/display categorical characteristics with colors?

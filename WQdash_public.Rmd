...
### NEXT TO DO
# > For dist from origin, can't use wells or other sources that don't link
# > TADA data cleaning
# > Save/show import/filter settings for .RDS
# > Change the right-click site pop-up
...
showModal(modalDialog(
  ...
  HTML("In the <strong>'2 (of 3). Filter sites to download'</strong> panel, choose the monitoring sites you want to download data from. You can:<ul>
    ...
    <li>Select specific watersheds.</li>
  </ul>
  You can also apply additional filters using the other options in the panel. When you're ready, click <strong>'Summarize Selected Data'</strong> to preview a summary of the data before downloading.")
))
...
showModal(modalDialog(
  ...
  HTML(paste0("<strong>You have summarized ", sum(rv$selectedDataSummary$ResultCount), " data records from ", length(unique(rv$selectedDataSummary$MonitoringLocationIdentifier)), " sites.</strong> Selected monitoring sites are shown as larger, white-outlined circles on the map.", 
              ...
              "<br><br>Review your selection in the 'Summary of Selected Characteristics' and 'Summary of Selected Site-Characteristics' tabs. <br><br>In the next panel, apply data availability filters, select gage stations (if available), then press the <strong>DOWNLOAD SELECTED DATA</strong> button.")) 
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  shiny::req(length(rv$selectedStations$MonitoringLocationIdentifier) > 0, length(rv$wqp_url_list) > 0)
...
rv$orgs <- readr::read_csv(tmp_org, show_col_types = FALSE) %>%
  ...
  div(style = "margin-top: -8px; margin-bottom: 5px;",
    ...
    renderUI({
      ...
      checkboxGroupInput("sel_SiteType",
             ...
             selected = setdiff(unique(rv$sitesPointsSub$MonitoringLocationTypeName), "Well"))
    }),
...
output$map <- renderLeaflet({
  ...
  if(!is.null(rv$selectedStations) && !"Selected monitoring sites" %in% isolate(rv_param_temp$saved_map_groups)) {

...
# > plot title gets cut off when only one plot, and it runs into plots when a lot of plots
# > BITH vertice error
...
# > Better layout of inputs on plots page
...
# > On plots page, add a table to show total threshold %/# for each site-char
...
observeEvent(eventExpr = input$saved_thresh_file, {
  ...
  tempThresh_df <- out_thresh_file %>%
    ...
    if(all(!is.null(rv_param_temp$subDat), !is.null(rv_param_temp$threshList))) {
      rv_param_temp$subDat <- FuncAddThreshCateg(rv_param_temp$subDat)
    }
...
unlink("Rplots.pdf")
tags$style("#out_ts_plots{height: calc(85vh - 350px) !important;overflow-x:scroll;overflow-y:scroll}")
...
unlink("Rplots.pdf")
tags$style("#out_season_plots{height: calc(85vh - 350px) !important;overflow-x:scroll;overflow-y:scroll}")

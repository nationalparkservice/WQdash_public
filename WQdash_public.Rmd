...
FuncParamTable <- function(df, threshold_df, MonitoringLocationIdentifier) {
  ...
  build_css_bar <- function(low, high, recent_values_str, recent_dates_str, n) {
    ...
    q05 <- quantile(all_values, 0.05, na.rm = TRUE)
    q95 <- quantile(all_values, 0.95, na.rm = TRUE)
    ...
    sprintf(
      '<div style="position: relative; height: 80px; background: linear-gradient(to right, #f0f4f8 0%%, #f0f4f8 5%%, #ffffff 5%%, #ffffff 95%%, #f0f4f8 95%%, #f0f4f8 100%%); border-radius: 8px; border: 1px solid #dee2e6; overflow: visible; margin-top: 12px;">
         ...
         <div style="position: absolute; top: -18px; left: 5%%; font-size: 10px; color: #495057; font-weight: 600; text-align: center; width: 90%%; white-space: nowrap;">MIDDLE 90%% OF DATA (%.1f - %.1f)</div>
      ...
      q05, q05, q95, q95, q05, q95, median_pos, med, dots_html
    )
  }
  ...
  rows <- lapply(1:nrow(df), function(i) {
    ...
    param_info <- sprintf('<div class="param-info"><div class="param-name">%s</div><div class="param-details"><div>N = %d samples</div><div>Years: %s</div><div>Range: %.1f - %.1f %s</div><div>Most recent: %.1f (%s)</div></div></div>', param, n, row$Year_Range, low, high, unit, val, row$Most_Recent_Sample_Date)
  ...
  shiny::req(length(rv$selectedStations$MonitoringLocationIdentifier) > 0, length(rv$wqp_url_list) > 0)
    ...
    withProgress(message = "Processing data request...", value = 0, {
      ...
      for(i in 1:length(rv$wqp_url_list)) {
        setProgress(1/12, detail = paste0("Importing set #", i, " (up to 20 sites imported per set)"))

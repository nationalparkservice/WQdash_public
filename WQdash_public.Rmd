...
### PICK UP FROM HERE
#> In pop-up, if no data for recent value/date, should drop but keep if just missing recent detection condition
#> fix data imports so they don't ping API twice (function 'grabit'?)
#> fix data imports so they report when data not imported

### QUESTION
# > For sample fraction, is it reasonable to treat blank and total and none as same?
#> CHAT one Ecoli not showing in plot options b/c some non-numeric entries... then how does the pop-up classify it for pie chart?
#> Similar categorical issue for CONG--several characteristics listed as categorical b/c of a few non-numeric entries. What to do about that?
#> Censored data--for pop-up and other things I'm setting them to lowest detectable... is that okay?
# > For E.coli, sometimes #/100mL and MPN/100mL are really the same thing--user would have to refer to the methods description to confirm. This is going to be the case with other "different" measurement units also.

### NEXT CHANGES:
#> Refresh plot needs progress bar
#> Initial map park unit should turn yellow if select by park unit
...
# > Update to work with hydrography revisions: https://waterdata.usgs.gov/blog/nldi-intro/
...
showModal(modalDialog(
  title = "Done",
  HTML("In the <strong>'3. Filter sites to download'</strong> panel, choose the monitoring sites you want to download data from. You can:<ul>
...
observeEvent(eventExpr = input$button_downloadData, { 
 ...
 #  setProgress(1/8, detail = "Importing data from Water Quality Portal...please be patient!")
 ...
 # saveRDS(rawDat, "TEMP_rawDat.RDS")
  
  # wqp_downloads <- list.files(
#     path = here::here(),
#     pattern = paste0("^tempWQPDat*"),
#   full.names = TRUE
# )

  # temp_dat_list <- lapply(wqp_downloads, FUN = function(x) read.csv(x))
...
* <font size="2">The **'Raw Data'** page displays the filtered Water Quality Portal data. You can search for specific records using the search/filter fields.</font>
...
shinyjs::hidden( # initially hide this well panel, show only if user wants to import data from WQP
  wellPanel(
    id = "new_data_P1",
    p(strong("2. Choose a park, then click 'SHOW MONITORING SITES' button")),
...
conditionalPanel(
  ...
  renderUI({
    ...
    sliderInput("sel_MapZoom", label = h5("Set map zoom:"), min = max(1, map_zoom - 6), max = 15, value = map_zoom, step = 0.5, ticks = FALSE, dragRange = FALSE, width = "100%")
    })
)
...
shinyjs::hidden( # initially hide this well panel
  wellPanel(
    id = "new_data_P2",
    p(strong("3. Filter sites to download, then click 'SUMMARIZE SELECTED SITES' button")),
...
shinyjs::hidden( # initially hide this well panel
  wellPanel(
    id = "new_data_P3",
    p(strong("4. Select stream gage data to import, then click 'DOWNLOAD SELECTED DATA' button")),

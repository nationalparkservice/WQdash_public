...
pkgs <- c("flexdashboard", "shiny", "knitr",  "here", "janitor", "mgcv", "plyr", "scales", "rtf", "rmarkdown", "shinyWidgets", "leaflet", "RgoogleMaps", "plotrix", "GGally", "plotly", "httr", "jsonlite", "tidyverse", "readr", "sp", "rgdal", "sf", "rgeos", "magrittr", "leaflet.extras", "lubridate", "fontawesome", "viridisLite", "purrr",
          "stringr",
...
FuncCalcAggStats <- function(dat_list, summarize_to = "week", min_perc_needed = 80, stat_vec) {
  # Calculate statistics from instantaneous data, for periods with at least 80% of data available. Keep as list because smaller object size.
  agg_stats_list <- pblapply(
    dat_list,
    FUN = function(x) {
      x %<>% dplyr::select(date = collect_date, value)
      min_date <- min(x$date, na.rm = TRUE)
      max_date <- max(x$date, na.rm = TRUE)
      pbmapply(
        FUN = function(stat_name, perc) {
          timeAverage(x, avg.time = summarize_to, data.thresh = min_perc_needed, statistic = stat_name, type = "default", start.date = min_date, end.date = max_date, vector.ws = FALSE, fill = FALSE) %>%
            dplyr::mutate(
              stat = stat_name,
              value = round(value, 2))
          },
      ...
      do.call("rbind", .) %>%
      dplyr::select(wk_start = date, stat, value) %>%
      tidyr::pivot_wider(names_from = stat, names_prefix = "weekly_", values_from = value)
    }
  )

    return(agg_stats_list)
}

# FuncCalcAggStats <- function(dat_list, summarize_to = "day", min_perc_needed = 80, stat_vec) { 
#   # Calculate statistics from instantaneous data, for periods with at least 80% of data available. Keep as list because smaller object size.
#   agg_stats_list <- pblapply(
#     dat_list,
#     FUN = function(x) {
...
#         ) %>%
#       do.call("rbind", .) %>%
#       dplyr::select(collect_date = date, stat, value) %>%
#       tidyr::pivot_wider(names_from = stat, names_prefix = "daily_", values_from = value)
#     }
#   )
#     
#     return(agg_stats_list)
...
FuncBaseMap <- function(maptype) {
  ...
  map <- leaflet(options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.25)) %>% # ORIGINAL INITIAL_MAP HAD leaflet(options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.25)) %>%...
...
FuncUpdateFilter <- function() {
  ...
  # updateNumericInput(session,
  #                    "sel_MinDepth",
  #                    value = min(rv$dat$ActivityBottomDepthHeightMeasure.MeasureValue, na.rm = TRUE))
  # 
  # updateNumericInput(session,
  #                    "sel_MaxDepth",
  #                    value = max(rv$dat$ActivityBottomDepthHeightMeasure.MeasureValue, na.rm = TRUE))
...
FuncPlotlyHeatMap <- function(dat, y_name, facet_by_name, legend_name, is_discrete = FALSE, indiv_scale = FALSE) {
  ...
  heatplot_list <- lapply(levels(template$FacetBy), function(i) {
    ...
    mat_values <- matrix(pull(subdat, FillCol),
    ...
    mat_hover <- matrix(pull(subdat, HoverLabel), nrow=length(unique(subdat$Y)), byrow = TRUE)
...
FuncPlotHeatMap <- function(dat, y_name, legend_name) {
  ...
  # Create color scale
    ...
    sites_per_char <-dat %>% distinct(CharacteristicName, MonitoringLocationIdentifier) %>% count(CharacteristicName) %>% pull(.)
    
    temp_distinct <- dat %>%
      dplyr::select(CharacteristicName, MonitoringLocationIdentifier, TimeUnit) %>% distinct()
...
FuncPlotDat <- function() {
  ...
  # plotDat$ResultMeasureValue_replaced <- plotDat$ResultMeasureValue
  # plotDat$ResultMeasureValue_replaced[plotDat$ResultDetectionConditionText != "Detected and Quantified" & is.na(plotDat$ResultMeasureValue) & !is.na(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)] <-
  #     as.numeric(plotDat$DetectionQuantitationLimitMeasure.MeasureValue[plotDat$ResultDetectionConditionText != "Detected and Quantified" & is.na(plotDat$ResultMeasureValue) & !is.na(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)]) # create a column in which the detection limit replaces NA, for records where result was above or below detection limit--this column will be used to calculate probability quantiles <<<<<<<<<<<<<<< INCLUDE THIS EXPLANATION IN DOCUMENTATION
    ...
    rv_param_temp$threshType <- input$sel_ShowThresholds # save the input setting so plots don't change unless the plot data has been refreshed
...
FuncGageDat <- function(gage_station) {
  ...
  shiny::req(!is.null(rv_param_temp$plotDat), !is.null(rv$gagesDat), !is.null(input$sel_DefineSeasons))
...
FuncPlotScatter <- function(plot_dat, quant_df = NULL, categ_df = NULL, x_nam = "ActivityStartDate", combine_stations = FALSE, fixed_free_y = "fixed", trans_y = "identity", trans_gage_y = "identity", trans_precip_y = "identity", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, gage_dat = NULL, precip_dat = NULL, month_summary = FALSE, highlight_yr = NULL) {
  ...
  #   highlight_yr: In seasonal point plots, year of data to highlight
  #
  # Returns:
  #   List of scatterplot
  # 
...
if(x_nam == "ActivityStartDate") {
  ...
  if(input$sel_Categ != "none" & !is.null(categ_df)) { 
    ...
    p_points <- p_points +  
      ...
      if(rv_param_temp$plotSummarizeBy == "by_char" & combine_stations==TRUE) { # if coloring by monitoring site, use viridis color scale
        p_points <- p_points +
          geom_point(aes(shape = pointShape, color = MonitoringLocationIdentifier, fill = MonitoringLocationIdentifier, text = paste0("Characteristic Set: ", CharacteristicSet, "<br>Monitoring Site: ", MonitoringLocationIdentifier, "<br>Survey Date: ", ActivityStartDate, "<br><br>X-value: ", x, "<br>Parameter Value: ", y, "<br>Detection Level: ", ResultDetectionConditionText)), size = 2, show.legend = c(fill = FALSE, color = TRUE, shape = FALSE)) + # this will give a false unknown aesthetics warning for 'text'
          scale_fill_viridis_d() +
          scale_color_viridis_d(name = "Site")
      } else { # ...if not coloring the points...
      p_points <- p_points +
        geom_point(aes(shape = pointShape, text = paste0("Characteristic Set: ", CharacteristicSet, "<br>Monitoring Site: ", MonitoringLocationIdentifier, "<br>Survey Date: ", ActivityStartDate, "<br><br>X-value: ", x, "<br>Parameter Value: ", y, "<br>Detection Level: ", ResultDetectionConditionText)), size = 2, color = "black", show.legend = FALSE)
      }
  }
  ...
  if(incl_cens) {
    ...
    if(nrow(dat_cens) > 0) {
      ...
      p1_points <- p1_points +
        geom_segment(data = dat_cens, aes(x = x, xend = x, y = segment_start, yend = segment_end), size = 0.25, linetype = "dotted")
    }
  }
  
  # If highlighting a certain year in seasonal plots...
    if(!is.null(highlight_yr)) {
      highlight_dat <- subset(dat_df, Yr == highlight_yr, select = c(x, y))
      if(nrow(highlight_dat) > 0) {
        p1_points <- p1_points +
          geom_point(data = highlight_dat, aes(x = x, y = y), color = "turquoise2", size = 2.5)
      }
    }
  ...
  if(add_loess) {
    ...
    if(rv_param_temp$plotSummarizeBy == "by_char" & combine_stations==TRUE) {
      p1_points <- p1_points + 
        geom_smooth(aes(color = MonitoringLocationIdentifier, fill = MonitoringLocationIdentifier), method = "loess", se = FALSE, size = 0.5, alpha = 0.3, show.legend = FALSE) +
  ...
  if(input$sel_AddGagePrecip == "streamflow" & !is.null(gage_dat)) {
    if(x_nam=="ActivityStartDate") {
      if(month_summary == TRUE) {
        ...
        geom_line(aes(y = MnthMean_MeanDailyDischarge, group = 1, text = paste0("Gage Name: ", GageName, "<br>Mid-month date: ", x, "<br>Mean daily discharge (cfs), MEAN for month: ", MnthMean_MeanDailyDischarge)), linetype = "solid", colour = "blue") +
        geom_line(aes(y = MnthMax_MeanDailyDischarge, group = 1, text = paste0("Gage Name: ", GageName, "<br>Mid-month date: ", x, "<br>Mean daily discharge (cfs), MAX for month: ", MnthMax_MeanDailyDischarge)), linetype = "dashed", colour = "blue") + # for time series point plots, present data summarized by month
          scale_x_date(date_breaks = "1 year", date_labels = "%Y", limits = c(xaxes_min, xaxes_max))
      } else { # just plot the points
        gage_plot <- ggplot(gage_dat, aes(x = ActivityStartDate, y = MeanDailyDischarge)) +
        geom_point(aes(text = paste0("Gage Name: ", GageName, "<br>Survey date: ", ActivityStartDate, "<br><br>Mean daily discharge (cfs): ", MeanDailyDischarge))) + 
        scale_x_date(date_breaks = "1 year", date_labels = "%Y", limits = c(xaxes_min, xaxes_max))
        ...
        gage_plot <- ggplot(gage_dat, aes(x = x, y = MeanDailyDischarge)) +
          geom_point(aes(text = paste0("Gage Name: ", GageName, "<br>Survey date: ", ActivityStartDate, "<br><br>X-value: ", x, "<br>Mean daily discharge (cfs): ", MeanDailyDischarge)), alpha = 0.5)
        
        # If highlighting a certain year...
        if(!is.null(highlight_yr)) {
          highlight_gage_dat <- subset(gage_dat, Yr == highlight_yr, select = c(x, MeanDailyDischarge))

          if(nrow(highlight_gage_dat) > 0) {
            gage_plot <- gage_plot +
              geom_point(data = highlight_gage_dat, aes(x = x, y = MeanDailyDischarge), color = "turquoise2", size = 2.5)
          }
          }
  ...
  if(input$sel_AddGagePrecip == "precipitation" & !is.null(precip_dat)) {
    if(x_nam=="ActivityStartDate") {
      ...
      precip_plot <- ggplot(precip_dat, aes(x = x)) +
        geom_line(aes(y = AvgPrecip_cm, group = 1, text = paste0("Weather Station Name: ", MeteoName, "<br>Mid-month date: ", x, "<br>Daily precipitation (cm), MEAN for month: ", AvgPrecip_cm)), linetype = "solid", colour = "blue") +
        geom_line(aes(y = MaxPrecip_cm, group = 1, text = paste0("Weather Station Name: ", MeteoName, "<br>Mid-month date: ", x, "<br>Daily precipitation (cm), MAX for month: ", MaxPrecip_cm)), linetype = "dashed", colour = "blue") + # for time series point plots, present data summarized by month
        scale_x_date(date_breaks = "1 year", date_labels = "%Y", limits = c(xaxes_min, xaxes_max))
      } else { # just plot the points
      precip_plot <- ggplot(precip_dat, aes(x = ActivityStartDate, y = PRCP_cm)) +
        geom_point(aes(text = paste0("Weather Station Name: ", MeteoName, "<br>Survey date: ", ActivityStartDate, "<br><br>Daily precipitation (cm): ", PRCP_cm))) + 
        scale_x_date(date_breaks = "1 year", date_labels = "%Y", limits = c(xaxes_min, xaxes_max))
      ...
      if(add_loess) {
        ...
        precip_plot <- ggplot(precip_dat, aes(x = x, y = PRCP_cm)) +
          geom_point(aes(text = paste0("Weather Station Name: ", MeteoName, "<br>Survey date: ", ActivityStartDate, "<br><br>X-value: ", x, "<br>Daily precipitation (cm)): ", PRCP_cm)), alpha = 0.5)
        
        # If highlighting a certain year...
        if(!is.null(highlight_yr)) {
          highlight_precip_dat <- subset(precip_dat, Yr == highlight_yr, select = c(x, PRCP_cm))

          if(nrow(highlight_precip_dat) > 0) {
            precip_plot <- precip_plot +
              geom_point(data = highlight_precip_dat, aes(x = x, y = PRCP_cm), color = "turquoise2", size = 2.5)
          }
          }
...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, y_scale, highlight_yr = NULL) {
# Includes censored data
  withProgress(message = "Generating plots", detail = "...updating plots", value = 0, {
    ...
    thresh_list <- switch(rv_param_temp$threshType,
      ...
      if(!is.null(input$sel_AddGagePrecip) & !is.null(input$sel_PlotType) & !is.null(input$sel_MonthSummary)) {
        month_summary <- all(input$sel_AddGagePrecip != "none", input$sel_PlotType %in% c("points", "points_smooth"), input$sel_MonthSummary == TRUE)
      } else {
        month_summary <- FALSE
      }
    ...
    if(plot_type %in% c("points", "points_smooth")) {
      ...
      ts_plot <- FuncPlotScatter(plot_dat = plot_dat, quant_df = quant_dat, categ_df = rv_param_temp$categLevelsPlot, x_nam = time_unit, combine_stations = ifelse(length(unique(plot_dat$CharacteristicName)) == 2, ifelse(input$sel_PlotByStation, FALSE, TRUE), FALSE), fixed_free_y = ifelse(free_y, "free_y", "fixed"), trans_y = y_scale, trans_gage_y = gage_YScale, trans_precip_y = precip_YScale, add_loess = ifelse(plot_type == "points_smooth", TRUE, FALSE), incl_cens = incl_cens, thresh_list = thresh_list, rotate_x = TRUE, gage_dat = gageDat, precip_dat = precipDat, month_summary = month_summary, highlight_yr = highlight_yr)}
    ...
    # Get the shared legend if it exists - getting shared legends to format properly with plotly is a PITB. Tried following many stackexchange suggestions, with no success.
      ...
      rv_param_temp$sharedLegend$timeSeriesLegend <- ggpubr::as_ggplot(get_legend(ts_plot[[1]][[1]])) # if a legend exists, extract it
    } else {
      rv_param_temp$sharedLegend$timeSeriesLegend <- NULL
    }

    plot_ht <- 340
    total_plot_height <- plot_ht * (length(ts_plot[[1]]) + as.integer(input$sel_AddGagePrecip!="none")) # Final plot will take the layout of the first plot, so the first plot needs to have the height of final plot. Initially tried to account for x_axis height, but seems it was probably already adjusted for
    ...
    if(rv_param_temp$plotSummarizeBy=="by_char") {
      temp_pie_dat <- rv_param_temp$plotDat %>%
        ...
        left_join(rv_param_temp$plotDat %>%
                    ...
                    group_by(MonitoringLocationIdentifier, Yr) %>%
                    count(CategVar), by = c("MonitoringLocationIdentifier", "Yr")) %>%
      ...
      temp_pie_dat <- rv_param_temp$plotDat %>%
        ...
        left_join(rv_param_temp$plotDat %>%
                    dplyr::rename("CategVar" = limit_type) %>%
                    group_by(CharacteristicName, Yr) %>%
                    count(CategVar), by = c("CharacteristicName", "Yr")) %>%
...
rv <- reactiveValues(parkID = NULL, unitPoly = NULL, HUC10 = NULL, HUC12 = NULL, sitesPointsSub = NULL, meteoStations = NULL, selectedMeteo = NULL, meteoDat = NULL, facilitiesPointsSub = NULL, selectedHucs = NULL, selectedStations = NULL, selectedOriginStation = NULL, selectedUpstreamStations = NULL, selectedDownstreamStations = NULL, selectedUpstreamLines = NULL, selectedDownstreamLines = NULL, orgs = NULL, gageStations = NULL, selectedGages = NULL, gagesDat = NULL, dat = NULL, userSelect = list(), savedFilterResults = NULL, savedFilterResultsYears = NULL)
  
rv_param_temp <- reactiveValues(loadMap = FALSE, resetFilter = FALSE, palOrg = NULL, selectParam = NULL, N_plotly_page = NULL, subDat = NULL, subDatTypes = NULL, subDatYears = NULL, filterNPlot = NULL, showStations = NULL, showCharSets = NULL, plotSummarizeBy = NULL, plotDat = NULL, quantDat = NULL, finalPlot = NULL, seasonalPlot = NULL, plotSummary = NULL, precipDat = NULL, categLevels = NULL, categLevelsPlot = NULL, categLevelsImport = NULL, threshList = NULL, threshCustom = NULL, threshType = NULL, percCateg= NULL, sharedLegend = list(heatLegend = NULL, timeSeriesLegend = NULL, MKLegend = NULL), showDM = FALSE, trendDat = NULL, MK = NULL, MKHeatPlots = NULL, seasonType = NULL)
```
...
# Limited to specified date range and minimum activities number
    ...
    sites <- read_delim("tempSites.csv", "\t", escape_double = FALSE, trim_ws = TRUE) # seems WQP might have changed its download format in Sept 2019, so I switched from 'read_csv' to 'read_delim' with tab, to get it to work
...
observeEvent(eventExpr = input$button_highlightStations, {
  ...
  if(input$sel_SelectMethod == "methodHuc") {
    if(is.null(rv$selectedHucs)) {
      ...
      shiny::req(!is.null(rv$srvelectedHucs))
    }
  ...
  if(input$sel_DistKmUpstream == 0 & input$sel_DistKmDownstream == 0) {
    ...
    rv$selectedStations <- 
      rbind(rv$selectedOriginStation@data, if(!is.null(rv$selectedUpstreamStations)) rv$selectedUpstreamStations@data, if(!is.null(rv$selectedDownstreamStations)) rv$selectedDownstreamStations@data) %>% # combine into one data frame
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  setProgress(1/5, detail = "Formatting data")
...
# Format WQP data ----
  ...
  setProgress(2/5, detail = "Getting streamflow data from selected USGS gage stations")
  ...
  if(http_status(GET(gagesDatURL))$category!="Success") { # make sure the service call is valid
    ...
    showModal(modalDialog(
      ...
      # shiny::req(http_status(GET(gagesDatURL))$category=="Success") # if not a valid service call, abort
  ...
  setProgress(3/5, detail = "Getting precipitation data from selected weather stations")
  ...
  setProgress(4/5, detail = "Saving output files")
  showModal( # Pop-up modal asking user to enter prefix to append to saved files
    modalDialog(
      textInput("out_prefix", "Prefix to append to output files: ",
                placeholder = paste0("WQdash_", rv$parkID, "_", paste0(gsub(pattern = "-", replacement = "", x = Sys.Date()))),
                value = paste0("WQdash_", rv$parkID, "_", paste0(gsub(pattern = "-", replacement = "", x = Sys.Date())))
      ),
      footer = tagList(
  ...
  observeEvent(input$button_goSave, {
    ...
    write_csv(rv$dat, paste0("Data_out/", input$out_prefix, "_WQP_formatted.csv"))
    write_csv(rawDat, paste0("Data_out/", input$out_prefix, "_WQP_raw.csv")) # save the raw WQP data
  
  rv_param_temp$resetFilter <- TRUE
 
    showModal(modalDialog(
      title = "Done",
      paste0("The data for dashboard use have been saved as '", input$out_prefix, ".RDS' in the folder 'Data_out' of the current working directory, ", getwd(), ". For your reference, the raw and formatted WQP data have been saved as '", input$out_prefix, "_WQP_raw.csv' and '", input$out_prefix, "_WQP_formatted.csv' in the same 'Data_out' folder.You may now navigate to the FILTER DATA tab to choose a subset of data to work with.") 
    ))
  })
...
observeEvent(eventExpr = input$button_showSavedFilter, {
  
  if(is.null(rv$dat)) {
    showModal(modalDialog(
      title = "No Data",
      "No data available. Please use the 'Get WQP Data' tab to first import or load water quality data."))
    }
  shiny::req(!is.null(rv$dat))
  
  if(is.null(rv$savedFilterResults)|is.null(rv$savedFilterResultsYears)) {
    showModal(modalDialog(
      title = "No saved filter results",
      "Could not find filter results saved with this .RDS file."))
    }
  shiny::req(!is.null(rv$savedFilterResults) & !is.null(rv$savedFilterResultsYears))
  
  withProgress(message = "Loading saved filter results....", value = 0, {
    
  rv_param_temp$subDat <- rv$savedFilterResults
  rv_param_temp$subDatYears <- rv$savedFilterResultsYears
  ...
  showModal(modalDialog(
    title = "Done",
    "In the 'Filtered Data Summary' table, select a water quality characteristic to see its sampling summaries and raw data."))
  } else {
    showModal(modalDialog(
      title = "No Data",
      "No data meet the filter criteria (e.g., the minimum required number of survey years may be too large)"
    ))
    }
  })
})

# Action to save filter results with the current .RDS file ----
output$button_saveFilter <- downloadHandler(
  
  filename = function() {
    return(here::here(input$saved_data_file$name))
  },
  content = function(file) {
    # update rv$userSelect with current filter settings (except for settings re: whether or not to show some input options)
    ...
    if(is.null(rv_param_temp$subDat)|is.null(rv_param_temp$subDatYears)) {
      showModal(modalDialog(
            title = "No data meet the selected criteria",
            "Please change the selected filter criteria. After filter results appear in the tab window, you can save the results for future use by clicking the 'Save these filter results...' hyperlink"))
    }
    
    shiny::req(!is.null(rv_param_temp$subDat) & !is.null(rv_param_temp$subDatYears))
    
    rv$savedFilterResults <- rv_param_temp$subDat # save the filter output to rv
    rv$savedFilterResultsYears <- rv_param_temp$subDatYears
    
    filter_input <- c("sel_DefineSeasons", "sel_SeasonsStart", "sel_Season1Name", "sel_Season1Range", "sel_Season2Name", "sel_Season2Range", "sel_Season3Name", "sel_Season3Range", "sel_Season4Name", "sel_Season4Range", "sel_YrRange", "sel_MinYrs", "sel_MinStations", "sel_ActivityType", "sel_ResultStatus", "sel_LimitDepths", "sel_DepthUnit", "sel_MinDepth", "sel_MaxDepth", "sel_SummaryOrg", "sel_FilterWQChar", "sel_NPSparam", "sel_SummaryParam", "sel_TimeSubunit", "sel_RawFilteredDataCols")
    FuncUpdateUserSelect(inputs_to_update = filter_input) # save the filter settings to rv$userSelect
...
observeEvent(eventExpr = input$button_refreshPlotData, { 
  ...
  rv_param_temp$plotSummarizeBy <- input$sel_SummarizeBy
  ...
  if(isolate(rv_param_temp$plotSummarizeBy) == "by_station") {
...
# Format plot data for time series plot tab
  if(any(is.null(rv_param_temp$subDat),  is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets), is.null(rv_param_temp$plotSummarizeBy), is.null(input$sel_AddQuantile), is.null(input$sel_Categ), is.null(input$sel_AddGagePrecip), is.null(input$sel_ShowThresholds))) { 
   
    rv_param_temp$plotDat <- NULL
  } else {
        withProgress(message = "Generating plot data...", value = 0, {
          
    rv_param_temp$plotDat <- FuncPlotDat() # this function also assigns data to rv_param_temp$quantDat
    
    # shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(rv_param_temp$plotSummarizeBy), !is.null(input$sel_SeasonalTimeUnit), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$threshType))

  # rv_param_temp$finalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = "ActivityStartDate", free_y = input$sel_FreeY, y_scale = input$sel_YScale, highlight_yr = NULL) # the plot should update only if one of these arguments changes
      
    # rv_param_temp$seasonalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = input$sel_SeasonalTimeUnit, free_y = input$sel_FreeY, y_scale = input$sel_YScale)
    
    # rv_param_temp$seasonalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = input$sel_SeasonalTimeUnit, free_y = input$sel_FreeY, y_scale = input$sel_YScale, highlight_yr = input$sel_HighlightSurvYr)
        })
    }
})
```

```{r events_trends_page}
# Action to update MK summaries ----
observeEvent(eventExpr = input$button_updateMK, {
  if(any(is.null(rv_param_temp$plotDat), is.null(rv_param_temp$finalPlot), is.null(rv_param_temp$plotSummarizeBy))) {
    rv_param_temp$trendDat <- rv_param_temp$MK <- rv_param_temp$MKHeatPlots <- NULL

  } else {
    shiny::req(!is.null(input$sel_TrendType), !is.null(rv_param_temp$plotSummarizeBy))

    withProgress(message = "Updating Mann-Kendall summaries....", value = 0, {

    trendDat <- rv_param_temp$plotDat %>%
      dplyr::mutate(Year = as.integer(Yr))
    trendDat$Yr <- as.factor("Annual")
    # trendDat$Mnth = lubridate::month(trendDat$ActivityStartDate, label = TRUE, abbr = TRUE)
    # trendDat$Mnth = factor(trendDat$Mnth, levels = levels(trendDat$Mnth), ordered = FALSE)
    

    trendDat %<>% 
      dplyr::rename("Interval" = switch(input$sel_TrendType, "MK" = "Yr", "MKS" = "Season", "MKM" = "Mnth")) %>%
    ...
    rv_param_temp$trendDat <- switch((nrow(trendDat) > 0)+1, NULL, trendDat)
 
    if(!is.null(rv_param_temp$trendDat)) {
      mk_out_list <- list()
      for(i in unique(trendDat$LocChar)) {
        subdat <- subset(trendDat, LocChar== i)
        MKsubdat <- subdat %>%
        ...
        MKsummary$SenSlope[MKsummary$`%MissingEnds` > 50] <- NA
        MKsummary$RelativeSenSlope[MKsummary$`%MissingEnds` > 50] <- NA
        MKsummary$Pvalue[MKsummary$`%MissingEnds` > 50] <- NA
        
        if(input$sel_TrendType %in% c("MKS", "MKM")) {
          MKsummary$Interval <- factor(MKsummary$Interval, levels = levels(trendDat$Interval))}
        mk_out_list[[i]] <- MKsummary
        
      }
      rv_param_temp$MK <- switch((length(mk_out_list) > 0) + 1, NULL, do.call("rbind", mk_out_list))
      
      MKHeatDat <- rv_param_temp$trendDat %>%
        dplyr::mutate(FacetBy = get(switch((rv_param_temp$plotSummarizeBy == "by_char")+1, "CharacteristicName", "MonitoringLocationIdentifier"))) %>%
        dplyr::rename(Yr = Year, Y = Interval, FillCol = MedianValue) %>%
  dplyr::mutate(
    FacetBy = factor(FacetBy),
    HoverLabel = paste0(MonitoringLocationIdentifier, "<br>", CharacteristicName, "<br>", rv_param_temp$seasonType, ": ", Yr, "<br>MEDIAN for ", Y, ": ", FillCol))
      
      out_MKPlot <- FuncPlotlyHeatMap(dat = MKHeatDat, y_name = "Interval", facet_by_name = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, "Characteristic Name", "Monitoring Site"), legend_name = "Median Value", is_discrete = FALSE, indiv_scale = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, TRUE, FALSE))
      
  rv_param_temp$sharedLegend$MKLegend <- out_MKPlot$legend
  rv_param_temp$MKHeatPlots <- out_MKPlot$plot
  
    } else {
      rv_param_temp$MK <- NULL
      rv_param_temp$MKHeatPlots <- NULL
      rv_param_temp$sharedLegend$MKLegend <- NULL
    }
    })
    
  #   showModal(modalDialog(
  #     title = "Done",
  #     "You may now view results in the summary and analysis tabs"
  # ))
  }
})
```
...
<font size="4"><center> *“If we have data, let’s look at data. If all we have are opinions, let’s go with mine.” – Jim Barksdale, former Netscape CEO*</center></font>
...
FuncAboutButton(nam = "button_aboutGetWQPData")
observeEvent(eventExpr = input$button_aboutGetWQPData, {
  browseURL(here::here("About_files", "aboutGetWQPData.html"))})
...
wellPanel(
  ...
  radioButtons("sel_GetData",
               label = NULL,
               choices = c("Load data file from computer"  = "existing_data",
...
# <<<<<<<<<< CHECK THESE LIMITS
# https://edits.nationalmap.gov/arcgis/rest/services/WBD/Wbd_PostVersion/FeatureServer/2/serviceLimits?f=geojson
```
...
output$table_selected_orgs <- renderReactable({
  ...
  df_site <- rv$sitesPointsSub@data %>%
    ...
    mutate(SiteStartYr = min(MeteoYr, na.rm = TRUE), 
           ...
           TotalSurveyYrs = n_distinct(MeteoYr)) %>%
...
```{r summarize_gage_stations}
output$table_gages <- DT::renderDT({
  # shiny::validate(
  #   need(!is.null(rv$dat), message = "No data available. Please first import or load water quality data."))
  shiny::req(!is.null(rv$dat), !is.null(rv$gageStations), !is.null(rv$selectedGages))
  shiny::req(all(rv$selectedGages$GageID %in% rv$gageStations$GageID))
  
  DT::datatable(
    rv$selectedGages%>% dplyr::select(GageName, GageID, StartDate, EndDate, NumRecords),
    filter = "top",
    rownames = FALSE,
    selection = "none",
    options = list(
...
DTOutput("table_gages")
tags$style("#table_gages{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```

### Selected Weather Stations

```{r summarize_weather_stations}
output$table_weather <- DT::renderDT({
  # shiny::validate(
  #   need(!is.null(rv$dat), message = "No data available. Please first import or load water quality data."))
  shiny::req(!is.null(rv$dat), !is.null(rv$meteoStations), !is.null(rv$selectedMeteo))
  shiny::req(all(rv$selectedMeteo$MeteoID %in% rv$meteoStations$MeteoID))
  
  DT::datatable(
    rv$selectedMeteo %>% dplyr::select(MeteoName, `Station ID` = MeteoShortID, Elevation_m = Elev, StartYr, EndYr, DataCoverage) %>%
      mutate(StartYr = as.integer(StartYr),
             EndYr = as.integer(EndYr),
             `DataCoverage%` = 100 * DataCoverage) %>%
      dplyr::select(-DataCoverage),
    filter = "top",
    rownames = FALSE,
    selection = "none",
    options = list(
...
DTOutput("table_weather")
tags$style("#table_weather{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```

### Discharge Facilities

```{r summarize_discharge_facilities}
output$table_discharge <- DT::renderDT({
  # shiny::validate(
  #   need(!is.null(rv$dat), message = "No data available. Please first import or load water quality data."))
  shiny::req(!is.null(rv$dat), !is.null(rv$facilitiesPointsSub))
  
  DT::datatable(
    rv$facilitiesPointsSub@data %>%
      dplyr::select(CWPName, `Facility ID` = SourceID, Street = CWPStreet, City = CWPCity, State = CWPState, County = CWPCounty, FacilityType = CWPFacilityTypeIndicator),
    filter = "top",
    rownames = FALSE,
    selection = "none",
...
DTOutput("table_discharge")
tags$style("#table_discharge{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```
...
FuncAboutButton(nam = "button_aboutCheckFilterData")
observeEvent(eventExpr = input$button_aboutCheckFilterData, {
  browseURL(here::here("About_files", "aboutCheckFilterData.html"))})
...
# actionButton("button_showSavedFilter", "USE SAVED FILTER RESULTS", style="border-color:black; background-color: lightblue; border:0px; width:100%; margin:0px") # refreshes with the saved filter results
# 
# br()
# 
# downloadLink("button_saveFilter", label = "Save filter results to the current .RDS file")
...
rv_param_temp$selectParam <- rv$dat %>% 
        ...
        dplyr::mutate(TotalSurveyYrs = n_distinct(Yr)) %>%
        dplyr::filter(TotalSurveyYrs >= input$sel_MinYrs) %>%
        dplyr::select(-Yr) %>%
        dplyr::distinct() %>%
        dplyr::ungroup() %>%
      ...
      checkboxGroupInput("sel_SummaryParam",
                         label = h6("Water quality characteristics to summarize: "),
                         choices = sort(rv_param_temp$selectParam),
                         selected = rv_param_temp$selectParam[rv_param_temp$selectParam %in% c("Dissolved oxygen (DO)", "Escherichia coli", "Flow, severity (choice list)", "Flow, stream stage (choice list)", "Nitrate", "pH", "Specific conductance", "Temperature, water", "Turbidity")]
      )
    }),
...
output$out_SamplePlots <- renderPlot({ 
  ...
  # Create sample size matrix
    ...
    distinct(), expand_grid(TimeUnit = levels(N_dat$TimeUnit), Yr = min(N_dat$Yr, na.rm = TRUE):max(N_dat$Yr, na.rm = TRUE))) %>%
  ...
  p(strong("2. Specify category levels")),
         renderUI({
           shiny::req(!is.null(input$sel_GetCategFile), !is.null(input$sel_CategParam))
              ...
              rv_param_temp$categLevels <- rv$dat %>% dplyr::filter(CharacteristicName == input$sel_CategParam, !is.na(ResultMeasureValue)) %>% dplyr::select(ResultMeasureValue) %>% distinct() %>% arrange() # can't just assign a vector to a reactive value initially assigned as NULL because it's expecting a dataframe or a single variable
...
...Time Series {data-navmenu="Explore WQP Data"}
======================================  
...
FuncAboutButton(nam = "button_aboutTSPlots")
observeEvent(eventExpr = input$button_aboutTSPlots, {
  browseURL(here::here("About_files", "aboutTSPlots.html"))})
...
renderUI({
  radioButtons("sel_SummarizeBy",
               ...
               choiceNames = list("characteristic", "site"),
...
tags$div(renderText({ 
  shiny::req(!is.null(rv_param_temp$finalPlot), !is.null(input$sel_SummarizeBy), !is.null(rv_param_temp$showCharSets))
...
# If summarizing by characteristic...
renderUI({
...
renderUI({
  ...
  renderUI({
    radioButtons("sel_Categ",
                 ...
                 selected = switch(is.null(input$sel_Categ)+1, input$sel_Categ, "none")
                 )
    }),
...
p(strong("Add streamflow/precip")),
  ...
  renderUI({
    shiny::req(!is.null(input$sel_AddGagePrecip), !is.null(input$sel_PlotType))
    shiny::req(input$sel_AddGagePrecip != "none" & input$sel_PlotType %in% c("points", "points_smooth"))
  checkboxInput("sel_MonthSummary",
                      label = "Summarize time series point plot by month-year",
                      value = TRUE)
  })
) # end of well panel
...
Time Series Plots {.tabset .tabset-fade}
-------------------------------------
...
renderPlot({ # separate validation statements so they are checked in sequence
  ...
  if(any(!rv$userSelect$sel_Organization %in% rv$sitesPointsSub@data$OrganizationFormalName, is.null(rv_param_temp$finalPlot), is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets), is.null(rv_param_temp$sharedLegend$timeSeriesLegend))) { # this if-statement forces the plot to be reset to blank when these conditions apply
    ts_plotly_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    ts_plotly_legend <- grid.arrange(rv_param_temp$sharedLegend$timeSeriesLegend)
  }
  ts_plotly_legend
...
renderPlotly({ # separate validation statements so they are checked in sequence
  ...
  shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$threshType))

  rv_param_temp$finalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = "ActivityStartDate", free_y = input$sel_FreeY, y_scale = input$sel_YScale, highlight_yr = NULL) # the plot should update only if one of these arguments changes
  
  shiny::req(!is.null(rv_param_temp$finalPlot))
  rv_param_temp$finalPlot
  })
unlink("Rplots.pdf")
```
...
<div align="center">
```{r seasonal_plots_slider}
# renderUI({
#   shiny::req(!is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$seasonalPlot))
#   shiny::req(length(unique(rv_param_temp$plotDat$Yr)) > 0)
# sliderTextInput("sel_HighlightSurvYr", force_edges = TRUE, label = h4(strong("Choose a survey year to highlight blue in point plots")), choices = sort(unique(rv_param_temp$plotDat$Yr)), selected = switch(is.null(input$sel_HighlightSurvYr)+1, input$sel_HighlightSurvYr, max(rv_param_temp$plotDat$Yr, na.rm = TRUE)), dragRange = FALSE, animate = FALSE, width = "500px")
#   })
```
</div>
...
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv$userSelect), !is.null(rv$sitesPointsSub), !is.null(rv_param_temp))
  if(any(!rv$userSelect$sel_Organization %in% rv$sitesPointsSub@data$OrganizationFormalName, is.null(rv_param_temp$seasonalPlot), is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets), is.null(rv_param_temp$sharedLegend$timeSeriesLegend))) { # this if-statement forces the plot to be reset to blank when these conditions apply
    ts_plotly_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    ts_plotly_legend <- grid.arrange(rv_param_temp$sharedLegend$timeSeriesLegend)
  }
...
renderPlotly({
  ...
  shiny::req(!is.null(rv_param_temp$seasonalPlot))

rv_param_temp$seasonalPlot
})
```

### Trends in Threshold Levels
...
...Spatial Patterns {data-navmenu="Explore WQP Data"}
====================================== 
Inputs {.sidebar data-width=250}
-------------------------------------
```{r summary_sidebar}  
br()
FuncAboutButton(nam = "button_aboutSpatialPatterns")
observeEvent(eventExpr = input$button_aboutSpatialPatterns, {
  browseURL(here::here("About_files", "aboutSpatialPatterns.html"))})
br()
br()
wellPanel(
...
renderUI({
  shiny::req(!is.null(rv_param_temp$plotSummarizeBy))
  radioButtons("sel_ParamMapShow",
                 label = h6("Show on map: "),
                 choiceNames = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, "Selected monitoring sites", c("Selected monitoring sites", "Annual mean", "Annual threshold categories", "Annual percentile categories")),
                 choiceValues = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, "show", c("show", "mean", "thresh", "perc")),
                 selected = "show")
}),
...
Map
-------------------------------------
<center>*Time lapse map of means and threshold/percentile categories for data from time series plots*</center>

```{r param_map}
output$param_map <- renderLeaflet({
  
  FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = TRUE) # make sure data have been imported and filtered, and plot data have been generated
  shiny::req(!is.null(rv_param_temp$subDat), !is.null(rv$sitesPointsSub), !is.null(rv$unitPoly), !is.null(rv$userSelect), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$palOrg))
...
iconMeteo = makeAwesomeIcon(icon= "fa-cloud", library = "fa", squareMarker = TRUE, markerColor = "white")
  ...
  param_map <- leaflet() %>%
    addMapPane("pane_param_selectedStations", zIndex = 250) %>% # always keep selected monitoring station points on "top" of map layers (highest zIndex)
    addMapPane("pane_param_availableStations", zIndex = 240) %>%
    ...
    addPolygons(data = rv[[rv$userSelect$sel_HUClevel]], group = paste0(rv$userSelect$sel_HUClevel, " boundaries"), color = "blue", fillColor = "lightblue", fillOpacity = 0.3, weight = 0.7, layerId = ~HUC, options = pathOptions(pane = "pane_param_HUC")) %>% # HUC polygons
    addScaleBar() %>%
    addControl(rv$parkID, position = "topleft") # add 4-letter UNIT code on map
  
  # if(!is.null(input$sel_ShowChar) & !is.null(input$sel_SurvYr) & !is.null(input$sel_ParamMapShow)) {
  ...
  show_layers <- data.frame(layer = c(paste0(rv$userSelect$sel_HUClevel, " boundaries"), "Imported monitoring sites", "Selected monitoring sites", "Imported stream lines", "Discharge facilities", "Available gage stations", "Available weather stations"), show = c(TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE)) # dataframe to track what layers to show
  
  show_char <- switch((rv_param_temp$plotSummarizeBy == "by_char")+1, input$sel_ShowStationChars, input$sel_ShowChar) # if summarize by station, this can be multiple characteristics; otherwise, it's just one characteristic
  param_map %<>%
    addCircleMarkers(data = rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% unique(rv_param_temp$subDat$MonitoringLocationIdentifier[rv_param_temp$subDat$CharacteristicName %in% show_char]),], group = "Imported monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 0.8, stroke = TRUE, weight = 1, color = ~rv_param_temp$palOrg(OrganizationFormalName), opacity = 0.7, options = pathOptions(pane = "pane_param_availableStations")) # WQ stations for which data are available
...
if(!is.null(rv$selectedGages)) { 
  ...
  # Highlight stations that are in the summary plots...
  if(input$sel_ParamMapShow == "show") {
    
    param_map %<>%
      addCircleMarkers(data = switch((rv_param_temp$plotSummarizeBy == "by_station") + 1, rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% input$sel_ShowStations,], rv$sitesPointsSub[rv$sitesPointsSub@data$MonitoringLocationIdentifier %in% input$sel_ShowOneStation,]), group = "Selected monitoring sites", lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~rv_param_temp$palOrg(OrganizationFormalName), fillOpacity = 1, stroke = TRUE, weight = 4, color = "white", opacity = 1, options = pathOptions(pane = "pane_param_selectedStations")) # selected WQ stations are shown as larger, white-outlined points
  }
  
  # If summarizing by characteristic, then additional summary options are available     
  if(rv_param_temp$plotSummarizeBy == "by_char") {
    shiny::req(input$sel_SurvYr)
    
    # summarize stations by a single characteristic, user can choose from several summary functions...
    # If map choice is 'mean'
    if(input$sel_ParamMapShow == "mean") {
      ...
      temp_mean <- rv_param_temp$plotDat %>% 
        dplyr::group_by(MonitoringLocationIdentifier, Yr) %>%
        summarise(mean_val = round(mean(ResultMeasureValue, na.rm = TRUE), 2)) %>%
      ...
      palNumeric <- colorNumeric(
        palette = "Reds",
        domain = temp_mean$mean_val)
      
      temp_mean_plot <- temp_mean %>%
        filter(Yr == as.numeric(input$sel_SurvYr)) %>% # Do this after setting the range of values for legend
        dplyr::mutate(SITEPOP = paste0("Site: ", MonitoringLocationIdentifier, "<br>Year: ", Yr, "<br>Mean Value: ", mean_val))
      param_map %<>%
        addControl(tags$div(tag.map.title, HTML(input$sel_ShowChar, " in Year ", input$sel_SurvYr)), position = "topleft", className="map-title") %>%
        addCircleMarkers(data = temp_mean_plot, lat = ~LatitudeMeasure, lng = ~LongitudeMeasure, label = ~SITELAB, popup = ~SITEPOP, radius = 15, fillColor = ~palNumeric(mean_val), fillOpacity = 1, color = ~palNumeric(mean_val), stroke = FALSE, opacity = 1, options = pathOptions(pane = "pane_param_selectedStations")) %>%
        addLegend(pal = palNumeric, values = temp_mean$mean_val, position = "topleft", title = input$sel_ShowChar) %>%
  ...
  param_map %<>%
    ...
    addLayersControl(
      ...
      options = layersControlOptions(collapsed = FALSE)) %>% # set map view at current settings or, if initial map viewing, set to initial defaults. Irritating that it blinks every time, but it seems this can't be easily addressed with leaflet (html-based) outputs
...
...Summary Tables {data-navmenu="Explore WQP Data"}
======================================
Inputs {.sidebar data-width=355}
-------------------------------------
```{r plot_data_sidebar}
...
FuncAboutButton(nam = "button_aboutPlotDataTables")
observeEvent(eventExpr = input$button_aboutPlotDataTables, {
  browseURL(here::here("About_files", "aboutPlotDataTables.html"))})
...
FuncAboutButton(nam = "button_aboutTrendAnalysis")
observeEvent(eventExpr = input$button_aboutTrendAnalysis, {
  browseURL(here::here("About_files", "button_aboutTrendAnalysis.html"))})

br()

br()

actionButton("button_updateMK", "UPDATE MK RESULTS", style="border-color:black; background-color: lightgreen; width:225px; border:0px; margin:0px") # refreshes the filter results

br()

wellPanel(
  id = "MK_type",
  p(strong("Choose a trend test")),
selectInput("sel_TrendType",
            label = "Type of trend test:",
            choices = c("Mann-Kendall" = "MK", "Mann-Kendall by Season" = "MKS", "Mann-Kendall by Month" = "MKM"),
            # choices = c("Mann-Kendall" = "MK", "Mann-Kendall by Season" = "MKS", "Mann-Kendall by Month" = "MKM", "Pettitt change point (year)" = "PCP"),
            selected = "MKS")
)
```

Mann-Kendall Trend Analysis {.tabset .tabset-fade}
-------------------------------------
### MK Analysis Results
#### <font size="3">**Analysis Considerations**</font>

* Results are not calculated if >50% of data are missing or if >50% of 'ends' are missing (see Column Definitions)
  
* Serial correlation is ignored, so the interval between points should be long enough to avoid strong serial correlation.
    
    
#### <font size="3">**Column Definitions (MK Analysis tab)**</font>
    
* Interval: Within-year interval for which trend is estimated. If multiple values are available within a season, the median is used for analyses.
        
* SenSlope: The median slope joining all pairs of observations, expressed by quantity per unit time. A positive value means values are increasing over time.
          
* RelativeSenSlope: The slope joining each pair of observations is divided by the first of the pair before the overall median is taken. Comparing relative slopes is useful when the water quality characteristics have different units. Relative slope is NOT valid for characteristics that allow negative values (e.g., temperature).
            
* Pvalue: Approximate p-value with corrections for ties and continuity if n > 10 or if there are any ties; otherwise, exact p-values based on Helsel and Hirsch (2002). Measures the significance of a monotonic time series. Commonly, p > 0.05 is considered a statistically significant trend.
            
* %MissingEnds: The proportion missing out of possible comparisons between the first and last 20% of the years. For Mann-Kendall by season or month, if >50% ends is missing for a time period, the estimate is considered unreliable  (Schertz et al 1991) and therefore is not calculated.

####
```{r MK_analysis_results}
output$table_MKsummary <- DT::renderDT({

  FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = TRUE, MK = TRUE) # make sure data have been imported and filtered, and plot data have been generated
  shiny::req(!is.null(rv_param_temp$trendDat), !is.null(rv_param_temp$MK))
  
  DT::datatable(rv_param_temp$MK,
                filter = "top",
                rownames = FALSE,
                selection = "none",
                options = list(
      ...
      lengthMenu = c(5, 10, 15, 20),
        ...
        formatStyle("Pvalue", target = "row", backgroundColor = styleInterval(0.05, c("yellow", "white")))
})

DTOutput("table_MKsummary")
tags$style("#table_MKsummary{height:500px;overflow-x:scroll;overflow-y:scroll}")
```

### Matrix of Calculated Medians

#### <font size="3">**Heat Map Notes**</font>

* Colors represent the median value calculated for each combination of time interval and year (e.g., Spring 2019), scaled from light gray (lowest medians) to dark red (highest medians)

* Colors are scaled to each water quality characteristic separately (e.g., the color scale used for pH is not the same as the color scale used for Specific Conductance)

* Intervals with missing data are shown as blank (white) cells

```{r MK_medians_legend}
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(input$sel_TrendType), !is.null(rv_param_temp$trendDat), !is.null(rv_param_temp$MK), !is.null(rv_param_temp$MKHeatPlots))

  if(any(is.null(input$sel_TrendType), is.null(rv_param_temp$trendDat), is.null(rv_param_temp$MK), is.null(rv_param_temp$MKHeatPlots))) { # this if-statement forces the plot to be reset to blank when these conditions apply
    MK_heatplotly_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    MK_heatplotly_legend <- grid.arrange(rv_param_temp$sharedLegend$MKLegend)
  }
  MK_heatplotly_legend
}, height = 40)

```

```{r MK_medians}
output$out_MKHeat <- renderPlotly({
  FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = TRUE, MK = TRUE)

  shiny::req(!is.null(input$sel_TrendType), !is.null(rv_param_temp$trendDat), !is.null(rv_param_temp$MK), !is.null(rv_param_temp$MKHeatPlots))

  rv_param_temp$MKHeatPlots
})

# tags$style("#out_MKHeat{height:100vh;overflow-x:scroll;overflow-y:scroll}")
plotlyOutput('out_MKHeat', height="100%", width = "100%")
``` 

### MK Data

```{r MK_data_summary}
output$table_MKraw <- DT::renderDT({

  shiny::req(!is.null(rv_param_temp$trendDat))

  DT::datatable(subset(rv_param_temp$trendDat, select = -LocChar),
                filter = "top",
                rownames = FALSE,
                selection = "none",
                options = list(
...
DTOutput("table_MKraw")
tags$style("#table_MKraw{height:100vh;overflow-x:scroll;overflow-y:scroll}")
```

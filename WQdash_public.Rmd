...
# > Define categorical should be a hidden tab but on filter data page, if a categorical parameter is selected, a warning/option to define categorical should show
# > Explore data page
# -- When refresh plot, the select data input tab should collapse
# -- Add map toggle to show site labels
# -- When refresh data, then the selected sites should get a number on the map and the plots should also get a number so they can be easily matched up
# -- Add 'guidance info' on how to navigate Explore data page
# -- Plots card user inputs show 'grow' to fit when map is closed, or they should all be left-aligned
...
## NOTES 
# > bslib accordion requires bootswatch themes that screw up flexdashboard formatting; shinyBS::bsCollapse does not work
...
# Format plot data for time series plot tab
  ...
  # , is.null(input$sel_AddQuantile), is.null(rv_param_temp$sel_add_gage_precip), is.null(rv_param_temp$sel_show_thresholds))) { 
...
wellPanel(
    ...
    actionButton("button_defineCategs", "Define Categorical Characters", 
             icon = icon("sliders"),
             style = "color: black; background-color: lightblue; border: 1px solid black;")
...
showModal(modalDialog(
  ...
  fluidRow(
    ...
    column(3,
      ...
      plotOutput("categ_color_plot")
    ), # end column 2

    # Right Panel
    column(6,
      tags$center(em("Double-click in any cell to edit contents. To delete a row, click it once to select and then choose 'Delete Selected Row(s).'")),
      br(),
      div(style="display:inline-block; width:100px; height:30px; margin-right:10px;",
...
Define Categorical Characters - OLD
====================================== 
...
explore_map_card <-
  bslib::card(
    ...
    full_screen = FALSE,
    card_header(
      HTML('<span style="color: red;">Select sites via the map or sidebar, then click REFRESH PLOT DATA to generate plots</span>')
      ),
    card_body(
      class = "p-0",

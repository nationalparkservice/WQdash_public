...
FuncUpdateFilterInputs <- function() {
  updateRadioButtons(session,
                     "sel_DefineSeasons",
                     selected = rv_param_temp$default$sel_DefineSeasons)
  ...
  updateSliderInput(session,
                    ...
                    max = NULL, 
                    step = NULL, 
                    value = rv_param_temp$default$sel_YrRange)
  
  updateNumericInput(session,
                     "sel_MinYrs",
                     value = rv_param_temp$default$sel_MinYrs)
  
  updateCheckboxGroupInput(session,
                           "sel_ActivityType",
                           selected = rv_param_temp$default$sel_ActivityType)
  
  updateCheckboxGroupInput(session,
                           "sel_ResultStatus",
                           selected = rv_param_temp$default$sel_ResultStatus)
  
  
  updateCheckboxGroupInput(session,
                           "sel_SummaryOrg",
                           selected = rv_param_temp$default$sel_SummaryOrg)
}
...
FuncInitFilter <- function() {
  ...
  rv_param_temp$default$sel_YrRange <- c(min(rv$dat$MeteoYr, na.rm = TRUE), max(rv$dat$MeteoYr, na.rm = TRUE))
  rv_param_temp$default$sel_MinYrs <- 3
  rv_param_temp$default$sel_DefineSeasons <- "MeteoSeasons"
  rv_param_temp$default$sel_SummaryOrg <-sort(unique(rv$dat$OrganizationFormalName))
  rv_param_temp$default$sel_NPSparam <- "National Park Service Water Resources Division" %in% rv$dat$OrganizationFormalName
  rv_param_temp$default$sel_ActivityType <- sort(intersect(unique(rv$dat$ActivityTypeCode), c("Field Msr/Obs", "Field Msr/Obs-Portable Data Logger",  "Sample", "Sample-Routine")))
  rv_param_temp$default$sel_ResultStatus <- sort(intersect(unique(rv$dat$ResultStatusIdentifier), c("Accepted", "Final", "Validated", "Historical")))
  
  shiny::req(!is.null(rv_param_temp$default$sel_NPSparam), !is.null(rv_param_temp$default$sel_YrRange), !is.null(rv_param_temp$default$sel_SummaryOrg), !is.null(rv_param_temp$default$sel_ActivityType), !is.null(rv_param_temp$default$sel_ResultStatus), !is.null(rv_param_temp$default$sel_MinYrs))

  rv_param_temp$selectCharSampleType <- FuncFilterSelectCharSampleType(
    sel_NPSparam = rv_param_temp$default$sel_NPSparam, 
    sel_YrRange = rv_param_temp$default$sel_YrRange, 
    sel_SummaryOrg = rv_param_temp$default$sel_SummaryOrg, 
    sel_ActivityType = rv_param_temp$default$sel_ActivityType, 
    sel_ResultStatus = rv_param_temp$default$sel_ResultStatus, 
    sel_MinYrs = rv_param_temp$default$sel_MinYrs)
...
FuncPlotDat <- function() {
  ...
  plotDat$DetectionQuantitationLimitMeasure.MeasureValue <- as.numeric(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)
    ...
    if(all(rv_param_temp$sel_show_thresholds == "showthresh_custom", rv_param_temp$sel_summarize_by == "by_char")) {
...
observeEvent(eventExpr = input$saved_data_file, {
  ...
  FuncInitFilter()
  FuncUpdateFilterInputs() 
  FuncRefreshFilter(sel_MinYrs = rv_param_temp$default$sel_MinYrs, sel_SummaryParam = rv_param_temp$selectCharSampleType, sel_SummaryOrg = rv_param_temp$default$sel_SummaryOrg, sel_YrRange = rv_param_temp$default$sel_YrRange, sel_ActivityType = rv_param_temp$default$sel_ActivityType, sel_ResultStatus = rv_param_temp$default$sel_ResultStatus, sel_DefineSeasons = rv_param_temp$default$sel_DefineSeasons) # generate the initial filtered data based on default filters. Also call this filter when action button clicked for refreshing filter
...
strong("Define threshold limits for imported data")
...
br()
  ...
  actionButton("button_refreshFilter", "REFRESH FILTER", style="border-color:black; background-color: lightgreen; width:100%; border:0px; margin:0px"), # filters data
    ...
    renderUI({
      shiny::req(!is.null(rv$dat$MeteoYr), !is.null(rv_param_temp$default$sel_YrRange))
    ...
    renderUI({
      shiny::req(!is.null(rv_param_temp$default$sel_MinYrs))
      numericInput("sel_MinYrs",
        ...
        renderUI({
          shiny::req(!is.null(rv_param_temp$default$sel_DefineSeasons))
      radioButtons("sel_DefineSeasons",
                 ...
                 selected = switch(is.null(input$sel_DefineSeasons)+1, input$sel_DefineSeasons, rv_param_temp$default$sel_DefineSeasons))
      }),
  ...
  wellPanel(
    ...
    renderUI({
      shiny::req(!is.null(rv$dat), !is.null(rv_param_temp$default$sel_SummaryOrg))
      checkboxGroupInput("sel_SummaryOrg",
                         ...
                         selected = switch(is.null(input$sel_SummaryOrg)+1, input$sel_SummaryOrg, rv_param_temp$default$sel_SummaryOrg))
    })
    )
...
# shinyjs::hidden( # initially hide this well panel
  wellPanel(
    ...
    renderUI({
      shiny::req(!is.null(rv$dat), !is.null(rv_param_temp$default$sel_NPSparam))
      checkboxInput("sel_NPSparam",
                    "Only show characteristics reported by NPS for the imported monitoring sites",
                    value = rv_param_temp$default$sel_NPSparam)
    }),
    ...
    sel_MinYrs = input$sel_MinYrs)
      ...
      checkboxGroupInput("sel_SummaryParam",
                         ...
                         # selected = input$sel_SummaryParam
...
# Filter data by sampling characteristics
  wellPanel(
    ...
    renderUI({
      shiny::req(!is.null(rv$dat), !is.null(rv_param_temp$default$sel_ActivityType))
    ...
    renderUI({
      shiny::req(!is.null(rv$dat), !is.null(rv_param_temp$default$sel_ResultStatus))
...
renderUI({
  ...
  shiny::req(input$sel_SummarizeBy == "by_station")
    ...
    checkboxGroupInput("sel_ShowStationCharSets",
                         label = h6("Select characteristic set(s):"),
                         choices = sort(unique(rv_param_temp$subDat$CharacteristicSet[rv_param_temp$subDat$CharSampleType %in% input$sel_ShowStationChars & rv_param_temp$subDat$MonitoringLocationIdentifier == input$sel_ShowOneStation])),
                         selected = sort(unique(rv_param_temp$subDat$CharacteristicSet[rv_param_temp$subDat$CharSampleType %in% input$sel_ShowStationChars & rv_param_temp$subDat$MonitoringLocationIdentifier == input$sel_ShowOneStation])))
    ...
    conditionalPanel(
      ...
      shiny::validate(need(input$sel_SummarizeBy == "by_char", message = "This option is available only when summarizing by characteristic (not by station)")),
        selectInput("sel_ThreshUnit",
                    ...
                    choices = sort(unique(na.omit(rv_param_temp$subDat$ResultMeasure.MeasureUnitCode[ rv_param_temp$subDat$CharSampleType == input$sel_ShowCharType]))),
                    selected = switch(is.null(input$sel_ThreshUnit)+1, input$sel_ThreshUnit, sort(unique(na.omit(rv_param_temp$subDat$ResultMeasure.MeasureUnitCode[ rv_param_temp$subDat$CharSampleType == input$sel_ShowCharType])))[1])
        ),

...
saveRDS(isolate(reactiveValuesToList(input)), "TEMP_input.RDS")
  ...
  df <- rv_param_temp$popupSiteSummary %>% dplyr::filter(Site == site)  # Changed: use popupSiteSummary and Site column
  ...
  shiny::showModal(
    shiny::modalDialog(
      ...
      generate_param_table(
        df = df,
        threshold_df =  rv_param_temp$threshDF %>% dplyr::filter(Site == site),  

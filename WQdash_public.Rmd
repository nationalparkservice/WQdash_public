...
conditionalPanel(
  ...
  renderUI({
    ...
    sliderInput("sel_MapZoom", label = h5("Set map zoom (higher values zoom in):"), min = max(1, map_zoom - 6), max = 15, value = map_zoom, step = 0.5, ticks = FALSE, dragRange = FALSE, width = "100%")
    })
)
...
shinyjs::hidden( # initially hide this well panel
  ...
  paste0("WARNING!! Large data imports may trigger time-out errors from web services. Use the filter criteria below to limit import requests to only the necessary data.")
    ...
    radioButtons("sel_SelectMethod", label = h6("Select WQP monitoring sites based on: "),
                 ...
                 # choiceNames = c("Park (include all data collected within the park unit)", "HUCs (click on map to select one or more HUCs)", ),
                 # choiceValues = c("methodPark", "methodHuc"),
                 selected = "methodPark"),
...
explore_map_card <-
  bslib::card(
    ...
    full_screen = FALSE,
    card_header(
      HTML('<span style="color: red;">Select sites via the map or sidebar, then select characteristic(s) from the sidebar and click REFRESH PLOT DATA. From the map, RIGHT CLICK ON ANY SITE for quick-peek parameter summaries.</span>')
      ),
    card_body(
      class = "p-0",
  ...
  sliderInput("sel_ParamMapZoom", label = h6(strong("Set map zoom (higher values zoom in):")), min = max(1, param_map_zoom - 3), max = param_map_zoom + 5, value = param_map_zoom - 1, step = 0.25, ticks = FALSE, dragRange = FALSE, width = "100%")
  })
  ),

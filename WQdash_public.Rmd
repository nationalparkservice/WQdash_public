...
observeEvent(eventExpr = input$button_summarizeStations, {
 ...
 for(i in 1:length(subset_stations_list)) {
    ...
    all_summaries[[i]] <- read_csv("tempSummary.csv", trim_ws = TRUE) %>%
    ...
    rv$selectedDataSummary <- do.call("rbind.data.frame", all_summaries)
    rm(all_summaries)
    unlink("tempSummary.csv")

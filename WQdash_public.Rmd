    ...
    rv_param_temp$plotDat <- NULL
      ...
      withProgress(message = "Formatting plot data...", value = 0, {
          ...
          rv_param_temp$plotDat <- FuncPlotDat() # this function also assigns data to rv_param_temp$quantDat
            ...
            shiny::req(!is.null(input$sel_PlotType), !is.null(input$sel_FreeY), !is.null(input$sel_YScale), !is.null(input$sel_PlotByStation), !is.null(input$sel_AddGagePrecip), !is.null(input$sel_Categ), !is.null(rv_param_temp$plotSummarizeBy), !is.null(rv_param_temp$seasonalTimeUnit), !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$threshType))
#   
...
observeEvent(eventExpr = input$button_updateMK, {
  if(any(is.null(rv_param_temp$plotDat), is.null(rv_param_temp$plotSummarizeBy))) {
    rv_param_temp$trendDat <- rv_param_temp$MK <- rv_param_temp$MKHeatPlots$plotly_page <- rv_param_temp$MKHeatPlots$shared_legend <- NULL
  ...
  dplyr::mutate(
    ...
    HoverLabel = paste0(MonitoringLocationIdentifier, "<br>", CharacteristicName, "<br>", rv_param_temp$seasonType, ": ", Yr, "<br>MEDIAN for ", Y, ": ", FillCol))
      ...
      MKPlot <- FuncPlotlyHeatMap(dat = MKHeatDat, y_name = "Interval", facet_by_name = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, "Characteristic Name", "Monitoring Site"), legend_name = "Median Value", is_discrete = FALSE, indiv_scale = switch((rv_param_temp$plotSummarizeBy == "by_char")+1, TRUE, FALSE))
      
  rv_param_temp$MKHeatPlots$shared_legend <- MKPlot$shared_legend
  rv_param_temp$MKHeatPlots$plotly_page <- MKPlot$plotly_page
  
    } else {
      rv_param_temp$MK <- NULL
  rv_param_temp$MKHeatPlots$shared_legend <- NULL
  rv_param_temp$MKHeatPlots$plotly_page <- NULL
    }
    })
...
renderPlot({ # separate validation statements so they are checked in sequence
  ...
  if(any(!rv$userSelect$sel_Organization %in% rv$sitesPointsSub@data$OrganizationFormalName, is.null(rv_param_temp$finalPlot$shared_legend), is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets))) { # this if-statement forces the plot to be reset to blank when these conditions apply
...
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(rv$userSelect), !is.null(rv$sitesPointsSub), !is.null(rv_param_temp))
  if(any(!rv$userSelect$sel_Organization %in% rv$sitesPointsSub@data$OrganizationFormalName, is.null(rv_param_temp$seasonalPlot$shared_legend), is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets))) { # this if-statement forces the plot to be reset to blank when these conditions apply
    ts_plotly_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
...
```{r MK_medians_legend}
renderPlot({ # separate validation statements so they are checked in sequence
  shiny::req(!is.null(input$sel_TrendType), !is.null(rv_param_temp$trendDat), !is.null(rv_param_temp$MK))

  if(any(is.null(input$sel_TrendType), is.null(rv_param_temp$trendDat), is.null(rv_param_temp$MK), is.null(rv_param_temp$MKHeatPlots$shared_legend))) { # this if-statement forces the plot to be reset to blank when these conditions apply
    MK_heatplotly_legend <- ggplot(dat = data.frame()) + theme_minimal()
  } else {
    MK_heatplotly_legend <- grid.arrange(rv_param_temp$MKHeatPlots$shared_legend)
  }
...
output$out_MKHeat <- renderPlotly({
  ...
  shiny::req(!is.null(input$sel_TrendType), !is.null(rv_param_temp$trendDat), !is.null(rv_param_temp$MK), !is.null(rv_param_temp$MKHeatPlots$plotly_page))

  rv_param_temp$MKHeatPlots$plotly_page
})

...
# > Try package 'polite' again
# > Only show parameters reported by NPS--fix the wording
# > Explore Data should not require Filter

## MODIFY FOR SCI4GA
# county_map <- us_map(regions = "counties", include = "Georgia)
...
if(x_nam == "ActivityStartDate") {
  ...
  if(incl_cens) {
    ...
    # Highlight a year
    if(all(x_nam != "ActivityStartDate", !is.null(input$sel_HighlightSurvYr), input$sel_HighlightSurvYr != "None",  suppressWarnings(as.integer(input$sel_HighlightSurvYr) %in% unique(dat_df$Yr)))) {
...
print("START OF HIGHLIGHT SELECTED")
...
# if(all(any(is.null(input$sel_ParamMapShow), input$sel_ParamMapShow == "show"), isolate(is.null(rv_param_temp$update_from_sidebar))))
      # {
            print("START UPDATE ON FIRST GO AROUND ONLY")
    # isolate({
...
# shinyjs::hidden( # initially hide this well panel
  wellPanel(
    ...
    sel_MinYrs = input$sel_MinYrs)  
      ...
      match_vec <- c("Dissolved oxygen", "Escherichia", "Flow, severity (choice list)", "Flow, stream stage (choice list)", "Nitrate", "pH", "Specific conductance", "Temperature, water", "Phosphorus", "Turbidity")
      init_match <- rv_param_temp$selectCharSampleType[grep(paste(match_vec, collapse = "|"), rv_param_temp$selectCharSampleType)]
...
tags$style("#categDT{height:70vh;overflow-x:scroll;overflow-y:scroll}")
...
wellPanel( # Panel to define high/low values
  id = "define_limits",
...
wellPanel( ## Panel to add streamflow/precip data 
...
# ),
p(plotlyOutput("out_season_plots"))
  ),
fillable = FALSE,
class = "p-0"
...
card_body(
  navset_card_tab(
    nav_panel(
      title = "Time Series",
#       p(renderPlot({ # separate validation statements so they are checked in sequence
#   shiny::req(!is.null(rv$importSettings), !is.null(rv$sitesPointsSub), !is.null(rv_param_temp))
...
nav_panel(
  ...
  p(explore_seasonal_patterns_subcard)
),
nav_panel(
  title = "Annual Threshold Levels",
  p(plotlyOutput("out_thresh_plots"))
),
nav_panel(
  title = "Annual Percentile Levels",

...
shiny::validate(need(length(na.omit(rv_param_temp$plotDat$ThreshCateg)) > 0, message = "No threshold limits defined for the selected characteristics"))
  ...
  thresh_plot_title <- paste0("Threshold levels by ", switch((rv_param_temp$sel_summarize_by == "by_chartype")+1, "water characteristic", "monitoring site"), " (rows) and year (cols) for ", switch((rv_param_temp$sel_summarize_by == "by_chartype")+1, unique(rv_param_temp$plotDat$MonitoringLocationIdentifier), unique(rv_param_temp$plotDat$CharacteristicName)))
...
output$table_selected_orgs <- renderReactable({
  ...
  df_site$TopCharacteristics[df_site$TotalSurveyYrs < 3] <- NA # if a site was surveyed less than 3 years, TopCharacteristics is NA (otherwise can get, e.g., huge list of characteristics surveyed only once because site surveyed only once)
    ...
    df_org <- rv$orgs %>% # alternatively, these column calculations can all be done within reactable()
      ...
      dplyr::select(-TotalSurveyYrs, -Latitude, -Longitude) %>%
      dplyr::distinct() %>%
      arrange(OrganizationFormalName)
...
wellPanel(
  ...
  renderUI({
    ...
    thresh_choice_values <- c("showthresh_none",
                              switch(is.null(rv_param_temp$threshList)+1, "showthresh_file", NA),
    ...
    thresh_default <- switch(("showthresh_file" %in% thresh_choice_values)+1, "showthresh_custom", "showthresh_file")
    ...
    radioButtons("sel_ShowThresholds",
                 ...
                 selected = switch(is.null(input$sel_ShowThresholds)+1, switch((input$sel_ShowThresholds %in% thresh_choice_values)+1, thresh_default, input$sel_ShowThresholds), thresh_default)
    )
    }),
  ...
  renderUI({ # Set custom threshold limits
    ...
    renderUI({
      ...
      numericInput("sel_ThreshUpper",
                     label = "Threshold 2 (higher cutoff value):",

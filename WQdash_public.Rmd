...
if(x_nam == "ActivityStartDate") {
  ...
  if(incl_cens) {
    ...
    if(all(!is.null(input$sel_HighlightSurvYr), input$sel_HighlightSurvYr != "None",  suppressWarnings(as.integer(input$sel_HighlightSurvYr) %in% unique(dat_df$Yr)))) {
      highlight_dat <- subset(dat_df, Yr == as.integer(input$sel_HighlightSurvYr), select = c(x, y))
      if(nrow(highlight_dat) > 0) {
        p1_points <- p1_points +
          geom_point(data = highlight_dat, aes(x = x, y = y), color = "turquoise2", size = 3)
      }
...
FuncPlotPage <- function(plot_list, time_unit) {
  ...
  if(!is.null(get_legend(plot_list[[1]][[1]]))) {
    shared_legend <- ggpubr::as_ggplot(get_legend(plot_list[[1]][[1]])) # if a legend exists, extract it
  } else {
    shared_legend <- NULL
  }
...
```{r rnoaa functions}
# The following are functions from rnoaa, copied verbatim. The package will be retired, so saving the functions here.
    ...
    refresh = FALSE, ...) 
{
    ...
    message("file min/max dates: ", min(dat[[1]]$date), 
        " / ", max(dat[[1]]$date))
    possvars <- paste0(names(dat), collapse = ", ")
    if (any(var != "all")) {
        vars_null <- sort(tolower(var))[!sort(tolower(var)) %in% 
            sort(names(dat))]
        dat <- dat[tolower(var)]
    }
    if (any(sapply(dat, is.null))) {
        ...
        warning(sprintf("%s not in the dataset\nAvailable variables: %s", 
            paste0(vars_null, collapse = ", "), possvars), 
            call. = FALSE)
    }
    if (!is.null(date_min)) {
        dat <- lapply(dat, function(z) dplyr::filter(z, date >= 
            date_min))
    }
    if (!is.null(date_max)) {
        dat <- lapply(dat, function(z) dplyr::filter(z, date <= 
...
meteo_tidy_ghcnd <- function (stationid, keep_flags = FALSE, var = "all", date_min = NULL, date_max = NULL) {
    dat <- suppressWarnings(ghcnd_search(stationid = stationid, 
        var = var, date_min = date_min, date_max = date_max))
    dat <- lapply(dat, meteo_tidy_ghcnd_element, keep_flags = keep_flags)
    cleaned_df <- do.call(rbind.data.frame, dat) %>% tidyr::spread("key", 
        "value")
    to_clean <- c("prcp", "tmax", "tmin", "tavg", 
        "snow", "snwd")
    for (i in seq_along(to_clean)) {
        if (to_clean[i] %in% names(cleaned_df)) {
            cleaned_df[[to_clean[i]]][cleaned_df[[to_clean[i]]] == 
                -9999] <- NA_real_
        }
    }
    return(cleaned_df)
...
meteo_pull_monitors <- function (monitors, keep_flags = FALSE, date_min = NULL, date_max = NULL, var = "all") {
    ...
    all_monitors_clean <- lapply(monitors, safe_meteo_tidy_ghcnd, 
        keep_flags = keep_flags, date_min = date_min, date_max = date_max, 
        var = var)
...
tempMeteoDat <- NULL
  tempMeteoDat <- meteo_pull_monitors(monitors = sort(unique(rv$selectedMeteo$MeteoShortID)), keep_flags = TRUE, date_min = range(rv$dat$ActivityStartDate)[1], date_max = range(rv$dat$ActivityStartDate)[2], var = "PRCP") %>% # pull precipitation data (tenths of mm) for the stations and date range corresponding with downloaded WQP data
...
cat("line5039")
  ...
  # # Highlight year in points plots
  # if(all(input$sel_PlotType %in% c("points", "points_smooth"), !is.null(input$sel_HighlightSurvYr), input$sel_HighlightSurvYr != "None", !is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$ggplot_ts_list), as.integer(input$sel_HighlightSurvYr) %in% unique(rv_param_temp$plotDat$Yr))) {
  #   # shiny::req(as.integer(input$sel_HighlightSurvYr) %in% unique(rv_param_temp$plotDat$Yr))
  #   rv_param_temp$ggplot_ts_list[[1]] <- FuncHighlightYr(rv_param_temp$ggplot_ts_list[[1]])
  # }
...
bslib::card(
  ...
  card_header(
    renderUI({
      ...
      paste0("Data for ",  switch((length(unique(rv_param_temp$plotDat$MonitoringLocationIdentifier)) > 1)+1, unique(rv_param_temp$plotDat$MonitoringLocationIdentifier), "multiple sites"),"<br>", switch((length(unique(rv_param_temp$plotDat$CharacteristicName)) > 1) + 1, unique(rv_param_temp$plotDat$CharacteristicName), "multiple water characteristics"), " (", min(rv_param_temp$plotDat$Yr, na.rm = TRUE), " to ", max(rv_param_temp$plotDat$Yr, na.rm = TRUE), ")")
    })
    ),
  card_body(

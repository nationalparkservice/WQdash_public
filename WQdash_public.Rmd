...
observeEvent(eventExpr = input$button_showStations, {
 ...
 saveRDS(isolate(reactiveValuesToList(input)), here::here("TEMP_input.RDS"))
  ...
  cat(sitesURL)
    ...
    sites$Location_HUCEightDigitCode <- gsub(pattern="-", replacement = "", x= as.character(sites$Location_HUCEightDigitCode)) # Weird issue with BLRI
  ...
  temp_points <- sf::st_as_sf(x = sites, 
                        coords = c("Location_LongitudeStandardized", "Location_LatitudeStandardized"),
...
# 
  ...
  searchURL <- paste0("search?siteid=", stationChoiceString, "&characteristicType=", charTypeChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=basicPhysChem&providers=NWIS&providers=STORET") # REMOVED SITETYPE & MINACTIVITIES & START & END DATES AS AN OPTION AT THIS STAGE)
  
    summaryURL <- paste0("https://www.waterqualitydata.us/wqx3/summary/monitoringLocation/", searchURL, "&summaryYears=all&count=no&dataProfile=periodOfRecord")
  ...
  subset_stations_list <- split(unique(rv$selectedStations$MonitoringLocationIdentifier), ceiling(seq_along(unique(rv$selectedStations$MonitoringLocationIdentifier))/10)) # import data for 10 stations (max) at a time
 
  rv$wqp_url_list <- lapply(subset_stations_list, function(s) {
    
    stationChoiceString_partial <- paste0(s, collapse = "&siteid=")
    paste0("https://www.waterqualitydata.us/wqx3/Result/search?siteid=", stationChoiceString_partial, "&characteristicType=", charTypeChoiceString, "&sampleMedia=Water&mimeType=csv&zip=no&dataProfile=basicPhysChem&providers=NWIS&providers=STORET")
...
observeEvent(eventExpr = input$button_downloadData, { 
  ...
  shiny::validate(need(httr::http_status(GET(wqp_url))$category=="Success", message = FALSE)) # if not a valid service call, abort
...
# Format WQP data ----
 ...
 cat("line3327")
  ...
  allOrgs <- paste(paste0("organization=", unique(rv$dat$OrganizationIdentifier)), collapse = "&")
    orgURL <- paste0("https://www.waterqualitydata.us/wqx3/Organization/search?", allOrgs, "&mimeType=csv&zip=no&providers=NWIS&providers=STEWARDS&providers=STORET")
cat("Line3348")
...
output$table_char_summary <- DT::renderDT({
  ...
  DT::datatable(
    rv$selectedDataSummary %>% dplyr::group_by(CharacteristicType, CharacteristicName) %>% dplyr::summarize(total_activity_count = sum(ActivityCount, na.rm = TRUE), total_result_count = sum(ResultCount, na.rm = TRUE), .groups = "drop"),
    rownames = FALSE,
    filter = "top",
    selection = "none",
    class="compact stripe",

...
### NOTES
# > FuncClearAll does not actually work--see secn wq dashboard for correct implementation
...
FuncPlotDat <- function() {
  ...
  if(any(is.null(input$sel_AddQuantile), input$sel_AddQuantile == FALSE, is.null(input$sel_QuantileProbLower), is.null(input$sel_QuantileProbUpper))) {
      ...
      if(rv_param_temp$sel_summarize_by == "by_station" | (rv_param_temp$sel_summarize_by == "by_chartype" & input$sel_QuantileGroupBy == "quant_each"))  { # if 'by_station' or if 'by_chartype' and calculating separate quantile for each station, then group data by station-characteristic name
...
FuncClearAll <- function() {
  ...
  restart_panels_filter <- c("new_data_P1", "new_data_P2", "new_data_P3", "create_categfile_P1", "create_categfile_P2", "create_categfile_P3", "create_categfile_P4") 
...
observe({
  ...
  Highest_Value_Numeric = max(ResultMeasureValue_replaced, na.rm = TRUE),  
      Lowest_Value = {
        ...
        conditions <- na.omit(unique(ResultDetectionConditionText[ResultMeasureValue_replaced == min_val])) # note: depending on date, it's possible that the minimum value could have more than one detection condition, as detection limits change over time
  ...
  Recent_Values = paste(head(ResultMeasureValue_replaced, 5), collapse = ","),
  Recent_Dates = paste(format(head(ActivityStartDate, 5), "%Y-%m-%d"), collapse = ","),
  Recent_Detection_Conditions = paste(head(ResultDetectionConditionText, 5), collapse = ","),
      .groups = 'drop'
    ) %>%

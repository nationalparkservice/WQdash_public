...
make_params <- function(n) {
  data.frame(
    ...
    Year_Range = paste0(sample(2000:2010, n, TRUE), "–", sample(2015:2024, n, TRUE)),
...
td:nth-child(7) {
  text-align: center;  /* ✅ ensures column 7 (Threshold Summary) centers contents */
}
  "

build_donut_svg <- function(parts, colors) {
  ...
  if (total == 0) {
    return(paste0(
      ...
      sprintf('<text x="25" y="30" text-anchor="middle" font-size="10">%s</text>', total),
      '</svg>'
    ))
  }
  ...
  # Arc paths
  arcs <- mapply(function(start_deg, end_deg, color) {
    large_arc <- as.integer(end_deg - start_deg >= 180)
    ...
    if (end_deg - start_deg >= 359.99) {
      arc1 <- sprintf(
        '<path d="M25,25 L%.2f,%.2f A20,20 0 1,1 %.2f,%.2f z" fill="%s"/>',
    ...
    sprintf(
      '<path d="M25,25 L%.2f,%.2f A20,20 0 %d,1 %.2f,%.2f z" fill="%s"/>',
 ...
 bar_html <- sprintf(
  ...
  low, high, med_pos, pos, val
    ...
    thresh_sub <- threshold_df[
      tolower(threshold_df$MonitoringLocationIdentifier) == tolower(MonitoringLocationIdentifier) &
    ...
    if (nrow(thresh_sub) > 0) {
      ...
      donut_svg <- build_donut_svg(parts, colors)
...
donut_svg <- build_donut_svg(c(1), c("#aaaaaa"))
legend_html <- '<div class="legend-text" style="color:#666666; padding-top: 2px;">No threshold available</div>'
  ...
  '<div style="display: flex; justify-content: center;">
     <div class="donut-container">%s<div class="legend-text">%s</div></div>
   </div>',

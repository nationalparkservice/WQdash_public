...
# TO DO:
# > Add col subtitle saying most recent at top of bar and hover over point to see dates.
...
observeEvent(input$popupmap_right_click, {
  site <- input$map_right_click$MonitoringLocationIdentifier
  df <- popupSiteSummary %>% dplyr::filter(Site == site)  # Changed: use popupSiteSummary and Site column
  ...
  shiny::showModal(
    shiny::modalDialog(
      ...
      generate_param_table(
        df = df,
        threshold_df = threshold_data %>% dplyr::filter(Site == site),  
...
observeEvent(input$param_map_right_click, {
  ...
  df <- rv_param_temp$popupSiteSummary %>% dplyr::filter(MonitoringLocationIdentifier == site)

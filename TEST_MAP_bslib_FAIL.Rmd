...
pkgs <- c("flexdashboard", "shiny", "knitr",  "here", "tidyverse", "janitor", "plyr", "scales", "rtf", "rmarkdown", "shinyWidgets", "leaflet", "RgoogleMaps", "plotrix", "GGally", "plotly", "httr", "jsonlite", "tidyverse", "readr", "sp", "sf", "magrittr", "leaflet.extras", "lubridate", "fontawesome", "viridisLite", "purrr",
          "bslib",
          "bsicons",
          "thematic", 
          "stringr",
...
```{r generic_action_buttons, include = FALSE}
...
FuncPlotDat <- function(sel_categ = switch(is.null(input$sel_Categ)+1, input$sel_Categ, "none")) {
  # Function to format plot data based on user selections on plot page
  #
  # Returns:
  #   Formatted plot data

  cat("STARTING FUNCPLOTDAT")
  shiny::req(!is.null(rv_param_temp$subDat), !is.null(rv_param_temp$plotSummarizeBy))
  ...
  plotDat$DetectionQuantitationLimitMeasure.MeasureValue <- as.numeric(plotDat$DetectionQuantitationLimitMeasure.MeasureValue)
    ...
    if(!is.null(sel_categ)) {
      if(sel_categ != "none") {
        ...
        shiny::validate(
          need(sel_categ %in% unique(rv_param_temp$categLevelsPlot$CharacteristicName), message = "The selected categorical characteristic is not recognized as a categorical characteristic in the dataset. Please first add it to the categorical definitions file."))
        plotCategDat <- rv$dat %>% # pulling from the original data
          dplyr::filter(CharacteristicName == sel_categ) %>%
  ...
  cat("ABOUT TO ASSIGN THRESHOLD LIMITS")
...
FuncPlotScatter <- function(plot_dat, quant_df = NULL, categ_df = NULL, x_nam = "ActivityStartDate", combine_stations = FALSE, fixed_free_y = "fixed", trans_y = "identity", trans_gage_y = "identity", trans_precip_y = "identity", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, add_gage_precip = switch(is.null(input$sel_AddGagePrecip)+1, input$sel_AddGagePrecip, "none"), gage_dat = NULL, precip_dat = NULL, month_summary = FALSE, sel_categ = switch(is.null(input$sel_Categ)+1, input$sel_Categ, "none")) {
...
cat("line1551")  
  ...
  if(add_gage_precip == "streamflow" & !is.null(gage_dat)) {
    if(x_nam=="ActivityStartDate") {
  ...
  if(add_gage_precip == "precipitation" & !is.null(precip_dat)) {
    if(x_nam=="ActivityStartDate") {
      if(month_summary == TRUE) {
...
FuncPlotBox <- function(plot_dat, quant_df = NULL, x_nam = "Yr", combine_stations = FALSE, fixed_free_y = "fixed", trans_y = "identity", trans_gage_y = "identity", trans_precip_y = "identity", add_loess = TRUE, incl_cens = FALSE, thresh_list = NULL, rotate_x = FALSE, add_gage_precip = switch(is.null(input$sel_AddGagePrecip)+1, input$sel_AddGagePrecip, "none"), gage_dat = NULL, precip_dat = NULL) {
...
FuncPlotPage <- function(plot_type, plot_dat, quant_dat, time_unit, free_y, y_scale, categLevelsPlot = rv_param_temp$categLevelsPlot, add_gage_precip = switch(is.null(input$sel_AddGagePrecip)+1, input$sel_AddGagePrecip, "none"), sel_categ = switch(is.null(input$sel_Categ)+1, input$sel_Categ, "none")) {
...
# Includes censored data
    ...
    thresh_list <- switch(rv_param_temp$threshType,
      ...
      # If adding streamflow data...
        if(any(add_gage_precip!="streamflow", is.null(input$sel_GageStation), is.null(rv$gagesDat), is.null(rv$selectedGages))) {
          gageDat <- NULL
          gage_YScale <- NULL
        } else {
      ...
      # If adding precipitation data...
        if(any(add_gage_precip!="precipitation", is.null(input$sel_PrecipStation), is.null(rv$meteoDat), is.null(rv$selectedMeteo))) {
          precipDat <- NULL
          precip_YScale <- NULL
        } else {
      ...
      if(!is.null(add_gage_precip) & !is.null(input$sel_PlotType)) {
        month_summary <- all(add_gage_precip != "none", input$sel_PlotType %in% c("points", "points_smooth"))
      } else {
        month_summary <- FALSE
      }
  cat("line1921")
...
cat("line1930")
    ...
    plot_ht <- 340
    total_plot_height <- 64 + plot_ht * (length(ts_plot[[1]]) + as.integer(add_gage_precip!="none")) # Final plot will take the layout of the first plot, so the first plot needs to have the height of final plot. Initially tried to account for x_axis height, but seems it was probably already adjusted for
    ...
    if(add_gage_precip == "streamflow" & !is.null(gageDat) & "ggplot" %in% class(ts_plot[[3]])) {
...
if(add_gage_precip == "precipitation" & !is.null(precipDat) & "ggplot" %in% class(ts_plot[[4]])) {
...
renderUI({
  ...
  rv_param_temp$startingInputs$map_zoom <- min(RgoogleMaps::MaxZoom(lonrange = rv$unitPoly@bbox["x",], latrange = rv$unitPoly@bbox["y",]))
  })
...
# Format plot data for time series plot tab
  if(any(is.null(rv_param_temp$subDat),  is.null(rv_param_temp$showStations), is.null(rv_param_temp$showCharSets), is.null(rv_param_temp$plotSummarizeBy), is.null(input$sel_AddQuantile),  is.null(rv_param_temp$threshType))) { 
...
```{r map_trends_accord_elements}
# Sidebar for overall data selection ----
accordion_filters <- accordion(
  accordion_panel(
    title = "Select data", 
    value = "accord_filter_wq", 
  ...
  tags$div(renderText({
    ...
    char_incomp <- rv_param_temp$plotDat %>%
      dplyr::group_by(CharacteristicName) %>% 
      dplyr::summarize(num_units = length(unique(na.omit(ResultMeasure.MeasureUnitCode)))) %>%
      dplyr::ungroup() %>% dplyr::filter(num_units > 1) %>% .$CharacteristicName
  ...
  # If summarizing by site (MANY CHARACTERISTICS for one station)...
  renderUI({
    shiny::req(!is.null(input$sel_SummarizeBy), !is.null(rv_param_temp$subDat), !is.null(input$sel_ShowChar), !is.null(input$sel_ShowStations))
...
renderUI({
  ...
  accordion_panel(
    title = "Define threshold categories", 
    value = "accord_thresh",
...
# Popover accordion for additional map formatting ----
# map_gear <- bslib::popover(
#   bs_icon("gear"),
#   title = "Additional map options",
#   
#   renderUI({
#     shiny::req(!is.null(rv$unitPoly))
#     sliderInput("sel_ParamMapZoom", label = h6("Set map zoom (smaller = wider view):"), min = max(1, rv_param_temp$startingInputs$map_zoom - 6), max = 15, value = rv_param_temp$startingInputs$map_zoom - 1, step = 0.5, ticks = FALSE, dragRange = FALSE, width = "95%")
#     }),
#   renderUI({
...
# Popover accordion for additional plot formatting ----
plots_gear <- popover(
  bs_icon("gear"),
...
if(!is.null(rv$selectedGages)) {
  ...
  if(input$sel_ParamMapShow == "show" & isolate(rv_param_temp$update_from_sidebar != FALSE)) # may be TRUE or NULL
    {
    # isolate({
    ...
    # Need to do this on first go-around because don't yet have a param_map to update with proxy. 
      ...
      isolate({ # THIS isolate(local()) is the key to keeping the dashboard from blitzing out
        local(
          if(all(rv_param_temp$update_from_sidebar != FALSE, !identical(rv_param_temp$param_map_highlight, sel_sites))) {
    ...
    if(input$sel_ParamMapShow == "thresh") {
      ...
      param_map %<>%
        ...
        addMinicharts(
          ...
          height = switch(is.null(input$sel_MapPointSize)+1, input$sel_MapPointSize, rv_param_temp$startingInputs$sel_MapPointSize),
          width = iswitch(is.null(input$sel_MapPointSize)+1, input$sel_MapPointSize, rv_param_temp$startingInputs$sel_MapPointSize),
  ...
  param_map %<>%
    ...
    setView(lng = switch((is.null(input$param_map_center$lng))+1, input$param_map_center$lng, mean(rv$unitPoly@bbox["x",])), lat = switch((is.null(input$param_map_center$lat))+1, input$param_map_center$lat, mean(rv$unitPoly@bbox["y",])), zoom = switch(is.null(input$sel_ParamMapZoom)+1, input$sel_ParamMapZoom, rv_param_temp$startingInputs$map_zoom))

})

# Seasonal plots ----
output$out_Seasonal <- renderPlotly({
  ...
  shiny::req(!is.null(rv_param_temp$plotDat), !is.null(rv_param_temp$quantDat))
    ...
    rv_param_temp$seasonalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = rv_param_temp$seasonalTimeUnit, free_y = switch(is.null(input$sel_FreeY)+1, input$sel_FreeY, FALSE), y_scale = input$sel_YScale)
    ...
    sliderInput("sel_MapPointSize", label = "Resize map pie charts:", min = 10, max = 60, value = 30, step = 10, ticks = FALSE, dragRange = FALSE, width = "95%")
      ...
      renderUI({
        ...
        sliderTextInput(
          "sel_SurvYr", 
          force_edges = TRUE, 
          label = h6("Choose a survey year"), 
          choices = sort(unique(rv_param_temp$plotDat$Yr)), 
          selected = max(rv_param_temp$plotDat$Yr, na.rm = TRUE), 
          dragRange = FALSE, animate = TRUE, width = "85%")
        }),
...
renderUI({
  ...
  nav_panel(
    title = "Time Series",
    # Time series plots ----
output$out_TS <- renderPlotly({ # separate validation statements so they are checked in sequence
      cat("check for plot order")
      FuncOrderCheck(import = TRUE, filter_internal = FALSE, filter_external = TRUE, plot = FALSE)
cat("check for plot inputs")
      shiny::validate(
  ...
  cat("About to check for plotDat and quantDat")
      if(!is.null(rv_param_temp$plotDat) &!is.null(rv_param_temp$quantDat)) {
        cat("CREATE PLOT")
        rv_param_temp$finalPlot <- FuncPlotPage(plot_type = input$sel_PlotType, plot_dat = rv_param_temp$plotDat, quant_dat = rv_param_temp$quantDat, time_unit = "ActivityStartDate", free_y = switch(is.null(input$sel_FreeY)+1, input$sel_FreeY, FALSE), y_scale = input$sel_YScale)
        }
  ...
  nav_panel(
    ...
    plotlyOutput("out_Seasonal")
          ),
  nav_panel(
    title = "High-Low Patterns", 
...
page_fillable(
  card(
  card_header("Examine Water Quality Data"),
